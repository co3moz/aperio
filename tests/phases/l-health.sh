...
HEALTH_BACKEND_PID=$!
start_client health "$HEALTH_BACKEND_PORT" APERIO_HOSTNAME=health.e2e.local \
...
kill "$HEALTH_BACKEND_PID" 2>/dev/null || true
sleep 1
start_client health-dead "$HEALTH_BACKEND_PORT" APERIO_HOSTNAME=dead.e2e.local \
...
start_backend "$HEALTH_BACKEND_PORT"
HEALTH_BACKEND_PID=$!
start_client health-slow "$HEALTH_BACKEND_PORT" APERIO_HOSTNAME=slow.e2e.local \
...
start_client wait-gate "$WAIT_BACKEND_PORT" APERIO_HOSTNAME=waitgate.e2e.local \
