...
- [x] **#159 `aperio-client api token create` / `update` cannot set `topics`,
  ...
  `docs/cli-api.md` and `docs/messaging.md` when they land. shipped: all three
  flags on both commands, plus `--clear-topics` on `update`, since an empty
  list on the wire means "keep" and withdrawing messaging needed a way to be
  said; `*` is passed through and the server stores it as `#`, as it does
  from the dashboard. Where it differed: nothing.
...
- [x] **#158 The mutation sweep's five survivors: one test to write, three to
  ...
  with the no-op `mutants` crate, so each reason sits beside the code it
  excuses. The four sweeps' staleness predicate is one `bucket_is_live`
  helper, skipped; `redaction_enabled` is skipped over `redaction_setting`,
  which is tested both ways. Survivor 2 turned out not to be a gap but a
  dead line: `StaticRoutes::compile` refuses a non-positive burst before the
  limiter is asked, which `a_zero_burst_is_refused_by_the_config_before_the_limiter_sees_it`
  already pins, so the `> 0.0` filter in front of the fallback did nothing
  and is removed; the assertion that holds the fallback itself to its word,
  a route with `rps` and no `burst` admitting `rps` immediate requests, is
  written instead. The original entry: From a `cargo mutants` run over the scoped
...
- [x] **#156 A configuration reload takes a service off the air for minutes,
  and the connection it replaced is never reaped.** shipped, all four and
  the drain: make before break in the reload supervisor
  (`wait_for_replacements`, a ten-second handover budget, the missing ones
  named), a Close frame before the old socket is dropped, a server-side
  reaper (`reap_silent_clients`, four down thresholds, on a ten-second
  ticker, through the same `disconnect` notify and cleanup a revoked token
  uses), a steady one-to-two-second retry on a `503` handshake with the
  backoff counter untouched, `max_tunnels` default 10 to 64 with a
  `CONFIG_CHANGES` entry, and the in-flight count made per connection
  behind a `Drop` guard so a reload's drain waits on its own socket's work
  and a panic cannot leak it. What was left as it is: a response that never
  ends still holds its own connection's drain until the budget runs out,
  said so in the changelog. Covered by a unit test on the reaper and the
  handover arithmetic, and an e2e spec that reloads a client under a stream
  of requests and asserts none fails and no ghost remains. The original
  entry: Reported from a running
...
- [x] **#152 A panel hostname: the dashboard at the root of a name of its
  ...
  `APERIO_DASHBOARD_HOSTNAME` for the server and `panel_hostname` on the
  organization record (`PUT /api/orgs/{id}/panel`, an Admin of that
  organization, fenced like a bind), one layer outside every route in
  `server/panel.rs` that moves a panel request under `/aperio` and remembers
  the original path for the login redirect, a panel set cached on the state
  so the bind check and the request path never take the org store's lock,
  binds refused at declaration, token edit, ephemeral tunnel and override,
  live binds dropped when a name becomes a panel, the login fence on the
  form, the passkey and the OIDC callback, the organization's name on the
  login page through the anonymous health body, the `aperio.<domain>`
  suggestion in the org form, the CLI, and a PanelSpec in the e2e suite.
  Where it differed from the plan: the dashboard bundle was not given a
  runtime base, since its history navigation only rewrites the query
  string, so the address bar on a panel stays at `/`; and a fence that no
  longer covers the panel takes it away rather than leaving a dashboard on
  a name the tenant no longer claims. The original entry: Today the only way into the dashboard is
...
- [x] **#146 The rate limiter's arithmetic is not pinned by anything.** The
  ...
  **shipped:** seventeen tests in `state/admission_tests.rs`, and the sweep's
  twenty-two survivors on this module now stand at eighteen killed, two alive
  ...
  all: `StaticRoutes::compile` refuses a non-positive burst before the limiter
  is asked, so the test that has teeth pins the refusal, not the filter. The
  `route_limits.is_empty()` guard is a fast path over an empty list, and
  `matched()` returns `None` on one anyway, so removing it changes nothing.
  The two left alive are the inline failsafes' ten-minute cutoff widened to
  `<=`, which needs `now` threaded through two hot paths to become reachable
  and describes a case with measure zero on a real clock.
...
- [x] **#139 A service the server can reach itself, without the two hops
  ...
  **The name, and the extension that is not coming.** `edge` is taken:
  ...
  longer word. Settled on `server_side: true` on the service, with
  `server_side_targets:` as the operator's list: it says where the last hop is
  made rather than what it avoids.
  ...
  **`direct` was the runner-up and lost on one word.** It is shorter and
  reads well, but `egress.rs` already uses "direct" for "not through the
  outbound proxy" and here it would mean "not through the tunnel". Both are
  "without an intermediary" with different intermediaries, which allows a
  genuinely confusing sentence: a `direct: true` service that the server
  reaches *through* the egress proxy is direct and not direct at once.
  `server_side` says where the last hop is made instead of what it avoids, and
  nothing else in the configuration uses the word.

  **Shipped** as `server_side: true` on a `services:` entry, with
  `server_side_targets:` / `APERIO_SERVER_SIDE_TARGETS` as the operator's list
  and `allow_server_side` on the token. Both have to agree, and an unset list
  permits nothing.

  **The delivery half was never touched, which was the whole trick.** The plan
  assumed a 400-line extraction so both paths could share the response
  handling. They do share it, but by a much smaller change: the answer is
  fetched here and handed back through the *same oneshot channel* a tunnel
  response arrives on. The response timeout, the header rewrite rules, the
  stats and per-organization attribution, the response cache, the inspector
  capture and the access log therefore run the code they already ran and
  cannot tell the two apart. Counting the crossers first is what found it: the
  cut had 20, and looking at what they were showed the seam was in the wrong
  place.

  **A design fault caught while writing it.** The first version resolved the
  target and matched its addresses against the CIDR entries, copying
  `OutboundPolicy::check`. That is sound for a callback made once and unsound
  here, where the server dials the same target for every request for as long
  as the client stays connected: a name admitted because it resolved into an
  allowed range could resolve elsewhere a second later, and the allowlist
  would have approved an address nobody checked. It now judges the target as
  written, so a CIDR entry admits an address and never a name. That also made
  it callable from the declaration path, which holds a lock and cannot await.

  **What the seam tests caught**, which is worth recording as evidence they
  work: both new fields were placed on `ServiceState` without being declared
  in `SERVICE_DECL_IN_SERVICE_STATE` or `SERVICE_SCOPED_DERIVED`, and two
  tests refused the build until somebody said where they belong and why.

  **The honest limitations**, both refused or documented rather than left to
  be discovered. A server-side response is buffered rather than streamed, so a
  large body is held in memory here instead of flowing through; streaming it
  is a separate change. The request body is buffered for the same reason,
  decided in one place in `forward.rs` so the dispatch cannot disagree with it.
  `serve:` is refused at client startup, naming the conflict. And the
  single-service top-level shape does not offer the key at all, because those
  spellings are the deprecated form this project is retiring.
...
- [x] **#136 Nothing checks that data written by an older Aperio still reads.**
  ...
  **Shipped:** three artifacts in `aperio-server/fixtures/`, each written by a
  build that is now the previous one, with a README saying what breaking each
  would look like in production.

  The store fixture is a real 3.46 artifact, not a simulated one: `rusqlite`
  was temporarily put back to 0.32 to generate it, so the file genuinely
  predates the upgrade it guards. It carries a row in all ten tables, which
  turned out to matter more than expected. Dropping a table from it does not
  make the test fail on opening, because the schema is `CREATE TABLE IF NOT
  EXISTS` and quietly recreates it; the row count is what notices the data is
  gone.

  The other two were captured *before* `#134` rather than after, which is the
  only order that helps: a fixture generated by the new version proves the new
  version agrees with itself. So a passkey serialized by `webauthn-rs` 0.5.5
  and a hash written by `argon2` 0.5.3 are now frozen, and the upgrade has
  something to fail against.

  Each was verified to fail for the right reason rather than assumed to work:
  a dropped table reports the table that lost its row, and a field renamed
  inside the stored credential reports `missing field cred_id`.

  The limitation is written into the README rather than left to be discovered.
  A fixture answers "can today's code read what 0.10.0 wrote", not "what any
  supported version wrote", and regenerating one turns it into a test that
  cannot fail, so a format change on purpose adds a file instead.
...
- [x] **#133 The server links two complete HTTP stacks, and unifying them
  ...
  **Shipped:** unified on reqwest 0.13, alias deleted, and the measurement
  held exactly: **414,096 bytes off the server, 2.39%**. The client moved too,
  for one version across the workspace, and grew 18 KB as predicted.

  Two things the plan did not foresee, both worth the entry.

  **`rustls-no-provider` does not fall back, it panics.** reqwest 0.13 aborts
  inside `ClientBuilder::build()` when no process-wide crypto provider is
  installed, where 0.12 would have picked one. Both binaries installed the
  ring provider in `run()`, which was fine for the binaries and wrong for
  everything else: the unit suite went from 2024 tests to 457 the moment the
  version changed, because a test builds clients without ever entering
  `run()`. It is now an idempotent `ensure_crypto_provider()` called by every
  builder in both crates, so the ordering rule is held by the code that needs
  it rather than by a panic. The hazard was real beyond the tests: any future
  caller building a client before startup finished would have taken the
  process down.

  **The trust change is narrower than this entry claimed.** The tunnel dial
  builds its roots from `webpki-roots` explicitly in `dial.rs`, so the
  `wss://` connection was never reqwest's to change and still verifies against
  the compiled-in Mozilla set. What moved to the platform store is outbound
  HTTP: webhooks, OIDC, JWKS, autoscaling callbacks. That split is documented
  in `docs/architecture.md` now, together with the failure it produces on a
  hand-rolled `scratch` image with no CA bundle, where outbound calls fail
  while the tunnel keeps working, a confusing pair of symptoms without the
  map.

  Verified against a real host rather than assumed: an HTTPS request through
  the new stack completes and verifies. `webpki-roots` stays duplicated and
  cannot be helped, `tokio-tungstenite 0.29` pins 0.26.
