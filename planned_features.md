...
- [ ] #123 Per-candidate allowed_ips + denied-request handling — change client `allowed_ips` from route-wide intersection (every declared list must pass) to per-candidate eligibility: sort candidates by priority, filter each by its own list, dispatch to a passing candidate; when none passes, answer with the `denied:` redirect of the most-primary rejecting entry (declared in the tunnel handshake — blocked traffic still never enters the tunnel), or stealth (same answer as an unclaimed route) when no rejecting entry declares one. Open points: union semantics is fail-open (an unrestricted client joining a route opens it — route-wide lockdown moves to the token-level IP allowlist, document this); stealth replaces the documented 403 (breaking, update docs + e2e)
