...
export const tr: Record<string, string> = {
  ...
  'Panel hostname (optional)': 'Panel hostname’i (isteğe bağlı)',
  'Use {host}': '{host} kullan',
  'A name inside the allowed hostnames whose root is this organization’s dashboard, instead of /aperio. It serves the panel and nothing else, and only this organization’s people sign in there.': 'İzinli hostname’ler içinden, kökü /aperio yerine bu organizasyonun paneli olan bir ad. Yalnızca paneli sunar ve orada yalnızca bu organizasyonun kişileri giriş yapar.',
  'Default role for a new email': 'Yeni bir e-posta için varsayılan rol',
  'None (grant by hand)': 'Yok (elle yetki verilir)',
  'Group grants (group=role, comma-separated)': 'Grup yetkileri (grup=rol, virgülle ayrılmış)',
  'Signs in through the identity provider only (no password)': 'Yalnızca kimlik sağlayıcı üzerinden giriş yapar (parola yok)',
  'Group': 'Grup',
  'Grants': 'Yetkiler',
  'Add grant': 'Yetki ekle',
  'Remove grant': 'Yetkiyi kaldır',
  'Every organization (*)': 'Tüm organizasyonlar (*)',
  'all organizations': 'tüm organizasyonlar',
  '{count} user(s) reach every organization (*). Narrow the ones that do not need it.': '{count} kullanıcı tüm organizasyonlara (*) erişiyor. Gerek duymayanları daraltın.',
  'A user granted several organizations is created here in master and managed only from master.': 'Birden fazla organizasyona yetkili bir kullanıcı burada, master altında oluşturulur ve yalnızca master tarafından yönetilir.',
  'Users': 'Kullanıcılar',
  ...
  'waiting for a client': 'istemci bekleniyor',
  'admission (concurrency)': 'kabul (eşzamanlılık)',
  'routing': 'yönlendirme',
  'dispatch prep': 'gönderim hazırlığı',
  ...
  'Organization id (empty = master, * = every organization)': 'Organizasyon kimliği (boş = ana, * = tüm organizasyonlar)',
  ...
  '1 d': '1 g',
  '2 h': '2 sa',
  'Requests / second, in 5-second slices (last 15 minutes)': 'Saniyedeki istek, 5 saniyelik dilimlerle (son 15 dakika)',
  'Requests / second, in 2-minute slices (last 2 hours)': 'Saniyedeki istek, 2 dakikalık dilimlerle (son 2 saat)',
  'Requests / second, in 15-minute slices (last 24 hours)': 'Saniyedeki istek, 15 dakikalık dilimlerle (son 24 saat)',
  'now': 'şimdi',
  ...
  'just now': 'az önce',
  'never': 'asla',
  'expired {date}': '{date} tarihinde doldu',
  ...
  '{days}d {hours}h': '{days} g {hours} sa',
  'Any event': 'Tüm olaylar',
  'Sign-in': 'Oturum açma',
  'Tokens and keys': 'Token ve anahtarlar',
  'Clients and tunnels': 'İstemciler ve tüneller',
  'Users and organizations': 'Kullanıcılar ve organizasyonlar',
  ...
  "visitors must sign in (or carry a share link) before this reaches a client: this route asks for an Aperio sign-in, a dashboard session reaching this hostname's organization":
    'Ziyaretçiler bu istek bir istemciye ulaşmadan önce giriş yapmalı (ya da bir paylaşım bağlantısı taşımalı): bu rota bir Aperio girişi istiyor, bu hostname’in organizasyonuna erişen bir panel oturumu',
  ...
  'no source-IP deny list is configured': 'kaynak IP engelleme listesi yapılandırılmamış',
  'a source-IP deny list is in force ({count} entries); a request from a listed address is refused before anything below runs, whatever this report says': 'bir kaynak IP engelleme listesi yürürlükte ({count} kayıt); listedeki bir adresten gelen istek, bu rapor ne derse desin, aşağıdaki hiçbir şey çalışmadan önce reddedilir',
  'every visitor IP gets a bucket of {max} with {refill}/s refill; this dry run does not spend from it': "her ziyaretçi IP'si {max} kapasiteli ve saniyede {refill} dolan bir kovaya sahiptir; bu kuru çalışma ondan harcamaz",
  '{in_flight} of {max} server-wide request slots are in use right now; a request arriving with none free is refused rather than queued': 'sunucu genelindeki {max} istek yuvasından {in_flight} tanesi şu anda kullanımda; boş yuva yokken gelen istek kuyruğa alınmaz, reddedilir',
  'the serving service declares allowed_ips, so a visitor outside that list is routed nowhere and gets the 504 below rather than a refusal naming the list': "servisi veren hizmet allowed_ips tanımlıyor, yani bu listenin dışındaki bir ziyaretçi hiçbir yere yönlendirilmez ve listeyi adıyla anan bir ret yerine aşağıdaki 504'ü alır",
  "a request body over {effective} bytes is refused with 413; the service may tighten the server's limit but never widen it": '{effective} baytı aşan istek gövdesi 413 ile reddedilir; hizmet sunucunun limitini daraltabilir ama asla genişletemez',
  'the serving client admits {max} at a time; past that a request waits for a slot and is refused if none frees before the gateway timeout': 'servisi veren istemci aynı anda {max} isteği kabul eder; bunun ötesinde istek bir yuva bekler ve ağ geçidi zaman aşımından önce yuva boşalmazsa reddedilir',
  'the serving client declares no concurrency limit of its own': 'servisi veren istemci kendine ait bir eşzamanlılık limiti tanımlamıyor',
  'this route is cacheable, so a fresh entry would answer here without the request reaching a client at all': 'bu rota önbelleklenebilir, yani taze bir kayıt isteği hiçbir istemciye ulaştırmadan burada yanıtlar',
  'response caching is off server-wide': 'yanıt önbellekleme sunucu genelinde kapalı',
  'the serving service does not opt into caching, so every request goes to the backend': 'servisi veren hizmet önbelleklemeyi seçmiyor, yani her istek arka uca gider',
  'a dynamic token carries its own requests-per-second limit and daily byte quota; which token a request arrives with is not a property of this route, so neither is checked here': 'dinamik bir token kendi saniyedeki istek limitini ve günlük bayt kotasını taşır; bir isteğin hangi token ile geldiği bu rotanın özelliği değildir, o yüzden ikisi de burada kontrol edilmez',
  "the same token's daily byte quota is likewise a property of the credential": "aynı token'ın günlük bayt kotası da benzer biçimde kimlik bilgisinin özelliğidir",
  'this organization is within its monthly byte quota': 'bu organizasyon aylık bayt kotasının içinde',
  '429: this organization is over its monthly byte quota, so every request it serves is refused until the month turns': '429: bu organizasyon aylık bayt kotasını aşmış, yani ay dönene kadar servis ettiği her istek reddedilir',
  'the master organization has no monthly byte quota': 'master organizasyonun aylık bayt kotası yoktur',
  'no ceiling on concurrently open streamed responses per visitor': 'ziyaretçi başına aynı anda açık akış yanıtı sayısında tavan yok',
  'one visitor IP may hold {max} streamed responses open at once; a streamed answer past that is refused': "bir ziyaretçi IP'si aynı anda {max} akış yanıtını açık tutabilir; bunu aşan akış yanıtı reddedilir",
  'Deny list': 'Engelleme listesi',
  'Visitor rate limit': 'Ziyaretçi hız limiti',
  'Server concurrency': 'Sunucu eşzamanlılığı',
  'Body limit': 'Gövde limiti',
  'Client concurrency': 'İstemci eşzamanlılığı',
  'Cache': 'Önbellek',
  ...
  '1. Mint a token': '1. Bir token oluştur',
  'A token is what a client connects with. Scope it to the hostname the service will answer on.': 'Token, istemcinin bağlanırken kullandığı şeydir. Servisin yanıt vereceği hostname ile sınırla.',
  'A token is what a client connects with. An operator mints one on the API Tokens page.': 'Token, istemcinin bağlanırken kullandığı şeydir. Bir operatör API Tokens sayfasından oluşturur.',
  '2. Run the client next to your service': '2. İstemciyi servisin yanında çalıştır',
  'On the machine that can reach the service, with the token from step 1. The Clients page has a wizard that writes this for you.': 'Servise erişebilen makinede, 1. adımdaki token ile. Clients sayfasındaki sihirbaz bunu senin için yazar.',
  '3. Watch it appear here': '3. Burada belirmesini izle',
  'The client dials out to this server and the tunnel is up; this card leaves the moment it connects.': 'İstemci bu sunucuya dışarı doğru bağlanır ve tünel açılır; bu kart bağlandığı anda kaybolur.',
  'No client is connected yet': 'Henüz bağlı istemci yok',
  'Three steps put a service on this server. Nothing on your side accepts inbound connections; the client dials out.': 'Üç adım bir servisi bu sunucuya taşır. Senin tarafında gelen bağlantı kabul eden hiçbir şey yok; istemci dışarı doğru bağlanır.',
  'Getting started, the full walkthrough': 'Başlangıç, tam anlatım',
  ...
  'Redirect hostname': 'Hostname yönlendir',
  'Redirect client {id}…': 'İstemci {id}… için yönlendirme',
  'A bind is the hostname or path prefix a client claims; a request matching it is routed to that client.': 'Bind, bir istemcinin sahiplendiği hostname veya yol öneki; ona uyan istek o istemciye yönlendirilir.',
  'A draining client has announced it is shutting down: it gets no new requests and finishes the ones in flight.': 'Draining durumundaki istemci kapanacağını bildirmiştir: yeni istek almaz, eldekileri bitirir.',
  'An ejected service failed too often under real traffic and is out of rotation for a while, even if its health probe is green.': 'Ejected servis gerçek trafik altında çok sık hata verdi ve sağlık probu yeşil olsa da bir süre rotasyon dışıdır.',
  'A canary token is a decoy: it serves nothing, and any attempt to use it raises an alert, so a leaked copy surfaces itself.': 'Canary token bir yem: hiçbir şey sunmaz, kullanılma girişimi alarm üretir; sızan bir kopya böylece kendini ele verir.',
  'An expose entry opens a raw public port on the server that relays into a tunnel a client declared, with no second client needed.': 'Expose girdisi sunucuda ham bir genel port açar ve bunu bir istemcinin bildirdiği tünele iletir; ikinci bir istemci gerekmez.',
  'A hostname override redirects a live client to other hostnames from the dashboard; it lives in memory and a reconnect or restart reverts it.': 'Hostname geçersiz kılma, canlı bir istemciyi panelden başka hostname\'lere yönlendirir; bellekte yaşar, yeniden bağlanma veya yeniden başlatma geri alır.',
  'updated just now': 'az önce güncellendi',
  'updated {seconds}s ago': '{seconds} sn önce güncellendi',
  'waiting for data…': 'veri bekleniyor…',
  'Pushed over the live stream; this is when it last changed.': 'Canlı akış üzerinden gelir; bu, son değiştiği andır.',
  'Clear the whole inbox?': 'Tüm gelen kutusu temizlensin mi?',
  'Every stored inbound webhook of this organization is deleted; a re-fire is no longer possible for them.': 'Bu organizasyonun saklanan tüm gelen webhookları silinir; artık yeniden gönderilemezler.',
  'Purge the whole response cache?': 'Tüm yanıt önbelleği boşaltılsın mı?',
  'Every cached response is dropped and the next request for each reaches its backend again.': 'Önbellekteki her yanıt atılır ve her birinin bir sonraki isteği yeniden arka uca ulaşır.',
  'Search tokens…': 'Token ara…',
  'Search users…': 'Kullanıcı ara…',
  'Search webhooks…': 'Webhook ara…',
