// Russian. Keys are the English source strings; missing keys fall back to English.
export const ru: Record<string, string> = {
  '(other)': '(прочее)',
  '* (all hostnames)': '* (все hostname)',
  '/docs (optional)': '/docs (необязательно)',
  '1 day': '1 день',
  '1 hour': '1 час',
  '1 month': '1 месяц',
  '1 week': '1 неделя',
  '1 year': '1 год',
  '10 minutes': '10 минут',
  '12 hours': '12 часов',
  '2 days': '2 дня',
  '2 weeks': '2 недели',
  '2 years': '2 года',
  '3 days': '3 дня',
  '3 hours': '3 часа',
  '3 months': '3 месяца',
  '30 minutes': '30 минут',
  '5 days': '5 дней',
  '5 years': '5 лет',
  '6 hours': '6 часов',
  '6 months': '6 месяцев',
  'API Tokens': 'API-token',
  Access: 'Доступ',
  Actions: 'Действия',
  'Active Tunnel Connections': 'Активные туннельные подключения',
  'Active tunnel connections': 'Активные туннельные подключения',
  'Actor IP': 'IP инициатора',
  Add: 'Добавить',
  'Add Webhook': 'Добавить webhook',
  'Add a tunnel client': 'Добавить туннельный клиент',
  'Add webhook': 'Добавить webhook',
  'Adjusts the token scope in place; the secret never changes.':
    'Изменяет область действия token на месте; секрет никогда не меняется.',
  'All-time (persisted)': 'За всё время (сохраняется)',
  'Allowed IPs': 'Разрешённые IP',
  'Allowed hostnames (comma separated, * = all)': 'Разрешённые hostname (через запятую, * = все)',
  'Allowed path binds (comma separated, * = all)': 'Разрешённые привязки path (через запятую, * = все)',
  'Allowed source IPs / CIDRs': 'Разрешённые исходные IP / CIDR',
  'Announced link capacity; the server paces frames to this client accordingly':
    'Заявленная пропускная способность; сервер дозирует кадры этому клиенту соответственно',
  'Another live connection reports the same client id ({id}), bind-tunnels and failover lookups by this id are ambiguous; give each client its own --client-id':
    'Другое активное подключение сообщает тот же id клиента ({id}), поиск bind-tunnels и failover по этому id неоднозначен; дайте каждому клиенту свой --client-id',
  'Aperio Reverse Tunneling System • Server Uptime: {uptime}':
    'Система обратных туннелей Aperio • Аптайм сервера: {uptime}',
  Appearance: 'Оформление',
  Apply: 'Применить',
  'Audit Log': 'Журнал аудита',
  'Audit generations kept': 'Хранимые поколения аудита',
  'Audit rotation size': 'Размер ротации аудита',
  'Avg Response': 'Средний ответ',
  'BACKEND DOWN': 'БЭКЕНД НЕДОСТУПЕН',
  'Behavior of the tunnel links and automatic hostnames.':
    'Поведение туннельных соединений и автоматических hostname.',
  'Body truncated at capture; cannot replay': 'Тело усечено при захвате; повтор невозможен',
  Breakdown: 'Разбивка',
  'Burst size': 'Размер всплеска',
  'By hostname': 'По hostname',
  'By token': 'По token',
  'Cache budget': 'Бюджет кэша',
  'Cache-Control-driven GET cache; disabling clears stored entries':
    'GET-кэш на основе Cache-Control; при отключении записи очищаются',
  Caching: 'Кэширование',
  Cancel: 'Отмена',
  'Cannot reach the server. Retrying automatically…': 'Сервер недоступен. Автоматическая повторная попытка…',
  'Capacity & Health': 'Ёмкость и состояние',
  'Captured transaction detail, bodies are capped at 64 KB.':
    'Детали захваченной транзакции, тела ограничены 64 КБ.',
  'Change language': 'Сменить язык',
  Clear: 'Сбросить',
  'Click to inspect & replay': 'Нажмите для просмотра и повтора',
  'Client ID': 'ID клиента',
  'Client down threshold (s)': 'Порог недоступности клиента (с)',
  'Client is gracefully shutting down': 'Клиент корректно завершает работу',
  'Client speaks tunnel protocol v{proto}, server differs, update the older side':
    'Клиент использует протокол туннеля v{proto}, сервер отличается, обновите более старую сторону',
  'Client {id} disabled': 'Клиент {id} отключён',
  'Client {id} enabled': 'Клиент {id} включён',
  Clients: 'Клиенты',
  'Clipboard unavailable': 'Буфер обмена недоступен',
  Close: 'Закрыть',
  'Cloudflare shorthand: trust the CF-Connecting-IP header':
    'Сокращение для Cloudflare: доверять header CF-Connecting-IP',
  'Connect a new client': 'Подключить нового клиента',
  'Connected & Active': 'Подключено и активно',
  'Connected For': 'В сети',
  'Connection attempts beyond this are refused': 'Попытки подключения сверх лимита отклоняются',
  'Consecutive failures per IP before a lockout starts':
    'Подряд неудачных попыток с одного IP до блокировки',
  Copied: 'Скопировано',
  'Copied cURL to clipboard': 'cURL скопирован в буфер обмена',
  Copy: 'Копировать',
  'Copy an equivalent curl command': 'Скопировать эквивалентную команду curl',
  'Copy as cURL': 'Скопировать как cURL',
  'Copy it now, it will NOT be shown again.': 'Скопируйте сейчас, он больше НЕ будет показан.',
  'Could not revoke token "{name}"': 'Не удалось отозвать token «{name}»',
  'Could not update client {id}': 'Не удалось обновить клиента {id}',
  Create: 'Создать',
  'Create API token': 'Создать API-token',
  'Create Token': 'Создать token',
  'Create link': 'Создать ссылку',
  'Creates a dynamic tunnel token with a restricted scope.':
    'Создаёт динамический token туннеля с ограниченной областью действия.',
  'Current value': 'Текущее значение',
  'Custom 503 maintenance page (HTML)': 'Своя страница 503 для обслуживания (HTML)',
  'Custom 504 page (HTML)': 'Своя страница 504 (HTML)',
  DOWN: 'НЕДОСТУПЕН',
  'Daily traffic quota (MB, 0 = no quota, empty = keep)': 'Дневная квота трафика (МБ, 0 = без квоты, пусто = сохранить)',
  'Daily traffic quota (MB, empty = no quota)': 'Дневная квота трафика (МБ, пусто = без квоты)',
  "Dashboard data isn't updating, the values shown may be stale.":
    'Данные панели не обновляются, показанные значения могут быть устаревшими.',
  'Dashboard/login language for visitors whose browser language is unsupported':
    'Язык панели/входа для посетителей с неподдерживаемым языком браузера',
  'Data Transferred': 'Передано данных',
  'Default UI language': 'Язык интерфейса по умолчанию',
  Delete: 'Удалить',
  'Delete webhook "{name}"?': 'Удалить webhook «{name}»?',
  'Deliveries carry X-Aperio-Signature (HMAC-SHA256 over "timestamp.body") and X-Aperio-Timestamp so the receiver can verify origin and freshness.':
    'Доставки содержат X-Aperio-Signature (HMAC-SHA256 от «timestamp.body») и X-Aperio-Timestamp, чтобы получатель мог проверить источник и свежесть.',
  'Describe the local service; copy a ready-to-run command below.':
    'Опишите локальный сервис; скопируйте готовую команду ниже.',
  Details: 'Детали',
  Disable: 'Отключить',
  'Disable all': 'Отключить все',
  'Disable client {id}…?': 'Отключить клиента {id}…?',
  'Disable {count} client(s)?': 'Отключить {count} клиент(ов)?',
  'Draining…': 'Завершение…',
  'Each stays connected but receives no new requests; in-flight requests complete. Affects the clients currently listed (matching your search).':
    'Каждый остаётся подключённым, но не получает новых запросов; текущие запросы завершаются. Затрагивает клиентов в текущем списке (по вашему поиску).',
  Edit: 'Изменить',
  'Edit token "{name}"': 'Изменить token «{name}»',
  Enable: 'Включить',
  'Enable all': 'Включить все',
  'Enable maintenance': 'Включить обслуживание',
  'Enabling is offered to connected clients immediately; disabling applies to new connections':
    'Включение сразу предлагается подключённым клиентам; отключение действует для новых подключений',
  'End maintenance': 'Завершить обслуживание',
  'End maintenance for {host}': 'Завершить обслуживание {host}',
  'Env vars provide the defaults; edits become live, persisted overrides. Master token, HOST/PORT, proxy trust and OIDC stay env-only.':
    'Переменные окружения задают значения по умолчанию; правки становятся живыми, сохраняемыми переопределениями. Мастер-token, HOST/PORT, доверие к proxy и OIDC остаются только в окружении.',
  'Environment Flags': 'Переменные окружения',
  Event: 'Событие',
  'Events (comma separated, * = all)': 'События (через запятую, * = все)',
  Expires: 'Истекает',
  'Failed to load request detail: {error}': 'Не удалось загрузить детали запроса: {error}',
  'Failover max jumps': 'Макс. переключений failover',
  'Failover non-idempotent methods': 'Failover неидемпотентных методов',
  'Failover window (s)': 'Окно failover (с)',
  'Filter by path/method...': 'Фильтр по path/методу...',
  'First lockout duration; doubles per repeat offense': 'Длительность первой блокировки; удваивается при повторе',
  'Freeze the table while you inspect': 'Заморозить таблицу на время просмотра',
  'Gateway & Requests': 'Шлюз и запросы',
  'Gateway timeout (s)': 'Тайм-аут шлюза (с)',
  'Give someone temporary access to an auth-protected site: the link carries a signed, expiring token scoped to the hostname (and optional path). Opening it sets a cookie and redirects to the clean URL. Links are stateless, they cannot be listed later, they simply expire.':
    'Дайте кому-то временный доступ к защищённому сайту: ссылка несёт подписанный истекающий token, ограниченный hostname (и необязательным path). При открытии ставится cookie и происходит переход на чистый URL. Ссылки не имеют состояния, их нельзя перечислить позже, они просто истекают.',
  'Go to {page}': 'Перейти: {page}',
  'Header consulted first for the visitor IP (behind CDN chains)':
    'Header, проверяемый первым для IP посетителя (за цепочками CDN)',
  Hostname: 'Hostname',
  'Hostname bind (optional)': 'Привязка hostname (необязательно)',
  Hostnames: 'Hostname',
  'How many clients may connect, how much runs at once, and when a client counts as down.':
    'Сколько клиентов могут подключиться, сколько работает одновременно и когда клиент считается недоступным.',
  'How requests pick a client and what happens when one is lost mid-request.':
    'Как запросы выбирают клиента и что происходит при его потере посреди запроса.',
  'I have a token': 'У меня есть token',
  'IP Address': 'IP-адрес',
  'Ignore client-declared visitor passwords; the server keeps the gate':
    'Игнорировать пароли посетителей, заявленные клиентом; контроль остаётся у сервера',
  'In-flight failover': 'Failover текущих запросов',
  'In-flight proxied requests; beyond it visitors get 429': 'Одновременные запросы; сверх лимита посетители получают 429',
  'Invalid credentials. Please try again.': 'Неверные учётные данные. Попробуйте ещё раз.',
  'It stays connected but receives no new requests; in-flight requests complete.':
    'Он остаётся подключённым, но не получает новых запросов; текущие запросы завершаются.',
  'JSONL access log file path (empty = disabled)': 'Path к журналу доступа JSONL (пусто = отключено)',
  'Known events: {events}. Use * to subscribe to everything.':
    'Известные события: {events}. Используйте * для подписки на все.',
  'Last Ping': 'Последний пинг',
  Latency: 'Задержка',
  'Latency (recent {count})': 'Задержка (последние {count})',
  'Lifetime in seconds (empty = never expires)': 'Время жизни в секундах (пусто = не истекает)',
  Limits: 'Ограничения',
  Live: 'Онлайн',
  'Live Request Activity': 'Активность запросов в реальном времени',
  'Live Traffic': 'Живой трафик',
  'Nothing yet. Events show up here as they happen.': 'Пока ничего. События появляются здесь по мере возникновения.',
  'Notifications': 'Уведомления',
  'Notifications, {count} unread': 'Уведомления, непрочитанных: {count}',
  'Route Trends': 'Тренды маршрутов',
  'One bar per minute over the last 30 minutes, colored by the worst status class, spot which route started erroring, and when.': 'Один столбик на минуту за последние 30 минут, цвет, по худшему классу статуса: видно, какой маршрут и когда начал ошибаться.',
  'Last 30 minutes': 'Последние 30 минут',
  'Error rate': 'Доля ошибок',
  'No traffic in the last 30 minutes.': 'Нет трафика за последние 30 минут.',
  'green = 2xx/3xx, amber = 4xx seen, red = 5xx seen': 'зелёный = 2xx/3xx, жёлтый = были 4xx, красный = были 5xx',
  'Bandwidth': 'Трафик',
  'Daily': 'По дням',
  'Monthly': 'По месяцам',
  'Bytes through the tunnel per token and hostname, hover a cell for the sent/received split.': 'Байты через туннель по token и hostname, наведите на ячейку для разбивки отправлено/получено.',
  'No traffic recorded for this window yet.': 'За это окно трафик ещё не записан.',
  'Total': 'Итого',
  'Slowest Endpoints': 'Самые медленные endpoint',
  'Recent-window latency per endpoint, worst p95 first, where the time is going right now.': 'Задержка по endpoint за недавнее окно, худший p95 первым, куда уходит время сейчас.',
  'Max': 'Макс',
  'Not enough recent traffic yet, the table needs a few requests per endpoint.': 'Пока недостаточно трафика, таблице нужно несколько запросов на endpoint.',
  'Webhook Inbox': 'Входящие webhook',
  'Clear inbox': 'Очистить входящие',
  'Services with webhook_inbox: true get every inbound POST persisted here, browse the payloads and re-fire any event to the connected client.': 'Для сервисов с webhook_inbox: true каждый входящий POST сохраняется здесь, просматривайте payload и повторно отправляйте события подключённому клиенту.',
  'Payload': 'Payload',
  'No captured webhooks yet, opt a service in with webhook_inbox: true.': 'Пока нет сохранённых webhook, включите сервис через webhook_inbox: true.',
  'Re-fire to the connected client': 'Повторно отправить подключённому клиенту',
  'Re-fire': 'Повторить',
  'Re-fired, backend answered {status}': 'Отправлено повторно, бэкенд ответил {status}',
  'Re-fire failed ({status})': 'Повторная отправка не удалась ({status})',
  'Headers': 'Headers',
  '(no body)': '(нет тела)',
  'API Explorer': 'Обозреватель API',
  'Filter endpoints...': 'Фильтр endpoint...',
  'Query string': 'Строка запроса',
  'JSON request body (optional)': 'JSON-тело запроса (необязательно)',
  'Sending...': 'Отправка...',
  'Send request': 'Отправить запрос',
  'Runs against this server with your current session.': 'Выполняется на этом сервере с вашей текущей сессией.',
  '(empty response body)': '(пустое тело ответа)',
  'Could not load the OpenAPI document.': 'Не удалось загрузить документ OpenAPI.',
  'Loading the API document...': 'Загрузка документа API...',
  'Waiting for the stream...': 'Ожидание потока...',
  'Jump to latest': 'К последним',
  'Showing the newest {max} lines; click a line to open the inspector.': 'Показаны последние {max} строк; щёлкните строку, чтобы открыть инспектор.',
  'Live Tunnel Traffic': 'Живой трафик туннеля',
  'Load balancing': 'Балансировка нагрузки',
  'Local target (where your service listens)': 'Локальная цель (где слушает ваш сервис)',
  'Login brute-force protection and audit log rotation.':
    'Защита входа от перебора и ротация журнала аудита.',
  'Login lockout base (s)': 'Базовая длительность блокировки (с)',
  'Login lockout threshold': 'Порог блокировки входа',
  Maintenance: 'Обслуживание',
  'Maintenance Mode': 'Режим обслуживания',
  'Maintenance enabled for {host}': 'Обслуживание включено для {host}',
  'Maintenance ended for {host}': 'Обслуживание завершено для {host}',
  'Max concurrent requests': 'Макс. одновременных запросов',
  'Max request body': 'Макс. тело запроса',
  'Max tunnel clients': 'Макс. туннельных клиентов',
  'May ask the server to reach a target itself (server_side)':
    'Может просить сервер обращаться к цели напрямую (server_side)',
  'May publish public services (visitor auth gate skipped)':
    'Может публиковать публичные сервисы (без проверки посетителей)',
  Method: 'Метод',
  'Mint a scoped token now': 'Выпустить ограниченный token сейчас',
  'Mint token': 'Выпустить token',
  'Missed-heartbeat window before a client leaves routing':
    'Окно пропущенных heartbeat до исключения клиента из маршрутизации',
  Name: 'Имя',
  Navigate: 'Навигация',
  'New connections with this token will be rejected.': 'Новые подключения с этим token будут отклонены.',
  'New lifetime in seconds from now (0 = never, empty = keep)':
    'Новое время жизни в секундах с этого момента (0 = никогда, пусто = сохранить)',
  'No active client sessions, start a tunnel client to see it here':
    'Нет активных клиентских сессий, запустите туннельный клиент, чтобы увидеть его здесь',
  'No active web socket client': 'Нет активного websocket-клиента',
  'No attributed traffic yet': 'Атрибутированного трафика пока нет',
  'No audit events': 'Нет событий аудита',
  'No clients match "{search}"': 'Нет клиентов по запросу «{search}»',
  'No clients to disable': 'Нет клиентов для отключения',
  'No clients to enable': 'Нет клиентов для включения',
  'No dynamic tokens created': 'Динамические token не созданы',
  'No further events will be delivered to {url}.': 'События больше не будут доставляться на {url}.',
  'No matching commands': 'Нет подходящих команд',
  'No requests matching filter': 'Нет запросов по фильтру',
  'No webhooks defined': 'Webhook не заданы',
  'Not a size, use e.g. 10 MB, 1.5 GB, or plain bytes':
    'Не размер, используйте, например, 10 MB, 1.5 GB или байты',
  'OIDC single sign-on (issuer, client id/secret, redirect URL, scopes)':
    'Единый вход OIDC (issuer, id/секрет клиента, redirect URL, scopes)',
  'OK / Failed': 'OK / Ошибки',
  Offline: 'Офлайн',
  'Offline (Waiting for Client)': 'Офлайн (ожидание клиента)',
  Online: 'Онлайн',
  'Override applied for {id}': 'Переопределение применено для {id}',
  'Override cleared for {id}': 'Переопределение снято для {id}',
  Overview: 'Обзор',
  'POST/PATCH may reach a backend twice when enabled': 'POST/PATCH может дойти до бэкенда дважды при включении',
  Password: 'Пароль',
  Path: 'Path',
  'Path bind': 'Привязка path',
  'Path bind (optional)': 'Привязка path (необязательно)',
  Paths: 'Path',
  Paused: 'Пауза',
  'Paused, table frozen at {count} requests.': 'Пауза, таблица заморожена на {count} запросах.',
  'Payload bytes transferred': 'Передано байт payload',
  'Per-hostname 503 switch': 'Переключатель 503 для каждого hostname',
  'Per-visitor-IP token bucket for proxied requests.':
    'Token bucket на IP посетителя для проксируемых запросов.',
  Prefix: 'Префикс',
  'Prometheus metrics endpoint at /aperio/metrics': 'Endpoint метрик Prometheus на /aperio/metrics',
  Purpose: 'Назначение',
  'Queue Status': 'Состояние очереди',
  'Random subdomain pattern': 'Шаблон случайного subdomain',
  'Rate Limiting': 'Ограничение частоты',
  'Rate limit (req/s, 0 = no limit, empty = keep)': 'Лимит частоты (запр./с, 0 = без лимита, пусто = сохранить)',
  'Rate limit (req/s, empty = no limit)': 'Лимит частоты (запр./с, пусто = без лимита)',
  'Re-dispatch attempts per request': 'Повторных отправок на запрос',
  'Reaction when the serving client drops mid-request': 'Реакция при потере обслуживающего клиента посреди запроса',
  Received: 'Получено',
  'Refill rate (req/s)': 'Скорость пополнения (запр./с)',
  Refresh: 'Обновить',
  Replay: 'Повторить',
  'Replay failed: {error}': 'Повтор не удался: {error}',
  'Request Body': 'Тело запроса',
  'Request Detail': 'Детали запроса',
  'Request Headers': 'Headers запроса',
  Requests: 'Запросы',
  'Requests / second (last 60 seconds)': 'Запросов / секунду (последние 60 секунд)',
  'Requests a single IP may fire at once': 'Запросов, которые один IP может отправить разом',
  'Requests pending reconnection': 'Запросы в ожидании переподключения',
  'Requests today': 'Запросов сегодня',
  'Requests with larger bodies are rejected up front': 'Запросы с большим телом отклоняются сразу',
  'Require hostname bind': 'Требовать привязку hostname',
  'Reset to env default ({value})': 'Сбросить к значению окружения ({value})',
  'Reset {label} to default': 'Сбросить {label} к значению по умолчанию',
  'Response Body': 'Тело ответа',
  'Response Headers': 'Headers ответа',
  'Response cache': 'Кэш ответов',
  'Response timeout (s)': 'Тайм-аут ответа (с)',
  'Resume live updates': 'Возобновить живые обновления',
  Revoke: 'Отозвать',
  'Revoke token "{name}" ({prefix}…)?': 'Отозвать token «{name}» ({prefix}…)?',
  'Rotated audit.jsonl.N files to keep; oldest is dropped':
    'Хранимые файлы audit.jsonl.N после ротации; старейший удаляется',
  'Routing & Failover': 'Маршрутизация и failover',
  'SHARED ID': 'ОБЩИЙ ID',
  Save: 'Сохранить',
  'Save & apply': 'Сохранить и применить',
  'Scoped tunnel credentials': 'Ограниченные учётные данные туннеля',
  'Search clients…': 'Поиск клиентов…',
  'Search…': 'Поиск…',
  'Security & Audit': 'Безопасность и аудит',
  'Security- and startup-critical flags stay environment-only so a compromised dashboard session cannot change them. The server is running inside a container, to change one:':
    'Критичные для безопасности и запуска переменные остаются только в окружении, чтобы скомпрометированная сессия панели не могла их изменить. Сервер работает в контейнере, чтобы изменить:',
  'Security- and startup-critical flags stay environment-only so a compromised dashboard session cannot change them. The server is running natively, to change one:':
    'Критичные для безопасности и запуска переменные остаются только в окружении, чтобы скомпрометированная сессия панели не могла их изменить. Сервер работает нативно, чтобы изменить:',
  'Send this request through the tunnel again': 'Отправить этот запрос через туннель снова',
  Sent: 'Отправлено',
  'Server Settings': 'Настройки сервера',
  'Server-side response cache for services that opt in with cache: true.':
    'Серверный кэш ответов для сервисов, включивших cache: true.',
  'Service name announced by the client (services: list)':
    'Имя сервиса, заявленное клиентом (список services:)',
  Session: 'Сессия',
  'Session cookies carry the Secure flag (HTTPS only)':
    'Cookies сессии несут флаг Secure (только HTTPS)',
  'Session expires in {duration}': 'Сессия истекает через {duration}',
  'Settings applied and persisted.': 'Настройки применены и сохранены.',
  'Share Links': 'Ссылки доступа',
  'Showing the latest {max} of {total} matching requests.':
    'Показаны последние {max} из {total} подходящих запросов.',
  'Shown for hostnames in maintenance mode': 'Показывается для hostname в режиме обслуживания',
  'Shown when no client answers in time': 'Показывается, когда ни один клиент не отвечает вовремя',
  'Sign In': 'Войти',
  'Sign in to continue': 'Войдите, чтобы продолжить',
  'Sign out': 'Выйти',
  'Signing secret (optional, 16-128 chars)': 'Секрет подписи (необязательно, 16-128 символов)',
  'Payload format': 'Формат payload',
  'Traffic History': 'История трафика',
  'Sign in with a passkey':
    'Войти с помощью passkey',
  'Passkey sign-in failed. Enter your username above and try again.':
    'Не удалось войти по passkey. Введите имя пользователя выше и попробуйте снова.',
  'Passkeys':
    'Passkey-ключи',
  'Sign in without a password using YubiKeys, Touch ID / Face ID, or your password manager.':
    'Входите без пароля с помощью YubiKey, Touch ID / Face ID или менеджера паролей.',
  'The built-in "aperio" admin signs in with the master token or dashboard password and cannot register passkeys. Create a named user instead.':
    'Встроенный администратор «aperio» входит по мастер-token или паролю панели и не может регистрировать passkey. Создайте именованного пользователя.',
  'Passkey sign-in is not configured on this server. Set APERIO_WEBAUTHN_ORIGIN to the dashboard’s public URL to enable it.':
    'Вход по passkey не настроен на этом сервере. Укажите APERIO_WEBAUTHN_ORIGIN, публичный URL панели.',
  'This browser does not support WebAuthn.':
    'Этот браузер не поддерживает WebAuthn.',
  'No passkeys registered yet.':
    'Passkey ещё не зарегистрированы.',
  'Name for the new passkey (optional)':
    'Название нового passkey (необязательно)',
  'e.g. YubiKey 5, MacBook Touch ID':
    'напр. YubiKey 5, MacBook Touch ID',
  'Register passkey':
    'Зарегистрировать passkey',
  'Passkey registered':
    'Passkey зарегистрирован',
  'Passkey "{name}" deleted':
    'Passkey «{name}» удалён',
  'Enter the code from your authenticator app':
    'Введите код из приложения-аутентификатора',
  'Authentication code':
    'Код аутентификации',
  'A recovery code also works here.':
    'Код восстановления здесь тоже подойдёт.',
  'Verify':
    'Подтвердить',
  'Back':
    'Назад',
  'Invalid code. Please try again.':
    'Неверный код. Попробуйте ещё раз.',
  'Two-factor auth':
    'Двухфакторная аутентификация',
  'Two-factor authentication':
    'Двухфакторная аутентификация',
  'Protect your dashboard sign-in with one-time codes from an authenticator app (Google Authenticator, Authy, 1Password, …).':
    'Защитите вход в панель одноразовыми кодами из приложения-аутентификатора (Google Authenticator, Authy, 1Password, …).',
  'The built-in "aperio" admin signs in with the master token or dashboard password and cannot enroll two-factor authentication. Create a named user instead.':
    'Встроенный администратор «aperio» входит по мастер-token или паролю панели и не может включить двухфакторную аутентификацию. Создайте именованного пользователя.',
  'Two-factor authentication is currently off for your account.':
    'Двухфакторная аутентификация для вашей учётной записи выключена.',
  'Two-factor authentication is on. Enter a current code (or a recovery code) to turn it off.':
    'Двухфакторная аутентификация включена. Введите текущий код (или код восстановления), чтобы выключить её.',
  'Scan the QR code with your authenticator app, then enter the 6-digit code it shows to finish.':
    'Отсканируйте QR-код приложением-аутентификатором и введите показанный 6-значный код, чтобы завершить.',
  'Verify & enable':
    'Подтвердить и включить',
  'Two-factor authentication is now on. Store these single-use recovery codes somewhere safe, they are shown only once and let you sign in if you lose your authenticator.':
    'Двухфакторная аутентификация включена. Сохраните эти одноразовые коды восстановления в надёжном месте, они показываются только один раз и позволят войти, если вы потеряете аутентификатор.',
  'Done':
    'Готово',
  'Two-factor authentication disabled':
    'Двухфакторная аутентификация выключена',
  'Recovery codes copied':
    'Коды восстановления скопированы',
  'Reset 2FA':
    'Сбросить 2FA',
  'Two-factor auth reset for "{name}"':
    'Двухфакторная аутентификация сброшена для «{name}»',
  'Reset two-factor auth for "{name}"?':
    'Сбросить двухфакторную аутентификацию для «{name}»?',
  'The user will sign in with their password only until they enroll again. Use this when someone lost their authenticator and recovery codes.':
    'Пользователь будет входить только по паролю, пока не зарегистрируется снова. Используйте, когда кто-то потерял аутентификатор и коды восстановления.',
  'Reset':
    'Сбросить',
  'Uptime': 'Аптайм',
  'Service': 'Сервис',
  '7 days': '7 дней',
  '30 days': '30 дней',
  'up': 'работает',
  'degraded': 'деградация',
  'down': 'недоступен',
  'no data': 'нет данных',
  'No availability history yet': 'Истории доступности пока нет',
  'Percentages cover observed time only, time while the server itself was offline is not counted against a service.': 'Проценты учитывают только наблюдаемое время, время, когда сам сервер был выключен, сервису не засчитывается.',
  'Last 7 days': 'Последние 7 дней',
  'Last 30 days': 'Последние 30 дней',
  'Last 60 days': 'Последние 60 дней',
  'Last 26 weeks': 'Последние 26 недель',
  'Last 24 months': 'Последние 24 месяца',
  'Custom range': 'Произвольный диапазон',
  'From date': 'Дата начала',
  'To date': 'Дата окончания',
  '{count} new': 'новых: {count}',
  '{requests} requests, {bytes} transferred in this range': '{requests} запросов, передано {bytes} за этот период',
  'Generic JSON': 'Обычный JSON',
  'Generic sends the raw event JSON; the chat formats send a ready-made message for the incoming-webhook URL of that service.':
    'Обычный формат отправляет сырой JSON события; чат-форматы отправляют готовое сообщение на входящий webhook сервиса.',
  'Standby tier {tier}: receives traffic only when no lower tier is available (primary-standby strategy)':
    'Резервный уровень {tier}: получает трафик, только когда нет более низкого уровня (стратегия primary-standby)',
  'Stats & live activity': 'Статистика и живая активность',
  Status: 'Статус',
  'Status mix': 'Распределение статусов',
  'Strategy for picking a client from the routed pool': 'Стратегия выбора клиента из пула маршрутизации',
  'Strict multi-tenant mode: unbound clients never receive traffic':
    'Строгий мультиарендный режим: непривязанные клиенты никогда не получают трафик',
  Success: 'Успех',
  'Sustained requests per second per IP': 'Устойчивых запросов в секунду на IP',
  'Switch to dark theme': 'Переключиться на тёмную тему',
  'Switch to light theme': 'Переключиться на светлую тему',
  System: 'Система',
  'Temporary visitor access': 'Временный доступ посетителей',
  "The client's own health probe reports its backend as down; excluded from routing while the tunnel stays connected":
    'Проверка здоровья самого клиента сообщает, что его бэкенд недоступен; исключён из маршрутизации, пока туннель подключён',
  'The commands below use a {placeholder} placeholder, replace it with your master token or an existing dynamic token.':
    'Команды ниже используют заполнитель {placeholder}, замените его мастер-token или существующим динамическим token.',
  "This client gates its service behind a client-set visitor login, overriding the server's own visitor password for this service":
    'Этот клиент закрывает свой сервис собственным входом для посетителей, переопределяя пароль посетителей сервера для этого сервиса',
  'This client serves its traffic without the visitor auth gate':
    'Этот клиент обслуживает трафик без проверки посетителей',
  Time: 'Время',
  'Timeouts and size limits applied to every proxied request.':
    'Тайм-ауты и лимиты размера для каждого проксируемого запроса.',
  Timestamp: 'Метка времени',
  Today: 'Сегодня',
  'Toggle color theme': 'Переключить цветовую тему',
  Token: 'Token',
  'Token "{name}" created': 'Token «{name}» создан',
  'Token "{name}" revoked': 'Token «{name}» отозван',
  'Token "{name}" updated': 'Token «{name}» обновлён',
  'Token created': 'Token создан',
  'Token minted and embedded below. It is scoped to {scope} and will not be shown again after this dialog closes.':
    'Token выпущен и встроен ниже. Его область, {scope}; после закрытия окна он больше не будет показан.',
  'Token name is required': 'Требуется имя token',
  'Token required to scrape the metrics endpoint': 'Token для чтения endpoint метрик',
  'Total Requests': 'Всего запросов',
  'Total memory for cached responses; entries closest to expiry are evicted first':
    'Общая память кэша ответов; записи ближе к истечению вытесняются первыми',
  'Total time budget across all jumps': 'Общий бюджет времени на все переключения',
  Traffic: 'Трафик',
  'Traffic Breakdown': 'Разбивка трафика',
  'Traffic by token & hostname': 'Трафик по token и hostname',
  'Trust X-Forwarded-For / X-Real-IP from a fronting reverse proxy':
    'Доверять X-Forwarded-For / X-Real-IP от фронтального обратного proxy',
  'Trusted proxy/CDN egress IPs or CIDRs used to resolve the real visitor IP':
    'Доверенные выходные IP/CIDR proxy/CDN для определения реального IP посетителя',
  'Tunnel Clients': 'Туннельные клиенты',
  'Tunnel compression': 'Сжатие туннеля',
  'Tunnels & Domains': 'Туннели и домены',
  'Type a command…': 'Введите команду…',
  'Unsaved changes': 'Несохранённые изменения',
  Username: 'Имя пользователя',
  Variable: 'Переменная',
  'Visitor Experience': 'Опыт посетителя',
  'Visitor password': 'Пароль посетителя',
  'Wait for a client to (re)connect before failing a request':
    'Ждать (пере)подключения клиента, прежде чем провалить запрос',
  'Wait for a client to answer a dispatched request': 'Ждать ответа клиента на отправленный запрос',
  'Webhook "{name}" added': 'Webhook «{name}» добавлен',
  'Webhook "{name}" deleted': 'Webhook «{name}» удалён',
  Webhooks: 'Webhook',
  'What visitors see in front of and around the proxied services.':
    'Что посетители видят перед проксируемыми сервисами и вокруг них.',
  'all binds': 'все привязки',
  'any hostname': 'любой hostname',
  'any path': 'любой path',
  'audit.jsonl rotates past this size; 0 = never rotate':
    'audit.jsonl ротируется при превышении этого размера; 0 = никогда',
  'name': 'имя',
  'client id': 'id клиента',
  connection: 'подключение',
  'custom auth': 'своя аутентификация',
  'IP filter': 'IP-фильтр',
  'Only these visitor IPs/CIDRs may reach this service: {ips}':
    'Только эти IP/CIDR посетителей могут обращаться к этому сервису: {ips}',
  'e.g. 10 MB, 1 GB, 65536': 'напр. 10 MB, 1 GB, 65536',
  'e.g. example.com, *.example.com or *-test.example.com, * becomes a random label; empty = disabled':
    'напр. example.com, *.example.com или *-test.example.com, * становится случайной меткой; пусто = отключено',
  'master token': 'мастер-token',
  'never expires': 'не истекает',
  override: 'переопределено',
  public: 'публичный',
  'public ok': 'публичный разрешён',
  'expiring soon': 'скоро истекает',
  'read-only': 'только чтение',
  'shared secret for X-Aperio-Signature': 'общий секрет для X-Aperio-Signature',
  signed: 'подписан',
  standby: 'резерв',
  token: 'token',
  'token name (e.g. staging-box)': 'имя token (напр. staging-box)',
  'user:password gate in front of all proxied traffic; empty = disabled':
    'шлюз user:password перед всем проксируемым трафиком; пусто = отключено',
  'valid until {date}': 'действует до {date}',
  '{count} client(s) disabled': 'Отключено клиентов: {count}',
  '{count} client(s) enabled': 'Включено клиентов: {count}',
  '{count} lifetime requests • {bytes} sent': 'Всего запросов: {count} • отправлено {bytes}',
  '{count} tunnel client(s) active': 'Активных туннельных клиентов: {count}',
  '{ok} of {total} successful': '{ok} из {total} успешно',
  '{ok} ok / {failed} failed • {bytes} sent today': 'сегодня {ok} ок / {failed} с ошибкой • отправлено {bytes}',
  '✔ Replayed: status {status} in {ms} ms': '✔ Повторено: статус {status} за {ms} мс',
  '(you)': '(вы)',
  'Account enabled': 'Учётная запись активна',
  'Add User': 'Добавить пользователя',
  'Add dashboard user': 'Добавить пользователя панели',
  'Admin': 'Администратор',
  'Dashboard Users': 'Пользователи панели',
  'Delete user "{name}"?': 'Удалить пользователя «{name}»?',
  'Edit user "{name}"': 'Изменить пользователя «{name}»',
  'New password (leave blank to keep)': 'Новый пароль (оставьте пустым, чтобы сохранить)',
  'Operator': 'Оператор',
  'Password (min. 8 characters)': 'Пароль (не менее 8 символов)',
  'Role': 'Роль',
  'Their active dashboard sessions are ended immediately.': 'Их активные сессии панели немедленно завершаются.',
  'User "{name}" created': 'Пользователь «{name}» создан',
  'User "{name}" deleted': 'Пользователь «{name}» удалён',
  'User "{name}" updated': 'Пользователь «{name}» обновлён',
  'Panel hostname (optional)': 'Имя хоста панели (необязательно)',
  'Use {host}': 'Использовать {host}',
  'A name inside the allowed hostnames whose root is this organization’s dashboard, instead of /aperio. It serves the panel and nothing else, and only this organization’s people sign in there.': 'Имя из разрешённых хостов, корень которого является панелью этой организации вместо /aperio. Оно обслуживает только панель, и входят там только люди этой организации.',
  'Default role for a new email': 'Роль по умолчанию для нового адреса',
  'None (grant by hand)': 'Нет (выдать вручную)',
  'Group grants (group=role, comma-separated)': 'Права по группам (группа=роль, через запятую)',
  'Signs in through the identity provider only (no password)': 'Входит только через провайдер идентификации (без пароля)',
  'Group': 'Группа',
  'Grants': 'Права',
  'Add grant': 'Добавить право',
  'Remove grant': 'Убрать право',
  'Every organization (*)': 'Все организации (*)',
  'all organizations': 'все организации',
  '{count} user(s) reach every organization (*). Narrow the ones that do not need it.': '{count} пользователь(ей) имеют доступ ко всем организациям (*). Сузьте тех, кому это не нужно.',
  'A user granted several organizations is created here in master and managed only from master.': 'Пользователь с несколькими организациями создаётся здесь, в master, и управляется только из master.',
  'Users': 'Пользователи',
  'Users sign in at the dashboard login with their username and password.': 'Пользователи входят в панель по имени пользователя и паролю.',
  'Viewer': 'Наблюдатель',
  'active': 'активен',
  'disabled': 'отключён',
  'Export & Import': 'Экспорт и импорт',
  'Download export': 'Скачать экспорт',
  'Import dump…': 'Импортировать дамп…',
  'Import applied': 'Импорт применён',
  'HAR': 'HAR',
  'Download as an HAR file (devtools importable)': 'Скачать как HAR-файл (импортируется в devtools)',
  'Recent deliveries': 'Недавние доставки',
  'No deliveries yet': 'Доставок пока нет',
  'failed': 'ошибка',
  'Redeliver': 'Отправить повторно',
  'Redelivery of "{event}" queued': 'Повторная отправка «{event}» поставлена в очередь',
  'Active sessions': 'Активные сессии',
  'Everyone currently signed in, and where from. Ending a session signs that browser out on its next request.': 'Все, кто сейчас вошёл, и откуда. Завершение сессии разлогинит этот браузер при следующем запросе.',
  'Sign out everywhere else': 'Выйти на всех остальных устройствах',
  'No live sessions': 'Нет активных сессий',
  'this session': 'эта сессия',
  'End session': 'Завершить сессию',
  'Session ended': 'Сессия завершена',
  '{count} other session(s) ended': 'Завершено других сессий: {count}',
  'usernameless': 'без имени пользователя',
  'Allow signing in without a username': 'Разрешить вход без имени пользователя',
  'Pressing the passkey button with an empty username offers this passkey directly. Requires an authenticator that stores the credential (platform authenticators and modern security keys do).': 'Нажатие кнопки passkey с пустым именем пользователя сразу предложит этот ключ. Требуется аутентификатор, хранящий учётные данные (платформенные аутентификаторы и современные ключи безопасности умеют это).',
  'Topology': 'Топология',
  'Routes, clients & backends as a live map': 'Маршруты, клиенты и бэкенды на живой карте',
  '(any request)': '(любой запрос)',
  'Routes': 'Маршруты',
  'Tunnel clients': 'Туннельные клиенты',
  'Backends': 'Бэкенды',
  'backend healthy': 'бэкенд здоров',
  'backend failing': 'бэкенд сбоит',
  'No clients connected': 'Нет подключённых клиентов',
  'Timeline': 'Хронология',
  'offsets from arrival at the server': 'смещения от прибытия на сервер',
  'client stages anchored by splitting transit evenly': 'этапы клиента привязаны делением транзита поровну',
  'queued & routed': 'очередь и маршрутизация',
  'waiting for a client': 'ожидание клиента',
  'admission (concurrency)': 'допуск (параллельность)',
  'routing': 'маршрутизация',
  'dispatch prep': 'подготовка отправки',
  'tunnel → client': 'туннель → клиент',
  'client processing': 'обработка на клиенте',
  'backend wait (first byte)': 'ожидание бэкенда (первый байт)',
  'backend body': 'тело ответа бэкенда',
  'response head → tunnel (body still streaming)': 'заголовок ответа → туннель (тело ещё передаётся)',
  'client → tunnel': 'клиент → туннель',
  'tunnel → server': 'туннель → сервер',
  'tunnel round-trip (client & backend)': 'полный оборот туннеля (клиент и бэкенд)',
  'server → visitor': 'сервер → посетитель',
  'Stage latencies': 'Задержки по этапам',
  'Mean and spread of each request stage (rolling window per route, buffered requests of timing-aware clients). A stage far outside its usual band is flagged.': 'Среднее и разброс каждого этапа запроса (скользящее окно на маршрут; буферизованные запросы клиентов с поддержкой таймингов). Этап, сильно выходящий за обычный диапазон, помечается.',
  'Route': 'Маршрут',
  'Stage': 'Этап',
  'Mean': 'Среднее',
  'Last': 'Последний',
  'No timing data yet': 'Данных о таймингах пока нет',
  'anomaly': 'аномалия',
  'normal': 'норма',
  'Save & apply (Ctrl+S)': 'Сохранить и применить (Ctrl+S)',
  'CHECKING': 'ПРОВЕРКА',
  'A health check is configured; waiting for its first probe. The backend is out of routing until a probe passes.': 'Настроена проверка работоспособности; ожидание первой пробы. Бэкенд вне маршрутизации, пока проба не пройдёт.',
  'User': 'Пользователь',
  'Organizations': 'Организации',
  'Organization': 'Организация',
  'New Organization': 'Новая организация',
  'Create organization': 'Создать организацию',
  'Tokens and users you create while an organization is selected belong only to it, its members never see another org’s clients or tokens.': 'Token и пользователи, созданные при выбранной организации, принадлежат только ей, её участники никогда не видят клиентов или token другой организации.',
  'Organization "{name}" created': 'Организация «{name}» создана',
  'Organization "{name}" deleted': 'Организация «{name}» удалена',
  'Delete organization "{name}"?': 'Удалить организацию «{name}»?',
  'This cannot be undone. An organization can only be deleted once all its users and tokens are removed.': 'Это действие необратимо. Организацию можно удалить только после удаления всех её пользователей и token.',
  'Isolated tenants. Switch into an organization from the sidebar to manage its own tokens, users, and clients. The master organization is implicit, everything created without an organization belongs to it.': 'Изолированные арендаторы. Переключитесь в организацию на боковой панели, чтобы управлять её собственными token, пользователями и клиентами. Организация master подразумевается неявно, всё, созданное без организации, принадлежит ей.',
  'No organizations yet': 'Организаций пока нет',
  'master': 'master',
  'Switch organization': 'Сменить организацию',
  'This one client process holds {count} parallel connections to this service (connections: {count}); actions apply to all of them': 'Один этот клиентский процесс держит {count} параллельных подключений к этому сервису (connections: {count}); действия применяются ко всем.',
  'Table': 'Таблица',
  'Console': 'Консоль',
  'Requests in real time, table or console': 'Запросы в реальном времени, таблица или консоль',
  'no client': 'нет клиента',
  'offline': 'офлайн',
  'How every route reaches its destination: tunnel clients and their backends (with live request rates), plus the client-less routing the server owns, static redirects/responses and public expose ports, dashed nodes for token-granted routes no client currently serves, and amber nodes for another machine dialing a tunnel with --bind-tunnels (identified by the address it dialed from, so several processes behind one address are one node). Green = healthy, amber = draining or failing backend probes, red = unhealthy, disabled, ejected, or no client serving.': 'Как каждый маршрут достигает назначения: туннельные клиенты и их бэкенды (с живой частотой запросов), бесклиентная маршрутизация сервера, статические перенаправления/ответы и публичные expose-порты, и пунктирные узлы для выданных token маршрутов, которые не обслуживает ни один клиент. Зелёный = исправен, янтарный = завершает работу или проверки бэкенда не проходят, красный = неисправен, отключён, исключён или нет обслуживающего клиента. Другая машина, подключающаяся к туннелю через --bind-tunnels, показывается янтарным узлом и определяется по адресу подключения, поэтому несколько процессов за одним адресом это один узел.',
  'ejected': 'исключён',
  'draining': 'завершает работу',
  'cache off': 'кэш выкл',
  'This service opted into caching (cache: true) but the server response cache is disabled (APERIO_CACHE off), so the opt-in has no effect, enable the server cache or drop the flag': 'Этот сервис включил кэширование (cache: true), но кэш ответов сервера отключён (APERIO_CACHE off), поэтому опция не действует, включите кэш сервера или уберите флаг.',
  'Add hostname': 'Добавить hostname',
  'Hostname binds': 'Привязки hostname',
  'Temporary binds for this connection: while set, these are the only hostnames routed to it. Edit the row you want to move and leave the others as they are; empty rows are dropped, and emptying them all clears the override. Nothing is persisted across reconnects.':
    'Временные привязки для этого соединения: пока они заданы, сюда маршрутизируются только эти hostname. Измените нужную строку, остальные оставьте как есть; пустые строки отбрасываются, а очистка всех снимает переопределение. Ничего не сохраняется после переподключения.',
  'declared by the client': 'объявлено клиентом',
  'granted by the token': 'предоставлено token',
  'random subdomain, assigned by the server': 'случайный subdomain, выданный сервером',
  '{count} more hostname(s) routed here': 'ещё {count} hostname маршрутизируется сюда',
  'Configuration of {label}': 'Конфигурация {label}',
  'What this connection announces over its heartbeat, plus what the server applies on top. Settings a client never announces (its target, timeouts, header rules, health probes) cannot be shown here.':
    'То, что это соединение сообщает в heartbeat, плюс то, что сервер применяет поверх. Настройки, которые клиент никогда не сообщает (его target, таймауты, правила headers, проверки состояния), здесь показать нельзя.',
  'Loading…': 'Загрузка…',
  '{count} setting(s) differ from the configuration': 'Настроек, отличающихся от конфигурации: {count}',
  'not set': 'не задано',
  'resolved by the client': 'разрешено клиентом',
  'applied by the server': 'применено сервером',
  "Show this connection's effective configuration": 'Показать действующую конфигурацию этого соединения',
  'Config Builder': 'Конструктор конфигурации',
  'Assemble an aperio.yaml or aperio-server.yaml from the settings this server understands, or open an existing one and edit it.': 'Соберите aperio.yaml или aperio-server.yaml из настроек, которые понимает этот сервер, либо откройте существующий файл и отредактируйте его.',
  'Which file?': 'Какой файл?',
  'The client file configures a tunnel client and what it exposes; the server file configures the Aperio server itself.': 'Клиентский файл настраивает туннельный клиент и то, что он публикует; серверный, сам сервер Aperio.',
  'Empty fields are left out of the file entirely, so the server keeps its own default.': 'Пустые поля вообще не попадают в файл, поэтому сервер сохраняет собственное значение по умолчанию.',
  'Import YAML': 'Импорт YAML',
  'Export YAML': 'Экспорт YAML',
  'New': 'Новый',
  'Import {file}': 'Импорт {file}',
  'Imported {file}': '{file} импортирован',
  'Started a new {file}': 'Начат новый {file}',
  'Paste the document, or open a file. Settings the form cannot edit are kept as they are.': 'Вставьте документ или откройте файл. Настройки, которые форма не может редактировать, сохраняются как есть.',
  'A configuration file must be a mapping of settings.': 'Файл конфигурации должен быть отображением настроек.',
  'Open file…': 'Открыть файл…',
  'OK': 'ОК',
  'Copy it, or save it next to the binary that reads it.': 'Скопируйте его или сохраните рядом с бинарником, который его читает.',
  'Nothing is set yet, so the file would be empty.': 'Пока ничего не задано, файл был бы пустым.',
  'Save as file': 'Сохранить как файл',
  'Add entry': 'Добавить запись',
  'Remove entry': 'Удалить запись',
  'Entry {n}': 'Запись {n}',
  'comma-separated': 'через запятую',
  'on': 'вкл',
  'unset (server default)': 'не задано (значение сервера)',
  'unit': 'единица',
  'A request for this hostname is held for up to {secs}s while the service starts, instead of failing.': 'Запрос к этому hostname удерживается до {secs} с, пока сервис запускается, вместо ошибки.',
  'A scoped, revocable Bearer credential for automation, no master token needed.': 'Ограниченный и отзываемый Bearer-ключ для автоматизации, мастер-token не нужен.',
  'Admin key "{name}" created': 'Админ-ключ «{name}» создан',
  'Admin key "{name}" revoked': 'Админ-ключ «{name}» отозван',
  'Admin key created': 'Админ-ключ создан',
  'Allowed emails (comma)': 'Разрешённые адреса (через запятую)',
  'Allowed hostnames': 'Разрешённые hostname',
  'Allowed hostnames (optional)': 'Разрешённые hostname (необязательно)',
  'Automation using "{name}" will immediately lose access.': 'Автоматизация, использующая «{name}», немедленно потеряет доступ.',
  'Autoscaling': 'Автомасштабирование',
  'Canary token (any use triggers a breach alert)': 'Канареечный token (любое использование вызывает оповещение)',
  'Client id': 'ID клиента',
  'Client secret': 'Секрет клиента',
  'Cold starts only: no scale-out ceiling is configured.': 'Только холодный старт: потолок масштабирования не задан.',
  'Copy it now, it is shown only once and cannot be retrieved later.': 'Скопируйте сейчас, показывается только один раз и позже недоступно.',
  'Disarm': 'Отключить',
  'Disarm autoscaling for "{host}"?': 'Отключить автомасштабирование для «{host}»?',
  'Disarmed autoscaling for {host}': 'Автомасштабирование для {host} отключено',
  'Endpoint': 'Endpoint',
  'Entries': 'Записи',
  'Expires in days (empty = never)': 'Истекает через (дней; пусто = никогда)',
  'Export traffic CSV': 'Экспорт трафика в CSV',
  'Fences every bind made inside the organization: its tokens and clients can only claim these hostnames. Leave empty for no restriction.': 'Ограничивает любую привязку внутри организации: её token и клиенты могут занимать только эти hostname. Оставьте пустым, чтобы не ограничивать.',
  'Hit ratio': 'Доля попаданий',
  'Hits / misses': 'Попадания / промахи',
  'Instances': 'Экземпляры',
  'Issuer URL': 'URL издателя',
  'Key name is required': 'Имя ключа обязательно',
  'Max MB / month': 'Макс. МБ / месяц',
  'Max clients': 'Макс. клиентов',
  'Max tokens': 'Макс. token',
  'Max users': 'Макс. пользователей',
  'Memory (RSS)': 'Память (RSS)',
  'New admin key': 'Новый админ-ключ',
  'No admin keys yet': 'Админ-ключей пока нет',
  'No autoscaling records. A client arms one by declaring a scaling: block, and the server must have APERIO_SCALING enabled.': 'Записей автомасштабирования нет. Клиент создаёт запись, объявив блок scaling:, и на сервере должен быть включён APERIO_SCALING.',
  'No cache data yet': 'Данных кэша пока нет',
  'No hostname restriction': 'Без ограничения по hostname',
  'No self-health data yet': 'Данных самодиагностики пока нет',
  'OIDC SSO override': 'Переопределение OIDC SSO',
  'OIDC cleared': 'OIDC очищен',
  'OIDC configured': 'OIDC настроен',
  'Only these hostnames may be bound by this organization. Empty = no restriction.': 'Эта организация может привязывать только эти hostname. Пусто = без ограничений.',
  'Organization id (empty = master, * = every organization)': 'ID организации (пусто = master, * = все организации)',
  'Presets': 'Пресеты',
  'Process memory, store size, and traffic export.': 'Память процесса, размер хранилища и экспорт трафика.',
  'Programmatic admin keys': 'Программные админ-ключи',
  'Purge all': 'Очистить всё',
  'Purged {n} cache entries': 'Очищено записей кэша: {n}',
  'Requires the master admin': 'Требуется мастер-администратор',
  'Revoke admin key?': 'Отозвать админ-ключ?',
  'Save OIDC': 'Сохранить OIDC',
  'Scoped Bearer credentials for automation (master-admin only).': 'Ограниченные Bearer-ключи для автоматизации (только мастер-администратор).',
  'Server self-health': 'Самодиагностика сервера',
  'Server-side GET cache occupancy and hit rate.': 'Заполненность и доля попаданий серверного GET-кэша.',
  'Services that ask for capacity when they need it. A request for a hostname nothing is serving triggers a cold start instead of a 504, and a saturated pool asks for one more instance. Declared by the client in its scaling: block; scaling back in is the client’s own idle_timeout.': 'Сервисы, запрашивающие мощность по мере надобности. Запрос к hostname, который никто не обслуживает, вызывает холодный старт вместо 504, а перегруженный пул просит ещё один экземпляр. Клиент объявляет это в блоке scaling:; обратное сжатие делает его собственный idle_timeout.',
  'Set max_concurrent on the client to make utilization measurable.': 'Задайте max_concurrent на клиенте, чтобы загрузку можно было измерить.',
  'Sign in at /aperio/oidc/login?org={id}. Leave issuer empty to clear.': 'Вход по адресу /aperio/oidc/login?org={id}. Оставьте издателя пустым, чтобы очистить.',
  'Size': 'Размер',
  'State': 'Состояние',
  'Store size': 'Размер хранилища',
  'The call carries the client’s secret as a bearer token. The secret itself is never shown.': 'Вызов передаёт секрет клиента как bearer-token. Сам секрет никогда не показывается.',
  'The server never asks for more than {max} instances.': 'Сервер никогда не запрашивает больше {max} экземпляров.',
  'The server stops calling this endpoint, so the service will no longer cold start or scale out. A client that is still running and still declares the block re-arms it on its next heartbeat.': 'Сервер перестаёт вызывать этот endpoint, поэтому сервис больше не будет холодно стартовать и масштабироваться. Клиент, который всё ещё работает и по-прежнему объявляет блок, создаст запись заново при следующем heartbeat.',
  'This month: {req} requests, {bytes}, {clients} clients, {tokens} tokens, {users} users': 'За месяц: запросов {req}, {bytes}, клиентов {clients}, token {tokens}, пользователей {users}',
  'Too many consecutive failures: the endpoint is no longer called. Fix it and restart the client to re-arm.': 'Слишком много ошибок подряд: endpoint больше не вызывается. Исправьте и перезапустите клиент, чтобы включить снова.',
  'URL': 'URL',
  'Utilization': 'Загрузка',
  'armed': 'активно',
  'canary': 'канарейка',
  'disarmed': 'отключено',
  'not measurable': 'не измеряется',
  'scale to zero': 'масштабирование до нуля',
  '{inflight} of {capacity} concurrent slots in use; scales out above {target}%.': 'Занято {inflight} из {capacity} одновременных слотов; масштабируется выше {target}%.',
  'Must be at least {min}': 'Не менее {min}',
  'Need more than this? ': 'Нужно больше? ',
  'Open the config builder': 'Открыть конструктор конфигурации',
  ' to set every option, add services and tunnels, and export the file.': ', задайте все параметры, добавьте сервисы и туннели и выгрузите файл.',
  'File': 'Файл',
  'New entry name': 'Имя новой записи',
  'none configured': 'не настроено',
  '{n} entry(s) configured': 'настроено записей: {n}',
  'Other settings': 'Прочие настройки',
  'Everything not filed under a section above.': 'Всё, что не попало в разделы выше.',
  'More settings': 'Больше настроек',
  'Edit as YAML': 'Редактировать как YAML',
  'configured': 'настроено',
  'This section has no fixed shape, so it is edited as YAML. Leave it empty to remove it.': 'У этого раздела нет фиксированной структуры, поэтому он правится как YAML. Оставьте пустым, чтобы удалить.',
  'Configures a tunnel client and the backends it exposes.': 'Настраивает туннельный клиент и публикуемые им бэкенды.',
  'Configures the Aperio server itself.': 'Настраивает сам сервер Aperio.',
  'Bind': 'Привязать',
  'Client': 'Клиент',
  'Copy config': 'Скопировать конфигурацию',
  'No tunnels declared. A client declares them with a tunnels: list in its aperio.yaml; nothing about them is routed or exposed publicly.': 'Туннели не объявлены. Клиент объявляет их списком tunnels: в своём aperio.yaml; ничего из этого не маршрутизируется и не публикуется.',
  'Private services a client declares but never exposes: a database, an admin port, an SSH daemon. Bind one locally with --bind-tunnels.': 'Приватные сервисы, которые клиент объявляет, но никогда не публикует: база данных, административный порт, демон SSH. Привяжите один локально с помощью --bind-tunnels.',
  'Protocol': 'Протокол',
  'Target': 'Цель',
  'Tunnels': 'Туннели',
  'encrypted': 'зашифрован',
  'May bind other clients\' tunnels in this organization': 'Может привязывать туннели других клиентов этой организации',
  'may bind': 'может привязывать',
  'Server connection': 'Подключение к серверу',
  'Which Aperio server this client dials, and how it identifies itself.': 'К какому серверу Aperio подключается этот клиент и как он себя идентифицирует.',
  'Services': 'Сервисы',
  'Each entry is one exposed backend with its own bind and tuning.': 'Каждая запись, это опубликованный бэкенд со своей привязкой и настройками.',
  'Private TCP/UDP services a peer client can bind locally, and the peers this client binds.': 'Приватные TCP/UDP-сервисы, которые другой клиент может привязать локально, и узлы, которые привязывает этот клиент.',
  'Access control': 'Контроль доступа',
  'Who may reach the exposed service.': 'Кто может обращаться к опубликованному сервису.',
  'Capacity & pacing': 'Ёмкость и темп',
  'How much work this client accepts and how fast the server may push to it.': 'Сколько работы принимает этот клиент и как быстро сервер может её ему передавать.',
  'Backend health': 'Состояние бэкенда',
  'Probing the backend so an unhealthy one leaves the routing pool without dropping the tunnel. Set here it applies to every entry under services:, and any entry may override it.': 'Проверяет бэкенд, чтобы неисправный вышел из пула маршрутизации, не разрывая туннель. Заданное здесь применяется к каждой записи в services:, и любая запись может это переопределить.',
  'Asking for capacity when it is needed, and retiring when idle.': 'Запрашивать ёмкость при необходимости и сворачиваться при простое.',
  'Response handling': 'Обработка ответов',
  'Caching, resilience, and the headers this client rewrites.': 'Кеширование, отказоустойчивость и headers, которые переписывает этот клиент.',
  'Logging': 'Логирование',
  'What this client writes and in which format.': 'Что пишет этот клиент и в каком формате.',
  'Core': 'Основное',
  'Identity, listening address, and where state lives.': 'Идентичность, адрес прослушивания и место хранения состояния.',
  'Routing & load balancing': 'Маршрутизация и балансировка',
  'How a request finds a client, and which clients are eligible.': 'Как запрос находит клиента и какие клиенты подходят.',
  'Failover & retry': 'Отказоустойчивость и повторы',
  'What happens when a client dies mid-request or answers with an error.': 'Что происходит, когда клиент умирает посреди запроса или отвечает ошибкой.',
  'Limits & protection': 'Ограничения и защита',
  'Ceilings on bodies, concurrency, tunnels, and per-visitor rate.': 'Пределы на тела запросов, параллелизм, туннели и частоту на посетителя.',
  'The server-side GET cache for services that opt in.': 'Серверный GET-кеш для сервисов, которые его включили.',
  'Stream flow control': 'Управление потоком',
  'How much of a slow visitor’s download is buffered before the client pauses.': 'Какая часть загрузки медленного посетителя буферизуется, прежде чем клиент приостановится.',
  'Authentication & dashboard': 'Аутентификация и панель',
  'Who may log in, how, and what the dashboard offers.': 'Кто может входить, каким образом и что предлагает панель.',
  'Proxy trust': 'Доверие к proxy',
  'How the real client IP is resolved behind a proxy or CDN.': 'Как определяется настоящий IP клиента за proxy или CDN.',
  'Observability': 'Наблюдаемость',
  'Metrics, tracing, logs, alerting, and how long records are kept.': 'Метрики, трассировка, логи, оповещения и срок хранения записей.',
  'Outbound & autoscaling': 'Исходящие и автомасштабирование',
  'Where the server may call out to, and honouring client scaling blocks.': 'Куда сервер может обращаться и соблюдение клиентских блоков масштабирования.',
  'Edge integration': 'Интеграция с edge',
  'Publishing the live hostnames to a reverse proxy in front of this server.': 'Публикует активные hostname на обратный proxy перед этим сервером.',
  'Pages & headers': 'Страницы и headers',
  'What visitors see on an error, and server-wide header rewriting.': 'Что видят посетители при ошибке и переписывание headers на уровне сервера.',
  'Process & maintenance': 'Процесс и обслуживание',
  'Startup behaviour, backups, and the public expose ports.': 'Поведение при запуске, резервные копии и публичные expose-порты.',
  'Settings': 'Настройки',
  'Configure this server, its organizations and who may sign in.': 'Настройте этот сервер, его организации и тех, кто может входить.',
  'Server, organizations and users': 'Сервер, организации и пользователи',
  '(truncated)': '(усечено)',
  'Discard': 'Отбросить',
  'Discard unsaved changes?': 'Отбросить несохранённые изменения?',
  'Keep editing': 'Продолжить правку',
  'These settings were edited but never applied. Leaving now throws the edits away; the server keeps running what it has.': 'Эти настройки изменены, но не применены. Если уйти сейчас, правки будут потеряны; сервер продолжит работать с текущими значениями.',
  '{count} attempts': 'попыток: {count}',
  '{count} tokens': 'token: {count}',
  '{count} users': 'пользователей: {count}',
  'Edit organization "{name}"': 'Изменить организацию «{name}»',
  'Leave a field empty for no limit. The monthly allowance covers traffic proxied for this organization and resets each calendar month.': 'Пустое поле означает отсутствие ограничения. Месячный лимит распространяется на трафик, проксируемый для этой организации, и обнуляется каждый календарный месяц.',
  'Leave a limit empty for no limit. Usage is for the current calendar month.': 'Пустой лимит означает отсутствие ограничения. Расход показан за текущий календарный месяц.',
  'Limits (optional)': 'Лимиты (необязательно)',
  'Organization "{name}" was created, but its limits could not be saved: {error}. Set them with Edit.': 'Организация «{name}» создана, но её лимиты сохранить не удалось: {error}. Задайте их через «Изменить».',
  'Organization updated': 'Организация обновлена',
  'Tools': 'Инструменты',
  'Diagnose, inspect and generate, the things you reach for when something needs working out.': 'Диагностика, разбор и генерация, то, к чему обращаются, когда нужно в чём-то разобраться.',
  'Audit log, API explorer and config builder': 'Журнал аудита, обозреватель API и генератор конфигурации',
  'Settings & Tools': 'Настройки и инструменты',
  'Noindex preview hosts': 'Hostname предпросмотра с noindex',
  'Random-subdomain services answer with X-Robots-Tag: noindex and a disallow-all robots.txt': 'Сервисы со случайным subdomain отвечают header X-Robots-Tag: noindex и robots.txt, запрещающим всё',
  'Serve-stale window (s)': 'Окно устаревших ответов (с)',
  'How long an expired entry may still answer while a resilient service has no healthy client; 0 = off': 'Сколько ещё просроченная запись может отвечать, пока у отказоустойчивого сервиса нет здорового клиента; 0 = выключено',
  'Stream Flow Control': 'Управление потоком стримов',
  'Pause above': 'Пауза выше',
  'Backlog at which the producing client is told to stop reading that stream’s source': 'Объём очереди, при котором клиенту-источнику велено перестать читать источник этого стрима',
  'Resume below': 'Возобновление ниже',
  'Backlog at which a paused producer carries on; kept well under the pause mark so the pair cannot flap': 'Объём очереди, при котором приостановленный источник продолжает; заметно ниже порога паузы, чтобы пара не «дребезжала»',
  'Hard backlog cap': 'Жёсткий предел очереди',
  'A stream whose producer cannot be paused (an older client) is dropped past this': 'Стрим, источник которого нельзя приостановить (старый клиент), сверх этого сбрасывается',
  '(binary body, {bytes} bytes)': '(двоичное тело, {bytes} байт)',
  '(binary body, {bytes} bytes, truncated)': '(двоичное тело, {bytes} байт, усечено)',
  '(binary payload)': '(двоичный payload)',
  '(empty)': '(пусто)',
  '(streamed body, not captured)': '(потоковое тело, не сохранено)',
  '(truncated at 64 KB)': '(усечено на 64 КБ)',
  '(unable to decode body)': '(тело не удалось декодировать)',
  'Connected': 'Подключено',
  'Disconnected': 'Отключено',
  'Waiting': 'Ожидание',
  'status trend': 'динамика статусов',
  'TOTP QR code': 'QR-код TOTP',
  'How much of a slow visitor’s download the server buffers before asking the client to pause producing it.': 'Какую часть загрузки медленного посетителя сервер буферизует, прежде чем попросить клиента приостановить её создание.',
  'Service defaults': 'Значения по умолчанию для сервисов',
  'Applied to every entry under services:, and overridable per entry. The keys that name one backend at the top level are deprecated here, they only ever worked without a services: list, and appear only for a file that already writes them.': 'Применяются к каждой записи в services: и переопределяются в записи. Ключи, задающие один бэкенд на верхнем уровне, здесь устарели, они и работали только при отсутствии списка services:, и показываются лишь для файла, который их уже пишет.',
  '`{keys}` name one service at the top level, and this file also has a services: list, the client reads the list and ignores them. Move them into the entry they belong to.': '`{keys}` задают один сервис на верхнем уровне, а в файле есть ещё и список services:, клиент читает список и игнорирует их. Перенесите их в ту запись, к которой они относятся.',
  'Every healthy client of the route takes requests in turn, evenly. The default, and what you want when the clients are interchangeable.': 'Каждый исправный клиент маршрута берёт запросы по очереди, поровну. Значение по умолчанию и то, что нужно, когда клиенты взаимозаменяемы.',
  'Only the clients on the lowest priority tier receive traffic; a higher tier takes over when every client above it is unhealthy, draining or gone, and hands back when one returns. Tiers come from each client’s priority (0 = primary).': 'Трафик получают только клиенты самого низкого приоритетного уровня; уровень выше принимает нагрузку, когда все клиенты над ним неисправны, выводятся или исчезли, и отдаёт её обратно, когда один возвращается. Уровни берутся из priority каждого клиента (0 = основной).',
  'A visitor keeps the client that first served them, for as long as it stays healthy. For backends holding per-visitor state in memory; the pool spreads by visitor rather than by request.': 'Посетитель остаётся на том клиенте, который обслужил его первым, пока тот исправен. Для бэкендов, хранящих состояние посетителя в памяти; пул распределяет по посетителям, а не по запросам.',
  'The visitor gets the error. Nothing is retried, which is the only safe answer if a request may not run twice.': 'Посетитель получает ошибку. Ничего не повторяется, единственный безопасный ответ, если запрос нельзя выполнить дважды.',
  'Re-dispatch to another healthy client of the same route, if there is one. Nothing waits.': 'Переотправить другому исправному клиенту того же маршрута, если он есть. Ожидания нет.',
  'Hold the request while the same client reconnects, up to the failover window. For a single-client route, where there is nothing to fail over to.': 'Удерживать запрос, пока тот же клиент переподключается, в пределах окна failover. Для маршрута с одним клиентом, где переключаться не на кого.',
  'Try another client first; if the route has none, wait for one to come back. The most forgiving, and the slowest to give up.': 'Сначала попробовать другого клиента; если у маршрута его нет, дождаться возвращения. Самый снисходительный и самый медленный в отказе.',
  'Message topics (comma separated, empty = no messaging)': 'Топики сообщений (через запятую, пусто = обмен сообщениями запрещён)',
  '{count} topic(s)': '{count} топик(ов)',
  'Subscribers': 'Подписчики',
  'Client processes of this organization listening for messages, and the topic filters they asked for. A client running several services counts once.': 'Клиентские процессы этой организации, слушающие сообщения, и запрошенные ими фильтры топиков. Клиент с несколькими сервисами считается один раз.',
  'Nothing is subscribed. A client subscribes with subscribe: in its config, or by attaching to its local message face.': 'Подписок нет. Клиент подписывается через subscribe: в своей конфигурации или подключившись к своему локальному интерфейсу сообщений.',
  'unnamed client': 'клиент без имени',
  '{count} connection(s)': '{count} соединени(й)',
  'Publish': 'Опубликовать',
  'Sends a message to this organization. The reply says how many client processes it reached, which is the quickest way to tell a wrong filter from a wrong topic.': 'Отправляет сообщение этой организации. В ответе, до скольких клиентских процессов оно дошло: самый быстрый способ отличить неверный фильтр от неверного топика.',
  'Topic': 'Топик',
  'A topic, not a filter: `+` and `#` belong to subscriptions.': 'Топик, а не фильтр: `+` и `#` относятся к подпискам.',
  'Message': 'Сообщение',
  'At least once': 'Как минимум один раз',
  'Held until each subscriber acknowledges it, and resent meanwhile. Nothing is stored for a client that is offline.': 'Удерживается, пока каждый подписчик не подтвердит, и до тех пор пересылается. Для клиента не в сети ничего не сохраняется.',
  'Published to "{topic}", {count} client(s)': 'Опубликовано в «{topic}», {count} клиент(ов)',
  'Published to "{topic}", but nothing is subscribed to it': 'Опубликовано в «{topic}», но на него никто не подписан',
  'Messages': 'Сообщения',
  'Topics this client listens for, and where an application on the same machine attaches to send and receive them.': 'Топики, которые слушает этот клиент, и куда подключается приложение на той же машине, чтобы отправлять и получать.',
  'Cold start & scale-out records': 'Записи о холодном старте и масштабировании',
  'Private services reachable with --bind-tunnels': 'Приватные сервисы, доступные через --bind-tunnels',
  'Display name': 'Отображаемое имя',
  'Handle': 'Идентификатор',
  'Only what it is called. The handle {handle} is what addresses it and never changes.': 'Только то, как её называют. Идентификатор {handle} адресует её и никогда не меняется.',
  'Service {name} (custom_name: {custom})': 'Сервис {name} (custom_name: {custom})',
  'What this organization is called on screen. Any language, any punctuation, and changeable later.': 'Как эта организация называется на экране. Любой язык, любая пунктуация, изменяемо позже.',
  'a-z, 0-9 and _ . This is what addresses the organization, in {example}, in a server’s expose: rule and in the API, so it is fixed once created.': 'a-z, 0-9 и _ . Именно это адресует организацию, в {example}, в правиле expose: сервера и в API, поэтому после создания оно не меняется.',
  'Could not delete ({status})': 'Не удалось удалить ({status})',
  'Parallel connections per service (0 = server default, empty = keep)': 'Параллельные соединения на сервис (0 = значение сервера, пусто = не менять)',
  'Parallel connections per service (empty = server default)': 'Параллельные соединения на сервис (пусто = значение сервера)',
  'Detail not available: only recent requests are captured, and a service with capture: false (or a server with the inspector off) is not captured at all.': 'Детали недоступны: сохраняются только недавние запросы, а сервис с capture: false (или сервер с выключенным инспектором) не сохраняется вовсе.',
  'This service allows capture but the server has the request inspector switched off (APERIO_INSPECTOR=0), so nothing here can be inspected or replayed.': 'Этот сервис разрешает запись, но на сервере инспектор запросов выключен (APERIO_INSPECTOR=0), поэтому здесь нечего просматривать и нечего повторить.',
  'This service is not recorded for the request inspector (capture: false in its aperio.yaml), so its requests cannot be inspected or replayed. Live traffic and statistics are unaffected.': 'Этот сервис не записывается для инспектора запросов (capture: false в его aperio.yaml), его запросы нельзя просмотреть или повторить. На живой трафик и статистику это не влияет.',
  'inspector off': 'инспектор выключен',
  'no capture': 'без записи',
  '{count} setting(s) here differ from what the server started with': 'Здесь {count} настроек(и) отличаются от тех, с которыми запустился сервер',
  'from file': 'из файла',
  'aperio-server.yaml sets this, and the file wins. Edit it there: a change made here would be refused, because a stored override the file contradicts is exactly the invisible state this avoids.': 'Это задаёт aperio-server.yaml, и файл главнее. Меняйте там: изменение здесь будет отклонено, ведь сохранённый override, которому противоречит файл, это ровно то невидимое состояние, которого мы избегаем.',
  '{count} setting(s) are stored overrides, not what your config file says': '{count} настроек хранятся как override, а не как то, что написано в вашем конфиге',
  'They apply immediately and survive a restart, but they live in the server’s own store, so aperio-server.yaml still describes a different server than the one running. Put them in the file to make them the deployment’s: the file wins, and the override behind each of these keys is dropped at the next start.': 'Они действуют сразу и переживают перезапуск, но лежат в собственном хранилище сервера, поэтому aperio-server.yaml по-прежнему описывает не тот сервер, который работает. Впишите их в файл, чтобы они стали частью развёртывания: файл побеждает, и override по каждому из этих ключей отбрасывается при следующем старте.',
  'Copy as YAML': 'Копировать как YAML',
  'A logical JSON dump, a failsafe for upgrades and migrations. Choose what travels: the configuration that rebuilds a deployment is on by default, the history is there if you want it. Sessions and the audit log are never exported.': 'Логический JSON-дамп, страховка при обновлениях и переездах. Выберите, что поедет: конфигурация, восстанавливающая развёртывание, включена по умолчанию, история, если она нужна. Сессии и журнал аудита не экспортируются никогда.',
  'An import applies whatever sections the file holds, so it is the export that decides.': 'Импорт применяет те разделы, которые есть в файле, так что решает именно экспорт.',
  'Importing replaces every section the file contains, on this server. Continue?': 'Импорт заменит на этом сервере каждый раздел, который есть в файле. Продолжить?',
  'Organizations are not selected, so only the master organization travels: {sections} belonging to a tenant are left out, its statistics with them. A row whose organization does not exist on the target server would be an orphan.': 'Организации не выбраны, поэтому поедет только master: {sections}, принадлежащие арендатору, останутся, и его статистика вместе с ними. Запись, чьей организации нет на целевом сервере, осталась бы сиротой.',
  'Access tokens': 'Token доступа',
  'Dashboard users': 'Пользователи панели',
  'Settings overrides': 'Override настроек',
  'Statistics': 'Статистика',
  'Uptime history': 'История доступности',
  'Webhook inbox': 'Входящие webhook',
  'Admin API keys': 'Административные API-ключи',
  'Hashes only, never the secrets': 'Только хеши, никаких секретов',
  'Endpoints, events and signing secrets': 'Endpoint, события и секреты подписи',
  'Password hashes, TOTP secrets, passkeys': 'Хеши паролей, секреты TOTP, passkey',
  'Tenants, quotas and hostname allowlists': 'Арендаторы, квоты и списки разрешённых hostname',
  'Cold-start and scale-out records': 'Записи холодного старта и масштабирования',
  'Server settings changed from this dashboard': 'Настройки сервера, изменённые из этой панели',
  'Lifetime counters and the daily, weekly, monthly and yearly buckets': 'Счётчики за всё время и дневные, недельные, месячные и годовые корзины',
  'Per-client and per-hostname availability by day': 'Доступность по клиентам и hostname, по дням',
  'Captured inbound webhook payloads': 'Захваченные входящие payload webhook',
  'Programmatic admin credentials, hashes only': 'Программные административные учётные данные, только хеши',
  'No hostnames in maintenance. Visitors of a listed hostname get the 503 page while its clients stay connected; cleared on server restart. Use *.example.com for every subdomain of a domain, and list the domain itself too if you want it as well.': 'Нет hostname на обслуживании. Посетители указанного hostname видят страницу 503, пока его клиенты остаются подключёнными; сбрасывается при перезапуске сервера. Для всех subdomain домена используйте *.example.com, а если нужен и сам домен, добавьте его отдельно.',
  'app.example.com, *.example.com, *-pi.example.com, or *': 'app.example.com, *.example.com, *-pi.example.com или *',
  'every subdomain': 'все subdomain',
  'Not in the snippet: {settings}. The file has no spelling for these, its 503/504 keys take a path where this holds the HTML itself, so they stay dashboard overrides.': 'Нет в сниппете: {settings}. В файле для них нет написания: его ключи 503/504 принимают путь, а здесь хранится сам HTML, поэтому они остаются override панели.',
  '{settings}: the file key is read at startup, so this one takes a restart rather than applying on reload.': '{settings}: ключ в файле читается при старте, поэтому нужен перезапуск, а не перечитывание конфигурации.',
  'Maintenance window': 'Окно обслуживания',
  'lifts in {duration}': 'снимется через {duration}',
  'reason, shown on the 503 page (optional)': 'причина, видна на странице 503 (необязательно)',
  'set by {actor} {when}': 'установил {actor} {when}',
  'until turned off': 'до отключения',
  '15 min': '15 мин',
  '1 h': '1 ч',
  '4 h': '4 ч',
  'Explain': 'Объяснить',
  'Explain a request': 'Объяснить запрос',
  'What would answer a request to this hostname and path, and why. Nothing is sent: no rate limit is spent, no round-robin cursor moves, and a scaled-to-zero service is not woken.': 'Что ответило бы на запрос к этому hostname и path и почему. Ничего не отправляется: rate limit не расходуется, курсор round-robin не двигается, свёрнутый до нуля сервис не будится.',
  'app.example.com/api, or a full URL': 'app.example.com/api или полный URL',
  '60 s': '60 с',
  '1 d': '1 д',
  '2 h': '2 ч',
  'Requests / second, in 5-second slices (last 15 minutes)': 'Запросов / секунду, срезами по 5 секунд (последние 15 минут)',
  'Requests / second, in 2-minute slices (last 2 hours)': 'Запросов / секунду, срезами по 2 минуты (последние 2 часа)',
  'Requests / second, in 15-minute slices (last 24 hours)': 'Запросов / секунду, срезами по 15 минут (последние 24 часа)',
  'now': 'сейчас',
  '{count} requests': 'запросов: {count}',
  '{count}m ago': '{count} мин назад',
  '{count}s ago': '{count} с назад',
  'Search': 'Поиск',
  'From': 'С',
  'To': 'По',
  'Apply filters': 'Применить фильтры',
  'Export CSV': 'Экспорт в CSV',
  'Download the matching events as CSV': 'Скачать подходящие события в CSV',
  'No events match these filters': 'Нет событий, соответствующих фильтрам',
  'Searching the whole audit log; live updates are paused': 'Поиск по всему журналу аудита; live-обновления приостановлены',
  'Showing recent events. Filter to search the whole log.': 'Показаны недавние события. Отфильтруйте, чтобы искать по всему журналу.',
  'details, event or user': 'детали, событие или пользователь',
  'Tunnel round trip, as the client measures it': 'Время оборота по туннелю, по измерению клиента',
  'jitter': 'джиттер',
  'reconnects': 'переподключений',
  'client CPU': 'CPU клиента',
  'client memory': 'память клиента',
  "\"{name}\" answered {status} in {ms} ms": "\"{name}\" ответил {status} за {ms} мс",
  "\"{name}\" failed: {reason}": "\"{name}\" не удался: {reason}",
  "Test": "Тест",
  "no response": "нет ответа",
  idle: 'простаивает',
  "May send OpenTelemetry exports through the server's bridge": 'Может отправлять экспорт OpenTelemetry через мост сервера',
  'may export otel': 'может слать otel',
  'just now': 'только что',
  'never': 'никогда',
  'expired {date}': 'истёк {date}',
  '{count}h ago': '{count} ч назад',
  '{count}d ago': '{count} дн назад',
  '{count}s': '{count} с',
  '{count}m': '{count} мин',
  '{count}h': '{count} ч',
  '{count}d': '{count} дн',
  '{hours}h {minutes}m': '{hours} ч {minutes} мин',
  '{days}d {hours}h': '{days} дн {hours} ч',
  'Any event': 'Любое событие',
  'Sign-in': 'Вход',
  'Tokens and keys': 'Tokens и ключи',
  'Clients and tunnels': 'Клиенты и туннели',
  'Users and organizations': 'Пользователи и организации',
  'Configuration': 'Конфигурация',
  'Data': 'Данные',
  'Alerts and scaling': 'Оповещения и масштабирование',
  'Webhooks and messages': 'Webhooks и сообщения',
  'Toggle Sidebar': 'Переключить боковую панель',
  'Displays the mobile sidebar.': 'Показывает мобильную боковую панель.',
  'a {kind} name cannot be empty': 'Имя {kind} не может быть пустым',
  '{kind} name is longer than {max} characters': 'Имя {kind} длиннее {max} символов',
  "{kind} name '{name}' may only contain a-z, 0-9 and _ (write it as '{suggestion}')": "Имя {kind} '{name}' может содержать только a-z, 0-9 и _ (запишите как '{suggestion}')",
  '503: a maintenance flag set by {actor} covers this hostname, until someone turns it off':
    '503: отметка обслуживания, поставленная {actor}, покрывает этот hostname, пока её кто-нибудь не снимет',
  '503: a maintenance flag set by {actor} covers this hostname ({reason}), until someone turns it off':
    '503: отметка обслуживания, поставленная {actor}, покрывает этот hostname ({reason}), пока её кто-нибудь не снимет',
  '503: a maintenance flag set by {actor} covers this hostname, lifting at unix {until}':
    '503: отметка обслуживания, поставленная {actor}, покрывает этот hostname, снимается в unix {until}',
  '503: a maintenance flag set by {actor} covers this hostname ({reason}), lifting at unix {until}':
    '503: отметка обслуживания, поставленная {actor}, покрывает этот hostname ({reason}), снимается в unix {until}',
  'no maintenance flag covers this hostname':
    'ни одна отметка обслуживания не покрывает этот hostname',
  'no routes: rules configured':
    'правила routes: не настроены',
  '{status}: a routes: rule answers this path':
    '{status}: правило routes: отвечает на этот path',
  '{status}: a routes: rule answers this path, to {location}':
    '{status}: правило routes: отвечает на этот path, перенаправляя на {location}',
  'no routes: rule matches this hostname and path':
    'ни одно правило routes: не подходит к этому hostname и path',
  '200: preview_noindex answers /robots.txt itself, so this path never reaches a client':
    '200: preview_noindex сам отвечает на /robots.txt, поэтому этот path никогда не доходит до клиента',
  'no waf: rules configured':
    'правила waf: не настроены',
  '403: blocked by a waf: rule ({reason})':
    '403: заблокировано правилом waf: ({reason})',
  'no waf: rule matches this method and path (header and body rules need a real request)':
    'ни одно правило waf: не подходит к этому методу и path (правила для header и тела требуют настоящего запроса)',
  'a rate_limits: rule covers this path ({rps} rps, burst {burst}); this dry run does not spend from it':
    'правило rate_limits: покрывает этот path ({rps} rps, всплеск {burst}); этот пробный прогон из него не расходует',
  'a rate_limits: rule covers this path ({rps} rps, burst {burst}, {methods} only); this dry run does not spend from it':
    'правило rate_limits: покрывает этот path ({rps} rps, всплеск {burst}, только {methods}); этот пробный прогон из него не расходует',
  'no rate_limits: rule covers this path':
    'ни одно правило rate_limits: не покрывает этот path',
  "visitors must sign in (or carry a share link) before this reaches a client: the serving client declared a visitor password for this route, which supersedes the server's own gate":
    'посетители должны войти (или иметь ссылку доступа), прежде чем это дойдёт до клиента: обслуживающий клиент объявил собственный пароль посетителя для этого маршрута, и он важнее собственной двери сервера',
  "visitors must sign in (or carry a share link) before this reaches a client: the server's visitor gate is on, and OIDC is configured":
    'посетители должны войти (или иметь ссылку доступа), прежде чем это дойдёт до клиента: дверь для посетителей на сервере включена и настроен OIDC',
  "visitors must sign in (or carry a share link) before this reaches a client: the server's visitor password is set":
    'посетители должны войти (или иметь ссылку доступа), прежде чем это дойдёт до клиента: на сервере задан пароль посетителя',
  'visitors must sign in (or carry a share link) before this reaches a client: OIDC is configured for visitors':
    'посетители должны войти (или иметь ссылку доступа), прежде чем это дойдёт до клиента: для посетителей настроен OIDC',
  'this hostname is served without a visitor gate':
    'этот hostname обслуживается без двери для посетителей',
  "visitors must sign in (or carry a share link) before this reaches a client: this route asks for an Aperio sign-in, a dashboard session reaching this hostname's organization":
    'Посетители должны войти (или принести ссылку общего доступа), прежде чем это дойдёт до клиента: этот маршрут требует входа Aperio, сессии панели, достигающей организации этого имени хоста',
  'nothing declares this route reachable and `default_access` is `deny`, so it is not published: a visitor carrying no Aperio session gets the same answer an unclaimed hostname gives, and a signed-in Aperio user still reaches it':
    'ничто не объявляет этот маршрут доступным, а `default_access` установлен в `deny`, поэтому он не публикуется: посетитель без сессии Aperio получает тот же ответ, что и для незанятого имени хоста, а вошедший пользователь Aperio по-прежнему до него доходит',
  '{count} client(s) would take it: {clients}':
    'запрос взяли бы {count} клиент(ов): {clients}',
  'no connected client serves this hostname and path':
    'ни один подключённый клиент не обслуживает этот hostname и path',
  'no connected client serves this hostname and path, though {count} could: {ineligible}':
    'ни один подключённый клиент не обслуживает этот hostname и path, хотя {count} могли бы: {ineligible}',
  'an autoscaling record is armed for this bind, so the request would be held while capacity is asked for, rather than answered at once':
    'для этой привязки взведена запись автомасштабирования, поэтому запрос удерживали бы, пока запрашивается мощность, вместо немедленного ответа',
  '{status}: the fallbacks: rule for this hostname redirects to {url} instead of a 504':
    '{status}: правило fallbacks: для этого hostname перенаправляет на {url} вместо 504',
  '504: nothing serves this route, and no fallbacks: rule covers it':
    '504: этот маршрут никто не обслуживает, и ни одно правило fallbacks: его не покрывает',
  'the request reaches a tunnel client ({clients})':
    'запрос доходит до туннельного клиента ({clients})',
  'disabled from the dashboard':
    'отключён из панели',
  'its backend health probe is failing':
    'его проверка здоровья бэкенда не проходит',
  'missed heartbeats':
    'пропущенные heartbeat',
  'its path bind does not match':
    'его привязка path не совпадает',
  'Static route':
    'Статический маршрут',
  'Preview noindex':
    'Preview noindex',
  'WAF':
    'WAF',
  'Rate limit':
    'Ограничение частоты',
  'Visitor gate':
    'Дверь для посетителей',
  'Routing':
    'Маршрутизация',
  'Cold start':
    'Холодный старт',
  'Fallback':
    'Запасной вариант',
  'No client':
    'Нет клиента',
  'maintenance mode':
    'режим обслуживания',
  'auth: on the service':
    'auth: у сервиса',
  'server_auth / OIDC':
    'server_auth / OIDC',
  'default_access':
    'default_access',
  'server_auth':
    'server_auth',
  'OIDC':
    'OIDC',
  'hostname/path binds':
    'привязки hostname/path',
  'Served by': 'Обслужил',
  'Connection id: {id}': 'Идентификатор соединения: {id}',
  "another organization's client": 'клиент другой организации',
  'no source-IP deny list is configured': 'список блокировки исходных IP не настроен',
  'a source-IP deny list is in force ({count} entries); a request from a listed address is refused before anything below runs, whatever this report says': 'действует список блокировки исходных IP ({count} записей); запрос с указанного адреса отклоняется до того, как выполнится что-либо ниже, что бы ни говорил этот отчёт',
  'every visitor IP gets a bucket of {max} with {refill}/s refill; this dry run does not spend from it': 'каждый IP посетителя получает ведро на {max} с пополнением {refill}/с; этот пробный прогон из него не расходует',
  '{in_flight} of {max} server-wide request slots are in use right now; a request arriving with none free is refused rather than queued': '{in_flight} из {max} общесерверных слотов запросов заняты прямо сейчас; запрос, пришедший без свободного слота, отклоняется, а не ставится в очередь',
  'the serving service declares allowed_ips, so a visitor outside that list is routed nowhere and gets the 504 below rather than a refusal naming the list': 'обслуживающий сервис объявляет allowed_ips, поэтому посетитель вне этого списка никуда не маршрутизируется и получает 504 ниже вместо отказа с упоминанием списка',
  "a request body over {effective} bytes is refused with 413; the service may tighten the server's limit but never widen it": 'тело запроса свыше {effective} байт отклоняется с 413; сервис может ужесточить предел сервера, но никогда не расширить его',
  'the serving client admits {max} at a time; past that a request waits for a slot and is refused if none frees before the gateway timeout': 'обслуживающий клиент принимает {max} одновременно; сверх этого запрос ждёт слот и отклоняется, если ни один не освободится до таймаута шлюза',
  'the serving client declares no concurrency limit of its own': 'обслуживающий клиент не объявляет собственного предела параллелизма',
  'this route is cacheable, so a fresh entry would answer here without the request reaching a client at all': 'этот маршрут кэшируем, поэтому свежая запись ответила бы здесь, и запрос вообще не дошёл бы до клиента',
  'response caching is off server-wide': 'кэширование ответов выключено на всём сервере',
  'the serving service does not opt into caching, so every request goes to the backend': 'обслуживающий сервис не подключается к кэшу, поэтому каждый запрос идёт на бэкенд',
  'a dynamic token carries its own requests-per-second limit and daily byte quota; which token a request arrives with is not a property of this route, so neither is checked here': 'динамический токен несёт собственный предел запросов в секунду и суточную квоту байт; с каким токеном придёт запрос, не является свойством этого маршрута, поэтому здесь не проверяется ни то, ни другое',
  "the same token's daily byte quota is likewise a property of the credential": 'суточная квота байт того же токена точно так же является свойством учётных данных',
  'this organization is within its monthly byte quota': 'эта организация укладывается в месячную квоту байт',
  '429: this organization is over its monthly byte quota, so every request it serves is refused until the month turns': '429: эта организация превысила месячную квоту байт, поэтому каждый обслуживаемый ею запрос отклоняется до смены месяца',
  'the master organization has no monthly byte quota': 'у мастер-организации нет месячной квоты байт',
  'no ceiling on concurrently open streamed responses per visitor': 'нет предела одновременно открытых потоковых ответов на посетителя',
  'one visitor IP may hold {max} streamed responses open at once; a streamed answer past that is refused': 'один IP посетителя может держать {max} потоковых ответов открытыми одновременно; потоковый ответ сверх этого отклоняется',
  'Deny list': 'Список блокировки',
  'Visitor rate limit': 'Лимит частоты посетителя',
  'Server concurrency': 'Параллелизм сервера',
  'Body limit': 'Предел тела',
  'Client concurrency': 'Параллелизм клиента',
  'Cache': 'Кэш',
  'Token quota': 'Квота токена',
  'Organization quota': 'Квота организации',
  'Streams per visitor': 'Потоки на посетителя',
  'the server admits {max} requests at once across every service; one arriving with no slot free is refused rather than queued': 'сервер принимает {max} запросов одновременно по всем сервисам; пришедший без свободного слота отклоняется, а не ставится в очередь',
  'caching applies to GET; a {method} always reaches a client': 'кэширование применяется к GET; запрос {method} всегда доходит до клиента',
  'Could not load the tunnels; retrying.': 'Не удалось загрузить туннели; повторная попытка.',
  '1. Mint a token': '1. Создайте токен',
  'A token is what a client connects with. Scope it to the hostname the service will answer on.': 'Токен — это то, с чем подключается клиент. Ограничьте его именем хоста, на котором будет отвечать сервис.',
  'A token is what a client connects with. An operator mints one on the API Tokens page.': 'Токен — это то, с чем подключается клиент. Оператор создаёт его на странице API-токенов.',
  '2. Run the client next to your service': '2. Запустите клиент рядом с сервисом',
  'On the machine that can reach the service, with the token from step 1. The Clients page has a wizard that writes this for you.': 'На машине, которой доступен сервис, с токеном из шага 1. На странице Клиенты есть мастер, который напишет это за вас.',
  '3. Watch it appear here': '3. Смотрите, как он появится здесь',
  'The client dials out to this server and the tunnel is up; this card leaves the moment it connects.': 'Клиент подключается наружу к этому серверу, и туннель поднят; эта карточка исчезнет в момент подключения.',
  'No client is connected yet': 'Клиент ещё не подключён',
  'Three steps put a service on this server. Nothing on your side accepts inbound connections; the client dials out.': 'Три шага выводят сервис на этот сервер. На вашей стороне ничто не принимает входящие соединения; клиент подключается наружу.',
  'Getting started, the full walkthrough': 'Начало работы, полное руководство',
  'Documentation for this page': 'Документация по этой странице',
  'Docs': 'Документация',
  'The built-in account is aperio, with the master token as its password; a named user signs in with their own.': 'Встроенная учётная запись — aperio, пароль — мастер-токен; именованный пользователь входит со своими данными.',
  'About this page': 'Об этой странице',
  'Server Health': 'Состояние сервера',
  'Process, store and cache health': 'Состояние процесса, хранилища и кеша',
  'Routes & latency': 'Маршруты и задержка',
  'Read more': 'Подробнее',
  'Redirect hostname': 'Перенаправить имя хоста',
  'Redirect client {id}…': 'Перенаправить клиента {id}…',
  'A bind is the hostname or path prefix a client claims; a request matching it is routed to that client.': 'Bind — это имя хоста или префикс пути, которые заявляет клиент; подходящий запрос направляется этому клиенту.',
  'A draining client has announced it is shutting down: it gets no new requests and finishes the ones in flight.': 'Клиент в состоянии draining объявил о завершении работы: новые запросы ему не поступают, текущие он завершает.',
  'An ejected service failed too often under real traffic and is out of rotation for a while, even if its health probe is green.': 'Исключённый сервис слишком часто отказывал под реальным трафиком и на время выведен из ротации, даже если его проба здоровья зелёная.',
  'A canary token is a decoy: it serves nothing, and any attempt to use it raises an alert, so a leaked copy surfaces itself.': 'Токен-канарейка — это приманка: он ничего не обслуживает, а любая попытка им воспользоваться поднимает тревогу, так утёкшая копия выдаёт себя.',
  'An expose entry opens a raw public port on the server that relays into a tunnel a client declared, with no second client needed.': 'Запись expose открывает на сервере сырой публичный порт, который ретранслирует в туннель, объявленный клиентом, без второго клиента.',
  'A hostname override redirects a live client to other hostnames from the dashboard; it lives in memory and a reconnect or restart reverts it.': 'Переопределение имени хоста перенаправляет работающий клиент на другие имена из панели; оно живёт в памяти, и переподключение или перезапуск его отменяет.',
  'updated just now': 'обновлено только что',
  'updated {seconds}s ago': 'обновлено {seconds} с назад',
  'waiting for data…': 'ожидание данных…',
  'Pushed over the live stream; this is when it last changed.': 'Приходит по живому потоку; это момент последнего изменения.',
  'Clear the whole inbox?': 'Очистить весь входящий ящик?',
  'Every stored inbound webhook of this organization is deleted; a re-fire is no longer possible for them.': 'Все сохранённые входящие вебхуки этой организации удаляются; повторно отправить их будет нельзя.',
  'Purge the whole response cache?': 'Очистить весь кеш ответов?',
  'Every cached response is dropped and the next request for each reaches its backend again.': 'Каждый кешированный ответ удаляется, и следующий запрос снова доходит до бэкенда.',
  'Search tokens…': 'Поиск токенов…',
  'Search users…': 'Поиск пользователей…',
  'Search webhooks…': 'Поиск вебхуков…',
  'Nothing matches "{search}"': 'Ничего не найдено по «{search}»',
  'No dashboard users yet; the master token still signs in.': 'Пользователей панели пока нет; мастер-токен по-прежнему входит.',
  'Role-based access. The master token always signs in as the built-in admin ("aperio").': 'Доступ по ролям. Мастер-токен всегда входит как встроенный администратор ("aperio").',
  'Healthy': 'Исправен',
  'Unhealthy': 'Неисправен',
  'Available': 'Доступен',
  'Unavailable': 'Недоступен',
  'Run the client with it': 'Запустите клиент с ним',
  "Replace 3000 with your service's port; the Clients page has a wizard for Docker and yaml.": 'Замените 3000 на порт вашего сервиса; на странице Клиенты есть мастер для Docker и yaml.',
  'by {actor}, {when}': '{actor}, {when}',
  '{count} alert(s) since yesterday, see the audit log': '{count} оповещений со вчерашнего дня, см. журнал аудита',
  'No alerts since yesterday': 'Оповещений со вчерашнего дня нет',
}
