...
export const ja: Record<string, string> = {
  ...
  'Cloudflare shorthand: trust the CF-Connecting-IP header':
    'Cloudflare向けショートカット: CF-Connecting-IP Header を信頼',
  ...
  'Env vars provide the defaults; edits become live, persisted overrides. Master token, HOST/PORT, proxy trust and OIDC stay env-only.':
    '環境変数がデフォルト値を提供し、編集は即時反映かつ永続化されるオーバーライドになります。マスター Token、HOST/PORT、Proxy 信頼、OIDCは環境変数のみです。',
  ...
  'Give someone temporary access to an auth-protected site: the link carries a signed, expiring token scoped to the hostname (and optional path). Opening it sets a cookie and redirects to the clean URL. Links are stateless, they cannot be listed later, they simply expire.':
    '認証保護されたサイトへの一時アクセスを付与します。リンクには Hostname（と任意の Path）に限定された署名付き期限付き Token が含まれます。開くとCookieが設定され、クリーンなURLへリダイレクトされます。リンクはステートレスで、後から一覧できず、期限が切れるだけです。',
  'Go to {page}': '{page}へ移動',
  'Header consulted first for the visitor IP (behind CDN chains)':
    '訪問者IPの解決で最初に参照される Header（CDNチェーンの背後）',
  Hostname: 'Hostname',
  ...
  'JSONL access log file path (empty = disabled)': 'JSONLアクセスログのファイル Path（空 = 無効）',
  ...
  'Per-visitor-IP token bucket for proxied requests.':
    'Proxy リクエストに対する訪問者IPごとの Token バケット。',
  Prefix: 'プレフィックス',
  'Prometheus metrics endpoint at /aperio/metrics': '/aperio/metricsのPrometheusメトリクス Endpoint',
  Purpose: '用途',
  'Queue Status': 'キュー状態',
  ...
  'Trust X-Forwarded-For / X-Real-IP from a fronting reverse proxy':
    '前段リバース Proxy のX-Forwarded-For / X-Real-IPを信頼',
  'Trusted proxy/CDN egress IPs or CIDRs used to resolve the real visitor IP':
    '実際の訪問者IPの解決に使う信頼済み Proxy/CDNの出口IPまたはCIDR',
  ...
  'user:password gate in front of all proxied traffic; empty = disabled':
    '全 Proxy トラフィック前のuser:passwordゲート。空 = 無効',
  ...
  'Backlog at which the producing client is told to stop reading that stream’s source': '生成side のクライアントにそのストリームの取得停止を指示するバックログ量',
