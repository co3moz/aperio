...
export const ja: Record<string, string> = {
  ...
  'Allowed hostnames': '許可するホスト名',
  'Allowed hostnames (optional)': '許可するホスト名（任意）',
  'Automation using "{name}" will immediately lose access.': '「{name}」を使う自動化は直ちにアクセスを失います。',
  'Autoscaling': 'オートスケーリング',
  'Canary token (any use triggers a breach alert)': 'カナリアトークン（使用されると侵害アラートが発報）',
  'Client id': 'クライアント ID',
  ...
  'Fences every bind made inside the organization: its tokens and clients can only claim these hostnames. Leave empty for no restriction.': '組織内のすべてのバインドを制限します。その組織のトークンとクライアントはこれらのホスト名しか要求できません。制限しない場合は空欄にします。',
  'Hit ratio': 'ヒット率',
  'Hits / misses': 'ヒット / ミス',
  'Instances': 'インスタンス',
  'Issuer URL': '発行者 URL',
  ...
  'Only these hostnames may be bound by this organization. Empty = no restriction.': 'この組織がバインドできるのはこれらのホスト名だけです。空欄 = 制限なし。',
  ...
  'State': '状態',
  'Store size': 'ストアサイズ',
  'The call carries the client’s secret as a bearer token. The secret itself is never shown.': 'この呼び出しはクライアントのシークレットを bearer トークンとして送ります。シークレット自体は表示されません。',
  'The server never asks for more than {max} instances.': 'サーバーが要求するインスタンスは最大 {max} 個までです。',
  'The server stops calling this endpoint, so the service will no longer cold start or scale out. A client that is still running and still declares the block re-arms it on its next heartbeat.': 'サーバーはこのエンドポイントの呼び出しを停止するため、サービスはコールドスタートもスケールアウトもしなくなります。まだ稼働していてブロックを宣言し続けているクライアントは、次のハートビートで再登録します。',
  'This month: {req} requests, {bytes}, {clients} clients, {tokens} tokens, {users} users': '今月：{req} リクエスト、{bytes}、クライアント {clients}、トークン {tokens}、ユーザー {users}',
  'Too many consecutive failures: the endpoint is no longer called. Fix it and restart the client to re-arm.': '連続した失敗が多すぎます。このエンドポイントは呼び出されなくなりました。修正してクライアントを再起動すると再び有効になります。',
  'URL': 'URL',
  'Utilization': '使用率',
  'armed': '有効',
  'canary': 'カナリア',
  ...
  'Caching, resilience, and the headers this client rewrites.': 'キャッシュ、耐障害性、そしてこのクライアントが書き換えるヘッダー。',
  'Logging': 'ログ出力',
  ...
  'Proxy trust': 'プロキシの信頼',
  'How the real client IP is resolved behind a proxy or CDN.': 'プロキシや CDN の背後で、実際のクライアント IP をどう解決するか。',
  ...
  'Publishing the live hostnames to a reverse proxy in front of this server.': 'このサーバーの前段にあるリバースプロキシへ、稼働中のホスト名を公開します。',
  ...
  'Leave a field empty for no limit. The monthly allowance covers traffic proxied for this organization and resets each calendar month.': '空欄は無制限を意味します。月間の割り当てはこの組織向けにプロキシされたトラフィックが対象で、暦月ごとにリセットされます。',
  ...
  'Random-subdomain services answer with X-Robots-Tag: noindex and a disallow-all robots.txt': 'ランダムサブドメインのサービスは X-Robots-Tag: noindex とすべてを禁止する robots.txt を返します',
