...
async fn a_name_resolves_without_any_client_id() {
  ...
  insert(&state, "conn-abc", |c| {
    c.sole_mut().tunnels = vec![decl("pg_main", "tcp")]
  })
  .await;
...
async fn the_listing_folds_a_process_into_one_entry_per_name() {
  ...
  for cid in ["conn-0", "conn-1", "conn-2"] {
    insert(&state, cid, |c| {
      c.sole_mut().tunnels = vec![decl("pg_main", "tcp"), decl("dns", "udp")];
      c.instance_group = Some("proc-1".to_string());
    })
    .await;
  }
...
async fn the_listing_shows_only_what_the_caller_may_bind() {
  let state = Arc::new(test_state());
  insert(&state, "mine", |c| {
    c.sole_mut().tunnels = vec![decl("mine", "tcp")];
    c.perms = perms("owner", Some("org-a"), false);
  })
  .await;
  insert(&state, "theirs", |c| {
    c.sole_mut().tunnels = vec![decl("theirs", "tcp")];
...
async fn a_combined_tunnel_resolves_on_both_transports() {
  ...
  let state = Arc::new(test_state());
  insert(&state, "conn-abc", |c| {
    c.sole_mut().tunnels = vec![decl("dns", "tcp/udp")];
  })
  .await;
...
async fn the_listing_names_the_process_not_one_of_its_connections() {
  ...
  for (cid, index) in [("conn-a", "0"), ("conn-b", "1")] {
    insert(&state, cid, |c| {
      c.sole_mut().tunnels = vec![decl("dns", "udp")];
...
async fn the_listing_names_the_declaring_client() {
  ...
  let state = Arc::new(test_state());
  insert(&state, "conn-a", |c| {
    c.sole_mut().tunnels = vec![decl("postgres", "tcp")];
    c.instance_group = Some("group-1".to_string());
    c.sole_mut().service_name = Some("db".to_string());
    c.sole_mut().service_custom_name = Some("db (primary)".to_string());
  })
  .await;
