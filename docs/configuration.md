...
**The flat spelling still works.** Every `health_interval:` / `cache_max_bytes:` key is read exactly as before, so no existing file needs changing; the block wins per field when a file uses both. The client logs a one-line deprecation notice per flat key it finds, naming the block to move it to. Client blocks: `health` (also per `services:` entry) and the long-standing `server`. Server blocks: `alert`, `audit`, `cache`, `dashboard`, `edge`, `failover`, `gateway`, `ip_limit`, `login_lockout`, `metrics`, `oidc`, `otel`, `retention`, `scaling`, `server`, `stream`.
