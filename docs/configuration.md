...
| `--serve DIR` | Serve a local directory of static files instead of forwarding to a backend (mutually exclusive with a target; directories serve their `index.html`). One command to publish a `dist/` folder: `aperio-client --serve ./dist`. Also available per `services:` entry via `serve:`, so one client can serve several directories on different binds. Files stream from disk and single-range `Range` requests are answered `206`. Two environment-only options refine not-found handling: `APERIO_SERVE_SPA=1` (navigation falls back to the root `index.html` with status 200, for React/Vue routers) and `APERIO_SERVE_404=/path/to/404.html` (custom page, status 404). Full article: [Static File Serving](static-serving.md). |
