...
| `APERIO_STREAM_BACKLOG_LIMIT` | Hard per-stream backlog cap: a stream whose producer cannot be paused (a pre-v3 client, or one ignoring the pause) is dropped past this many buffered bytes. A value below the pause mark is repaired to twice it. | `16777216` (16 MB) |
