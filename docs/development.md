...
The same run also publishes the multi-arch (amd64+arm64) Docker images `ghcr.io/co3moz/aperio-server` and `ghcr.io/co3moz/aperio-client`, tagged with the version (`v0.2.0`, `0.2.0`, `0.2`, `0`); `latest` tracks the most recent **stable** release (a pre-release tag such as `v1.0.0-rc1` publishes only its exact tag). The images are assembled from the **same Linux musl binaries** the release build just produced — the Dockerfiles copy the prebuilt binary in rather than recompiling — so the Rust code is never built twice and there is no in-container cross-compilation to go wrong. (For that reason the Dockerfiles are runtime-only: to run a locally-built binary in a container, `cargo build` it first.) Docker images are not built on ordinary pushes — the CI workflow (`ci.yml`) only builds, lints, tests, audits, and runs the e2e tunnel test.
