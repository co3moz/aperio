...
`aperio_client_requests_total` is one series per **service**, not per connection. That is the same thing for every client that runs one service per connection, which is the ordinary shape; a client using [`multiplex: true`](configuration.md#one-connection-for-several-services) carries several services on one `client_id`, and those get an extra `service="<name>"` label to tell them apart. The label appears only when a connection carries more than one service, so a deployment that does not multiplex sees exactly the series it saw before and no recording rule or dashboard has its series split.
