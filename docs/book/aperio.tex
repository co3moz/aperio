...
\begin{lstlisting}[language=aperiosh]
# from any machine, with a token permitted to bind it
...
token whose client may claim it:
...
Revoking that token closes the port's source. The older shared-secret form
(\code{key:} on both sides) still works but names no owner.
