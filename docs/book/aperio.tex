...
\rowsep
 &  & \code{depends\_on} & Services this one waits for before opening its tunnel, by name. The wait is bounded: after 60 seconds it proceeds anyway and logs what it gave up on, because a dependency that never arrives must not keep a service that could be serving traffic off the air. yaml only, and on a multiplexed connection a dependency on one of its own members is dropped, since both open together. \\
\rowsep
