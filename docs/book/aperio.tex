...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item \textbf{Nothing gates the route.} What that means is
        ...
        served, which is the default, or refused.
...
\code{allow} is what the server has always done: with no \code{auth:}
...
The default stays \code{allow} for now, and while it does, a client
connecting with a service nothing gates logs a warning once, naming what
to write. The eventual flip should not be the first time anyone hears
about it.
...
\rowsep
\code{APERIO\_DEFAULT\_ACCESS} & What a route nobody gated means: \code{allow}, which is what the server has always done, or \code{deny}, where a route is served because something declared it reachable rather than because nothing declared otherwise. An undeclared route is then refused with the answer an unclaimed hostname already gives, so its existence does not leak. A gated route is untouched either way. An unreadable value refuses the start rather than being guessed at. & \code{allow} \\
\rowsep
