...
\textbf{\code{cache:} remembers a verdict} for an identical credential,
so a busy route does not pay a round trip per request. Only admissions
are remembered, never refusals: somebody just given access must not keep
being turned away for the rest of the window. The key is a hash of the
endpoint, the hostname and the credential headers sent, so no secret is
held in it. The URL goes through the outbound policy like every other
