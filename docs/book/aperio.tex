...
% One shape for every screenshot: full text width, a hairline frame so a dark
% capture does not bleed into the page, and a caption that says what the
% reader is looking at rather than repeating the section heading.
\newcommand{\screenshot}[2]{%
  \begin{figure}[htbp]
  \centering
  {\setlength{\fboxsep}{0pt}\fbox{\includegraphics[width=0.985\linewidth]{#1}}}
  \caption{#2}
  \end{figure}%
}
...
%
% A name like APERIO_MAX_CONNECTIONS_PER_SERVICE is wider than the column
% it sits in throughout the reference tables, and \texttt offers TeX no
% breakpoint at all, so the name simply runs into the neighbouring cell.
% Every literal is therefore allowed to break after an underscore. No
% hyphen is inserted: a hyphen inside an environment-variable name would
% read as part of the name, which is worse than the break it explains.
% Robust, because \code appears in section titles, which are written to
% the .aux file and read back.
\newcommand{\aperiobreakableunderscore}{\textunderscore\allowbreak}

% The other two separators an identifier is built from are the dot of a
% grouped yaml key (`security_headers.referrer_policy`) and the slash of a
% path, and unlike the underscore they arrive as ordinary characters,
% tokenized long before \code sees them. XeTeX, the engine tectonic runs
% and therefore the one the release builds the guide with, can still insert
% a token between two characters at typesetting time, which is the only
% remaining place to put a breakpoint. It is switched on inside \code and
% nowhere else, so a sentence's full stops, a URL in the prose and every
% code listing are set exactly as before.
%
% The dash is deliberately left out: it would have to carry a character
% class document-wide, which breaks the `--` and `---` ligatures everywhere
% to fix a handful of CLI flags, whose column is simply widened instead.
% Under pdflatex none of this exists and the tables keep the underscore
% breakpoints alone, which is why the columns are sized to survive without it.
\ifdefined\XeTeXcharclass
  \newXeTeXintercharclass\aperiosepclass
  \XeTeXcharclass`\. =\aperiosepclass
  \XeTeXcharclass`\/ =\aperiosepclass
  \XeTeXinterchartoks \aperiosepclass 0 = {\allowbreak}
  \XeTeXinterchartoks \aperiosepclass \aperiosepclass = {\allowbreak}
  \newcommand{\aperiocodebreaks}{\XeTeXinterchartokenstate=1 }
\else
  \newcommand{\aperiocodebreaks}{}
\fi
\DeclareRobustCommand{\code}[1]{{\ttfamily\small\aperiocodebreaks
  \let\_\aperiobreakableunderscore#1}}

% --- Reference-table columns ----------------------------------------
% Every column of every reference table is ragged right. A justified 3 cm
% column of code literals is what produced most of the overfull lines in
% this book: TeX has almost no stretch to work with there, so it either
% pulls a line apart or lets it run past the column edge.
\newcolumntype{R}[1]{>{\raggedright\arraybackslash}p{#1\linewidth}}

% A last resort for the few paragraphs that are mostly literals with no
% good breakpoint left, a run of event names or a sentence ending in a long
% key. TeX may stretch the interword space of such a paragraph's last
% resort pass rather than let a line run into the margin.
...
\author{\sffamily Version 0.11.0}
\date{}
...
\begin{enumerate}[leftmargin=1.5em]
  \item \textbf{Maintenance mode.} A flag covering the hostname answers
        \code{503} before anything else is looked at, which is why a
        route that is otherwise perfectly healthy can stop serving.
  \item \textbf{Static routes.} A \code{routes:} rule with a
        \code{redirect} or a \code{respond} answers here, with no client
        involved. A rule carrying neither is policy rather than an answer
        and does not end the request.
  \item \textbf{Per-IP rate limit.} A token bucket keyed on the visitor
        address. Exceeding it answers \code{429}.
  ...
  \item \textbf{Request firewall.} A \code{waf:} rule matching the
        method, path or a header answers \code{403}. It runs here rather
        than at the door because a rule may match on the body, which is
        not read until a slot is held.
  \item \textbf{Per-route rate limit.} The \code{rate\_limits:} rule
        covering this path, or a \code{rate\_limit} on the matching
        \code{routes:} entry, spends from its own bucket.
...
This is the list \emph{explain} walks (Chapter~\ref{ch:dashboard}): if
the two ever disagree, the endpoint is the one that was compiled.
...
\code{tcp\_target:}, \code{target\_health:}) was deprecated from 0.6.0
and removed in 0.9.0: a client that finds one refuses to start and names
the keys, rather than running while the file says something it is not
doing. Single-service mode itself is unaffected where it belongs, on the
command line and in the environment, and a top-level \code{health:} block
without an \code{endpoint:} is still the defaults every entry inherits.
...
\section{Installing as a service}

Every release attaches a \code{.deb} and an \code{.rpm} for both binaries, on
amd64 and arm64. They exist because the alternative, a tarball and a unit file
you write yourself, is where hardening quietly does not happen. Both packages
can be installed on the same machine; they share no files.

\begin{lstlisting}[language=aperiosh]
sudo dpkg -i aperio-server_0.11.0_amd64.deb  # or: rpm -i aperio-server-...rpm
sudoedit /etc/aperio/aperio-server.yaml      # set server.token
sudo systemctl enable --now aperio-server
\end{lstlisting}

Installing starts nothing. The shipped config carries a placeholder master
token, and a tunnel server that comes up on \code{apt install} with a token
published in a package is worse than one that does not come up at all.

On macOS and Windows the same release carries a Homebrew formula and a Scoop
manifest, rendered from the release's own checksums. Neither needs a tap or a
bucket to exist, since the file is itself a release asset:

\begin{lstlisting}[language=aperiosh]
brew install --formula \
  https://github.com/co3moz/aperio/releases/latest/download/aperio-client.rb
\end{lstlisting}

The client's unit is a \emph{template}, and the name after the \code{@} is the
config file it reads, because a machine usually fronts more than one thing and
...
Both units run as the \code{aperio} system account and are sandboxed the same
way: \code{NoNewPrivileges}, \code{ProtectSystem=strict}, \code{ProtectHome},
\code{PrivateTmp}, \code{PrivateDevices}, \code{ProtectProc=invisible}, the
\code{ProtectKernel*} set, \code{RestrictNamespaces},
\code{MemoryDenyWriteExecute}, \code{SystemCallFilter=@system-service}, and
\code{RestrictAddressFamilies} narrowed to IP (the client also gets unix
sockets, since it may be pointed at a backend over one). Read that list as the
set of things you would otherwise have to trust the process not to do.

One consequence is worth knowing before it surprises you: \code{ProtectSystem=%
strict} makes the filesystem read-only apart from what is named, so moving the
data directory takes two lines rather than one.
...
A named account is used rather than systemd's \code{DynamicUser}, which would
otherwise be the better answer. The config file holds a token, so it is not
world-readable, and a file that is not world-readable has to be readable by
someone with a name; a dynamic user has a different uid on every start, so no
mode on that file can admit it and nothing else.
...
Every release attaches the JSON Schemas for both config files, under a
stable name that always serves the newest release, so an editor with the
YAML extension gives completion and inline validation for every key in
this book without a checkout or a build:
...
\begin{codecard}[aperiojson]{.vscode/settings.json}
{
  "yaml.schemas": {
    "https://github.com/co3moz/aperio/releases/latest/download/aperio-client.schema.json":
      ["aperio.yaml", "**/aperio.yaml", "~/.aperio.yaml"],
    "https://github.com/co3moz/aperio/releases/latest/download/aperio-server.schema.json":
      ["aperio-server.yaml", "**/aperio-server.yaml"]
  }
}
\end{codecard}

Each release also carries a versioned copy of the same two documents,
carrying the release tag in the file name where these carry the word
\code{schema}. Point at those instead when the editor should validate
against the version a deployment actually runs rather than against the
newest one.

Building from source is the offline equivalent: \code{cargo build -p
aperio-client} writes both schemas into \code{schemas/}, and
\code{aperio-server --print-schema} emits the server's own from an
installed binary with no source tree. All three come from the same parser
types, so none of them can drift from the code that reads the file.
...
\begin{description}[leftmargin=1.6em,style=nextline]
  ...
  \item[A directory] \code{serve: ./dist} starts a built-in static file
        server, so there is no backend process at all. Files are
...
The three read as three different files. What follows is one complete
\code{aperio.yaml} per case, each the whole file rather than the
interesting line out of it, since the question this section answers is
usually "what do I actually write down".

\subsection{An application server}

The common case: something already listening on a port, and a name for it
to answer on. The health probe is what tells the server to stop sending
traffic here while the backend is restarting, so it is worth the one line
even for a single instance.

\begin{codecard}[aperioyaml]{./aperio.yaml (client), a web app on port 3000}
server:
  url: https://tunnel.example.com
  token: apr_xxxxxxxxxxxxxxxx
...
target: http://localhost:3000
hostname: app.example.com
target_health: /health        # polled locally; failing means "do not route here"
connections: 4                # parallel tunnel connections, no head-of-line stalls
max_concurrent: 32            # what this backend can take at once
\end{codecard}

A gRPC or HTTP/2 backend is the same file with \code{h2c://localhost:50051}
as the target, and a backend on a Unix socket is
\code{unix:///var/run/app.sock}. Neither changes anything else here.

\subsection{A built static site}

No backend process at all: the client is the web server. This is the
shape a documentation build, a landing page, or a preview of a front-end
branch wants, and it removes an nginx from the picture.

\begin{codecard}[aperioyaml]{./aperio.yaml (client), ./dist served directly}
server:
  url: https://tunnel.example.com
  token: apr_xxxxxxxxxxxxxxxx
...
cache: true                   # let the server cache what the headers allow
security_headers: true        # HSTS, nosniff, frame-options, referrer-policy
\end{codecard}

Files stream from disk rather than being read into memory, and \code{Range}
requests are answered, so a video or a large download behaves. A
single-page app needs one more line, \code{serve\_spa: true}
(\code{APERIO\_SERVE\_SPA=1}), which makes a navigation that matches no
file fall back to the root \code{index.html} instead of \code{404}ing,
the behavior a client-side router depends on. It is process-wide rather
than per-entry, which is why it is not in the file above.

\subsection{A private port, reachable but not published}

A database or an SSH daemon that should be reachable by a colleague, and
by nobody on the web. Written as \code{tunnels:} rather than
\code{target:}, these are never routed from a hostname: there is no URL
that reaches them, only another client asking for them by name with a
token permitted to bind them.

\begin{codecard}[aperioyaml]{./aperio.yaml (client), next to the private service}
server:
  url: https://tunnel.example.com
  token: apr_xxxxxxxxxxxxxxxx
...
tunnels:
  - name: pg_main
    ...
    encrypt: true
    psk: a-long-random-string   # end-to-end; never sent to the server
  - name: ssh_box
    target: 127.0.0.1:22
    protocol: tcp
\end{codecard}

\begin{codecard}[aperiosh]{From the other machine}
aperio-client --bind-tunnels pg_main \
  --server-url https://tunnel.example.com \
  --server-token apr_yyyyyyyyyyyyyyyy
\end{codecard}

The three combine in one file when a machine hosts more than one of them:
\code{services:} takes a list of the first two kinds (Section~\ref{sec:several-services}),
and \code{tunnels:} sits beside it.
...
\section{Pinning the TLS floor}

The tunnel is an outbound WebSocket, and over \code{wss://} its TLS is
rustls' defaults: TLS 1.2 and 1.3, with rustls' own suite order. That is the
right default, and it is an awkward answer to a questionnaire that asks for
the floor \emph{in writing}, because ``we use the library's defaults'' is a
statement about a dependency rather than about this deployment.

Two client keys make it a statement about the deployment:
...
tls_min_version: "1.3"
# Rarely needed. Unset means rustls' own preference order, which is
# almost always the better choice.
# tls_cipher_suites: TLS13_AES_256_GCM_SHA384,TLS13_CHACHA20_POLY1305_SHA256
\end{lstlisting}

Neither changes what a correct peer negotiates; they change what is offered,
and what is written down.

Two things about how they fail are deliberate. A value the client cannot
offer, a misspelled \code{1.4}, a cipher suite name this build does not have,
or a floor of 1.3 with only 1.2 suites named, is \textbf{refused at startup}
rather than ignored: an operator who pins a floor wants a stricter dial than
they had, and quietly giving them the default is how a control comes to exist
only on paper. And both are fixed for the life of the process, like
\code{ip\_family}: a hot reload will not change the terms under which the
current connection was made.
...
Beyond scalar settings the file carries seven structured sections. They
are lists of mappings, so they have no flat environment spelling, which is
the main reason to use the file at all.
...
\label{sec:alert-rules}

Two threshold rules are built in (error rate, client down). Anything else is
written as a rule over a quantity the server already measures:
...
alert_rules:
  - name: disk-filling
    ...
    for: 300                  # ... and has been for five minutes
  - name: no-clients
...
Four quantities can be watched. Each is something the alert loop already
reads on its own 15-second tick, so a rule costs no new measurement and no
new background task:

\begin{longtable}{@{}R{0.26}R{0.42}R{0.24}@{}}
\toprule
\textbf{\code{metric:}} & \textbf{What it reads} & \textbf{Unit, and what it catches} \\
\midrule
\endfirsthead
\toprule
\textbf{\code{metric:}} & \textbf{What it reads} & \textbf{Unit, and what it catches} \\
\midrule
\endhead
\code{connected\_clients} \newline (alias \code{clients}) & Tunnel clients currently connected. & count. A fleet that walked off, with \code{below:}. \\
\rowsep
\code{pending\_requests} \newline (alias \code{pending}) & Proxied requests in flight right now. & count. A backend that stopped draining, before it becomes a timeout. \\
\rowsep
\code{store\_bytes} \newline (alias \code{disk\_bytes}) & On-disk size of \code{aperio.db} and its WAL/SHM sidecars. & bytes. The disk filling. Pairs with \code{APERIO\_DB\_MAX\_BYTES}. \\
\rowsep
\code{rss\_bytes} \newline (alias \code{memory\_bytes}) & Resident memory of the server process. \textbf{Linux only.} & bytes. Memory climbing across a week. Elsewhere the rule is reported unfirable at startup rather than sitting quiet. \\
\rowsep
\bottomrule
\end{longtable}

A rule is four keys, and it is deliberately not an expression language:

\begin{longtable}{@{}R{0.16}R{0.76}@{}}
\toprule
...
\midrule
\endhead
\code{name} & Required, and unique across the list. It becomes the event's \code{kind}, which is what a webhook receiver switches on. \\
\rowsep
\code{metric} & Required. One of the four above, by either spelling. \\
\rowsep
\code{above} / \code{below} & Exactly one of the two, and it is the bound. \code{above: 100} fires strictly above 100. \\
\rowsep
\code{for} & Seconds the condition must hold. Applies to firing \emph{and} to resolving, so a value resting on its threshold cannot alert every tick. Omitted means the next tick. \\
\rowsep
\bottomrule
\end{longtable}

Rules emit the same \code{alert\_triggered} / \code{alert\_resolved} events
as the built-in pair, with \code{kind} set to the rule's name, so they reach
the webhooks and the audit log already configured for those. A malformed
\code{alert\_rules:} section refuses startup and names the offending rule:
a rule an operator believes is armed, silently dropped, is worse than no
rule at all.
...
been disabled from the dashboard, and, where outlier ejection is turned
on, it has not been ejected. Everything else in this chapter operates on
...
Off unless \code{APERIO\_OUTLIER\_EJECTION} is set, and worth turning
on: with it, a client that crosses a failure threshold (buffered 5xx
responses, response timeouts, connection loss) is silently pulled from
rotation while its tunnel stays up, then allowed back after
\code{APERIO\_OUTLIER\_EJECT\_SECS}. It is fail-open per route, so the
last client serving a hostname is never ejected into a \code{504}. This
is the one routing behaviour invisible from the client's side, which is
why the topology view and the client detail both surface an
\code{ejected} flag.
...
The current protocol version is 9. Version 3 added flow control for
...
base64, and version 8 lets one connection describe its work as a list of
services rather than a single one.
...
\section{One gate, decided in one order}

A visitor reaching a proxied site meets at most one gate. Which one is
settled in a fixed order, and the order is worth reading once because
every later question in this chapter is "where does this fit in it".

\begin{enumerate}[leftmargin=1.5em]
  \item \textbf{A path that could escape its own scope.} A request whose
        path contains \code{..} is never treated as public and never
        unlocks a client's credentials, since a backend that resolves it
        would serve a sibling path. Such a request needs a full session.
  \item \textbf{The gate the serving clients declared}, if the token
        permits it and the server has not set
        \code{APERIO\_IGNORE\_CLIENT\_AUTH}. This is a whole
        \code{auth:} policy today, not only a password
        (Section~\ref{sec:visitor-auth}).
  \item \textbf{Declared open.} \code{public: true}, or its long spelling
        \code{method: none}, and only where the token allows it. This is
        the one sentence that opens a route under either posture, which
        is why it is asked before anything else about the server's own
        configuration.
  \item \textbf{Nothing gates the route.} What that means is
        \code{default\_access} (Section~\ref{sec:default-access}):
        refused, which is the default since 0.10.0, or served.
  \item \textbf{The server's own \code{auth:}}, evaluated method by
        method until one admits: a session, a credential presented on the
        request itself, or an endpoint asked about it.
  \item \textbf{A share link}, which is checked last and can still admit
        a visitor a gate has just refused.
\end{enumerate}

\textbf{Unanimity decides steps 2 and 3.} A route is public only when
\emph{every} client that could serve it says so, and a client-declared
gate applies only when they all declare the same one; a mixed pool falls
back to the server's. So a request can never be gated by, or slip past,
something only some members of its pool set.

A dashboard session also carries its holder past the gate, since the two
planes share one session store, and that is fenced by organization: a
session fixed to a tenant admits only on hostnames that tenant serves,
by the same rule maintenance flags and share links use, covered by the
fence \emph{and} not currently served by another organization's client.
A master session is unfenced here as everywhere else. The reverse does
not hold: a visitor session opens no part of Aperio
(Section~\ref{sec:two-planes}).

\section{How a gate is written}
\label{sec:visitor-auth}

\code{auth:} is one grammar on both sides of the tunnel: on the server it
is the default for every route, on a client's service it overrides that
default for the traffic routed there. Three spellings of the same thing,
and the first one is what every file written so far already says.

\begin{codecard}[aperioyaml]{Either file}
...
A list is \textbf{any-of}: the first method that admits the visitor
admits them. That is the shape "a browser signs in, a script presents a
key" needs, and it is why this is a list rather than a choice, since a
single choice cannot be widened into one later without changing every
line already written. Two methods exist today:

\begin{description}[leftmargin=1.6em,style=nextline]
  \item[\code{none}] Deliberately open. The long spelling of a client's
        \code{public: true}, and it needs the same token permission.
  \item[\code{basic}] A \code{user:password} login. \code{users:} takes
        one credential or a list, all alternatives on one gate.
  \item[\code{forward}] Ask an endpoint you run about each request:
        \code{2xx} admits it, anything else refuses it and the endpoint's
        own answer is what the visitor gets.
        \code{via: client} asks over the tunnel instead, so the endpoint
        can live on the client's own network, unreachable from the internet;
        a \code{forward} written on a client must say so.
  \item[\code{aperio}] This server's own sign-in: a dashboard session
        reaching the hostname's organisation, with no credential to invent.
        Alone, it admits the admin plane only, so the site's visitor
        password does not open it; beside \code{bearer}, a person signs in
        and a script presents a key. Both sides may write it.
  \item[\code{jwt}] A token the visitor already holds, verified against
        keys the issuer publishes (\code{jwks\_url:}) or a shared secret
        (\code{hmac\_secret:}). No round trip per request, which is the
        operational difference from a future \code{forward}. Server-side
        only, like \code{bearer}.
  \item[\code{bearer}] An opaque secret presented as
        \code{Authorization: Bearer <secret>}. \code{secret:} takes one
        or a list, so a key rotates by adding the new one before
        withdrawing the old.
\end{description}

\subsection{The shape of a refusal}

A browser navigation, a \code{GET} carrying \code{Accept: text/html}, is
sent to the login page as it always was. Anything else, where the gate
has a \code{bearer} method, is answered \code{401} with
\code{WWW-Authenticate}. Redirecting a script to an HTML form answers a
question it did not ask, and it is the reason a gated route could not be
reached with \code{curl} at all: the gate read the session cookie and
nothing else, so a caller with no browser had no way in.
...
\code{query: true} additionally accepts the secret as
\code{?aperio\_token=}, for the callers that cannot set a header: an
\code{<img src>}, a link in an email, a sender with a fixed request
shape. It is \textbf{off by default}, because a query string reaches the
\code{Referer} header, browser history and every proxy in front. What
Aperio itself records stays clean: the access log has only ever kept the
path, the inspector masks the value, the parameter is stripped before the
request reaches the backend, and a \emph{page load} carrying one is
answered with a redirect to the clean address plus a short-lived cookie,
so the secret is not repeated on every asset of that page. The last move
is what a share link already does on its first click, and it reuses the
same cookie rather than inventing a second one.

\subsection{Asking an endpoint you run}
...
auth:
  - method: forward
    ...
    cache: 30                                  # seconds; 0 asks every time
\end{codecard}

Five decisions, each deliberate. \textbf{What crosses to the endpoint} is
a \code{GET} describing the request rather than replaying it: the
\code{X-Forwarded-*} names that pattern already uses, plus the request
headers you name, defaulting to \code{cookie} and \code{authorization}
rather than everything the visitor sent, since handing over the whole
request makes every header it happens to read part of a contract nobody
wrote down. \textbf{What crosses back} is only the response headers you
name, empty by default: that list is how the pattern delivers an identity
onto the request reaching your backend, and an open one is how it becomes
a header injection. A visitor's own copy of a name on that list is
dropped, always, whether or not the endpoint answered with one: you named
those headers so the backend could trust them, and an inbound
\code{X-Auth-User: admin} would otherwise arrive beside the endpoint's
answer, where most frameworks read the first of the two.

\textbf{A timeout refuses.} A gate that opens when its check is
unreachable is not a gate, so the endpoint's availability becomes the
route's; that is the trade, stated rather than discovered. \textbf{A
refusal is the endpoint's own answer}, relayed with its status,
\code{Location}, \code{WWW-Authenticate}, \code{Content-Type} and
\code{Set-Cookie}, so it can send a browser to a login of its own.
Redirects from it are deliberately not followed: a \code{302} is an
answer for the visitor, not a request for Aperio to make.

\textbf{\code{cache:} remembers a verdict} for an identical
\emph{request}, so a busy route does not pay a round trip per page load.
Only admissions are remembered, never refusals: somebody just given
access must not keep being turned away for the rest of the window. The
key is a hash of everything the subrequest carried, the endpoint, the
hostname, the method, the path, the visitor's address and the credential
headers, so no secret is held in it and a \emph{yes} for one path, or for
one address, is never a \emph{yes} for another. The endpoint is told the method and the path and may answer on
them, which is the ordinary way this pattern is used, so a cache keyed on
the credential alone would quietly turn a per-request authorization into
a per-session one. The URL goes through the outbound policy like every other
destination the server is told to call.

\subsection{Verifying a token the visitor already holds}
...
auth:
  - method: jwt
    ...
    cookie: CF_Authorization         # where the token is, if not a bearer
\end{codecard}

One \code{jwt} method subsumes a shelf of provider integrations, since
Cloudflare Access, an ALB doing OIDC and anybody's own auth service all
hand out the same thing. That is the argument for it over anything
vendor-shaped, and it is why the method set can stay closed.

Two ways of knowing who signed a token, and exactly one of them per
method. \code{jwks\_url:} is the issuer's public keys, fetched and cached
for an hour and fetched again when a token names a key id the cache does
not have, which is what a rotation looks like from here, so a rotation
needs no restart. \code{hmac\_secret:} is for \code{HS256}, where the
issuer is your own service. The key-set URL goes through the server's
outbound policy like every other destination the server is told to call.

\code{iss} and \code{aud} are checked only when the file asks, and asking
means the claim must be \emph{present}. An unset \code{audience:} is not
"accept any audience", and a configured one refuses a token carrying
none, which is exactly the token the requirement was written to keep out.
\code{exp} is required whatever the file says: a token with no expiry is
one that never stops working.

A token in the \code{Authorization} header is Aperio's credential and is
stripped before the request is forwarded. One in a \code{cookie:} is the
visitor's own and travels, since stripping the cookie header would take
the application's own session with it.

A \code{bearer} secret is compared verbatim rather than hashed, unlike a
\code{basic} password. That is deliberate: it is a high-entropy value the
operator generated, so there is no dictionary to defend against and no
user half to slow a guess down. It is also why one shorter than sixteen
characters is refused where it is written, since its length is the whole
of its strength.

The set is closed on purpose rather than being an extension point, and
\code{forward} is what lets it stay closed: anything deliberately left
out, LDAP, SAML, a rule nobody anticipated, is thirty lines behind that
URL, in a process that is not ours, with a contract that is two HTTP
messages rather than an ABI. A \code{method:} this build does not know
\textbf{refuses the start},
naming the ones it does. An operator who wrote a gate should never be
given no gate, which is exactly what accepting an unknown method and
ignoring it would produce.

The scalar keeps working wherever it worked, and it is still what
\code{APERIO\_SERVER\_AUTH}, \code{APERIO\_VISITOR\_AUTH},
\code{--visitor-auth} and the dashboard's visitor-password field carry,
since each of those is a single value. A policy the scalar cannot express
shows as empty there, so \code{GET /aperio/api/settings} reports
\code{visitor\_auth\_methods} beside it and what is in force stays
readable. A client may declare \code{none}, \code{basic}, \code{bearer} and
\code{jwt}, and \code{forward} with \code{via: client}: its URL is then
asked over the tunnel, of the client that would serve the request, so a
client writing \code{localhost:7070} means its own localhost. Without
\code{via: client} the client refuses to start, since the server never
dials a URL a client chose.

Which of those actually travel is negotiated on the handshake rather
than assumed. The server announces the methods it accepts from a client
on the upgrade response, and a client whose \code{auth:} needs one that
is missing \textbf{does not serve that service}: it says which side is
too old and retries, rather than connecting under a gate the server was
never told about. A server too old to send the announcement sends
nothing, which reads as the two methods that always travelled. This is
the case rule 23 exists for: a server that silently ignored a policy it
did not understand would read the client as declaring no gate at all,
and the route would come up open. Only the one service stops, its
siblings are untouched.

\subsection{Telling the backend who came in}
...
greet anyone without building a second login beside Aperio's.
\code{visitor\_identity\_headers} adds two headers to the forwarded
request: \code{x-aperio-visitor-how}, which is \code{session},
\code{bearer} or \code{share}, and \code{x-aperio-visitor-id}, the email
or username behind a session where there is one. A \code{bearer} secret
identifies a caller rather than a person, so it carries no id.

The secret itself does not travel. An \code{Authorization} header that
opened Aperio's gate is stripped before the request is forwarded, on the
same rule that already strips the \code{aperio\_session} and
\code{aperio\_share} cookies while leaving every other cookie alone: a
credential addressed to the gate is not addressed to what is behind it,
and handing a backend a secret that opens every route the gate protects
is worse than useless to it. An \code{Authorization} that did \emph{not}
open the gate is the visitor's own and passes through untouched.

Off by default, because it is the same new trust surface as
\code{identity\_headers} and deserves the same deliberate adoption. A
route that is open or ungated identifies nobody and sends neither header,
since a value meaning "anonymous" is noise a backend has to learn to
ignore. The forgery half is already done: inbound \code{x-aperio-*}
headers are stripped from every proxied request whatever this is set to,
so a visitor cannot claim an identity and a backend that trusts these
need not check whether the announcement is even on.

\subsection{Two planes, one cookie}
\label{sec:two-planes}

A session is for one of two things: administering Aperio, or viewing a
site behind it. They share a store and a cookie, and what tells them
apart is the credential that created the session. The master token, a
named dashboard user, a passkey and OIDC create an \textbf{admin}
session: the dashboard, its API, and, fenced by organisation, the
visitor gate. \code{server.auth} creates a \textbf{visitor} session:
every proxied hostname, and nothing administrative. A client's own
\code{auth:} creates one too, on its hostname alone.

The distinction is worth stating because the scope of a session and the
plane it belongs to are different questions, and until they were asked
separately only the first was: a visitor session has no host scope
either, since the server's gate is server-wide, so "no scope" read as
"full access" and the password an operator hands to whoever should see
the site opened the dashboard. The visitor password is still server-wide,
so signing in on one hostname is not asked again on the next; what it
never opens is Aperio.

\subsection{Closed by default}
\label{sec:default-access}

\code{default\_access} says what a route \textbf{nobody gated} means.
\code{allow} is what the server did before 0.10.0: with no \code{auth:}
anywhere, every route serves everyone. \code{deny} inverts it, so a route
is served because something said so, an \code{auth:} policy that admits
the visitor or an explicit \code{method: none} / \code{public: true}, and
not because nothing said otherwise.

This is the setting that makes a throwaway \code{auth: DUMMY:DUMMY}
unnecessary. Writing one was the only way to say "not public" while
\code{public:} was an exemption from a gate that might not exist, which is
a configuration language admitting it cannot express \emph{closed}.

An undeclared route under \code{deny} is refused with the same answer an
unclaimed hostname gives, so the existence of something there never leaks
to a caller who was not going to be let in. A route that \emph{is} gated
is untouched: the posture decides what an unstated route means and
nothing else.

The default is \code{deny} from 0.10.0. Before that it was \code{allow},
and a client connecting with a service nothing gated was warned once per
connection, naming the line to write, so the flip was not the first anyone
heard of it. Setting \code{allow} restores the previous behaviour exactly
and is a supported posture, not a migration escape hatch.
...
A named user holds a list of \emph{grants}, a role per organisation. Most
hold one. An operator who administers several tenants is created from
master with a grant in each, switches between them from the picker, and
is what each grant says there, read from the record on every request. A
grant is bounded by the granter's: giving a role in an organisation takes
Admin there, and giving \code{*}, every organisation present and future,
takes \code{*} at Admin, which the built-in account holds. Admin in the
master organisation is what manages organisations and reaches the global
surfaces; it does not by itself reach the children.
...
\begin{itemize}[leftmargin=1.3em]
  ...
  \item \textbf{maintenance flags and share links} need the fence
        \emph{and} a second condition: the hostname must not be served
        right now by a client of another organisation. Coverage alone is
        not enough, because the master token is never fenced, so a master
        client can be serving a name inside an organisation's fence, and
        coverage by itself would let that organisation 503 it or hand out
        a link past its visitor gate. A name nobody serves stays
        actionable, which is what keeps a fence usable before the first
        client connects: putting up a maintenance page is most wanted
        precisely when nothing is up.
\end{itemize}
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item Back the database up. \code{aperio-client api export} produces a
        ...
        (\code{statistics}, \code{uptime}, \code{activity}, \code{inbox},
...
\code{aperio} with the master token, as a named user, with a passkey, or
through OIDC.

\screenshot{dashboard-overview.png}{Overview: the live request rate, the
counters, and the connected clients. The chart's four ranges are the same
data at four resolutions.}

\section{What each screen is for}

\begin{description}[leftmargin=1.6em,style=nextline]
  \item[Overview] Live request rate, totals, connected clients. The
        activity chart carries four ranges. \emph{Live} is derived in the
        browser from successive polls, so it moves with the traffic and
        starts empty on a reload; the other three come from the server's
        own rings and survive one, at five-second slices over a quarter
        hour, minute slices over two hours, and quarter-hour slices over a
        day. The bars are drawn to the width of their slice, so a coarse
        range does not pretend to a resolution it does not have.
  ...
  \item[Tunnels] The private services clients declare for
        \code{--bind-tunnels}, each with the client that declared it and
        a ready \code{bind-tunnels:} block to copy.
  ...
  \item[Breakdown] Traffic by token and hostname, with per-route trends
        and the per-stage latency table.
  \item[Share Links] The temporary visitor access of
        Chapter~\ref{ch:visitor-auth}, minted and listed here.
  \item[Topology] The routing map: clients, routes that exist without a
        ...
        why they did not. Clients read \code{<org>@<name>}, the address
        form used everywhere else, and connections answering to one name
        are counted rather than repeated. A client belonging to another
        organisation is counted without being named: the fence is on the
        hostname, and being allowed to ask about a name is not being
        allowed to learn whose deployment answers it. Every step is
        reported twice, as an English sentence and as a message code with
        its values, so a dashboard renders it in the reader's language
        and a script keeps reading the sentence.
...
\screenshot{dashboard-clients.png}{Clients: every connection with its binds,
health, version and flags. The kill switch and the bind overrule are here,
and the uptime panel underneath tracks availability per service.}

\screenshot{dashboard-tunnels.png}{Tunnels: the private services clients
declare for \code{--bind-tunnels}, each named \code{<org>@<name>} with the
client that declared it, and a copy button that hands over a ready
\code{bind-tunnels:} block.}

\screenshot{dashboard-traffic.png}{Live Traffic: requests as they complete,
over a server-sent event stream rather than a poll. The latency percentiles
and the status mix sit on the same feed.}

\screenshot{dashboard-breakdown.png}{Breakdown: traffic by token and by
hostname, with the per-route trend bars and the per-stage latency table that
makes a regression attributable to one hop.}

\screenshot{dashboard-topology.png}{Topology: routes, clients and backends as
a live map, including the routing that exists with no client at all, so a
service that should be up and is not is visible at a glance.}

\screenshot{dashboard-explain.png}{Explain, on the same page: the decision
chain for a request nobody sent, with the stage that would answer marked. It
spends no rate limit and wakes nothing.}

\screenshot{dashboard-tokens.png}{Tokens: the scoped credentials handed to
clients, each with its hostname and path permissions, its limits, and its
expiry.}

\screenshot{dashboard-maintenance.png}{Maintenance: the per-hostname 503
switch, with the reason the visitor sees and the window after which it lifts
by itself.}

\screenshot{dashboard-scaling.png}{Autoscaling: the records clients have
armed, with the live utilisation of each pool measured against what the
clients announced they can take.}
...
and the webhook inbox (inbound POSTs captured for services that opted in,
browsable and re-firable): things you open, change, and leave. \textbf{Tools}
...
\screenshot{dashboard-settings.png}{The settings dialog: server settings,
organisations, users and webhooks, opened over the page underneath rather
than replacing it. Everything on it applies to the running server without a
restart.}

\section{The request inspector}

Any captured request can be opened to see headers, body, the response,
which client served it, and a per-stage timing breakdown. The client is
named the way the clients table names it, with the connection id a hover
away: the id is what an action addresses, and the name is what a person
recognises. From there the request can be replayed through
...
\screenshot{dashboard-inspector.png}{The request inspector: headers, bodies,
which client served it, and the microsecond timeline as a waterfall. Replay,
cURL and HAR are one click from here, all three redacted.}
...
\section{What a client reports about itself}

A heartbeat carries more than "still here". Each one announces the round-trip
time of that tunnel connection as the client measures it, the jitter between
consecutive round trips, how many times the connection has been
re-established, and the client process's CPU and resident memory. The
dashboard shows them on the clients table and \code{/aperio/api/stats}
carries them.

The round trip is the one that earns its place: from the server's side a slow
backend and a slow tunnel are indistinguishable, and this separates them. The
two process figures are read from \code{/proc}, so they are Linux only and
absent elsewhere rather than approximated, and inside a container they
describe the process rather than the cgroup. A client that reports nothing
shows nothing: the absence is stored rather than the last value kept, which
would age silently while looking current.
...
Three reasons share one state machine, and the endpoint is told which:

\begin{description}[leftmargin=1.6em,style=nextline]
  \item[\code{cold\_start}, 0 to 1] A request arrives for a hostname
        nothing is serving. Instead of \code{504}, the endpoint is called
        and the request is \emph{held} until an instance is routable.
  \item[\code{scale\_out}, N to N+1] The pool stays saturated for a full
        window and one more instance is requested.
  \item[\code{scale\_in}, N to N-1] The pool stays idle for several
        windows and one instance is given back, never below \code{min}
        and never below one.
\end{description}

So the server asks for the last instance neither way: 1 to 0 stays the
client's own decision, through \code{idle\_timeout}, because the client
knows about requests in flight that the server cannot see. And it only
ever \emph{asks}. The server never kills anything, which removes a whole
class of "why did my instance disappear" incidents.
...
\textbf{The same fence covers a key-set URL a client declares.} A
\code{jwt} visitor gate may name \code{jwks\_url}, and a client may
declare that gate, so the server fetches an address the client chose,
from the server's network, before any signature has been checked. That
is the scaling shape exactly, so it gets the scaling treatment:
\code{APERIO\_JWKS\_ALLOW\_HTTP} and
\code{APERIO\_JWKS\_ALLOW\_PRIVATE} relax it, and both default off. A
\code{jwt} gate written in the \emph{server's} own configuration is not
subject to it, because an operator naming an issuer on their own network
is describing their deployment rather than aiming the server at it.
...
Everything Aperio addresses by name carries a \textbf{handle}: \code{a-z},
\code{0-9} and \code{\_}, at most 64 characters. An organisation, a service
and a tunnel each have one. A handle is written in one file and read in
another, typed into a command line, and joined with other handles to form an
address
...
% The dotless i is set in the body font on purpose: ec-lmtt10 has no glyph
% for U+0131, so \code{} here would drop the very character being shown.
\code{i} that is actually a dotless \textit{\i}.
...
Anything a person should read goes in \code{custom\_name:}. It is free text,
any language, changeable at any time, because nothing addresses it. The handle
...
\code{bind-tunnels:} key, is how the dashboard shows a tunnel and the
client that declared it, and is how a client is named wherever the
server reports one, so one address form reads everywhere. An organization name may not contain
...
Narrowing the list reaches the clients already connected under it, not
only their next reconnect. A subscription is the one messaging
capability the server goes on holding \emph{for} a client between
requests: a bind is checked when it is declared and a publish when it is
made, so narrowing either is felt at the next attempt, while a
subscription, once admitted, keeps delivering on its own. An edit that
waited for a reconnect would therefore keep delivering a revoked topic
for as long as the process stayed up, which for a tunnel client is
measured in weeks. Every subscription the new list no longer covers is
withdrawn at once and reported to the client by name; the connection
itself is left alone, since nothing else about it changed.
...
\subsection{Who receives them}

Not everyone, and not across an organisation. The two questions this
raises are worth answering separately, because they have different
answers.

\textbf{Asking for them is a token permission}, the same one as for any
other topic: a subscription to \code{\$aperio/client/\#} is admitted only
when the token's own \code{topics} list covers it, and a token with no
\code{topics} cannot subscribe to anything at all. There is no separate
"may read infrastructure events" flag, and there is no wildcard that
reaches the namespace either: \code{\#}, the "all topics" grant, does not
cover \code{\$aperio/\#}, exactly as it does not match a \code{\$aperio/}
topic at delivery. It has to be written into the token on purpose,
\code{topics: [\#, \$aperio/client/\#]} being a token that may talk on
any topic of its own \emph{and} watch clients connect. A subscription
refused for this reason says so by name, rather than looking like a
message that never arrived.

\textbf{Receiving them does not cross an organisation.} The fan-out is
fenced on the subscriber's organisation against the organisation the
event belongs to, exactly as an ordinary message is, and the master
organisation is not a superset: a client of \code{acme} is told about
\code{acme}'s clients connecting, its tokens, its maintenance windows and
its hostnames, and never about another tenant's. Events that belong to
the server rather than to any one tenant, an alert firing, a setting
changed, a backup written, have no organisation and reach the un-fenced
clients only.

Within an organisation the payloads are the webhook payloads, so a
subscriber sees its org-mates' client ids, source addresses and
hostnames. That is the point of the feature, and it is also the reason
the \code{topics} grant is the thing to be deliberate about: it is what
decides who inside the organisation gets that view.
...
overrides win over both. These tables are maintained by hand alongside
\code{docs/configuration.md}; \code{aperio-server --print-schema} emits
the machine-readable catalogue, which is the one that cannot drift.

\section{Common settings}
\begin{longtable}{@{}R{0.28}R{0.49}R{0.17}@{}}
\toprule
...
\rowsep
\code{APERIO\_SERVER\_AUTH} & The default visitor gate in front of all proxied traffic, \code{user:password}. In yaml \code{server.auth} also takes a method block or a list of them (Section~\ref{sec:visitor-auth}). &  \\
\rowsep
...
\rowsep
\code{APERIO\_MAX\_TUNNELS} & Max simultaneously connected tunnel connections; a client holds one per service, and a reload briefly wants two. Was \code{10} before 0.12.0. & \code{64} \\
\rowsep
...
\rowsep
\code{APERIO\_VISITOR\_IDENTITY\_HEADERS} & \code{1} = tell the backend who the visitor is, as \code{x-aperio-visitor-how} (\code{session} / \code{bearer} / \code{share}) and \code{x-aperio-visitor-id} (the email or username behind a session, where there is one). Off by default, the same new trust surface as \code{APERIO\_IDENTITY\_HEADERS}. An open or ungated route identifies nobody and sends neither header. & \code{0} \\
\rowsep
\code{APERIO\_DEFAULT\_ACCESS} & What a route nobody gated means: \code{deny}, the default since 0.10.0, where a route is served because something declared it reachable rather than because nothing declared otherwise, or \code{allow}, what the server did before 0.10.0. An undeclared route is refused with the answer an unclaimed hostname already gives, so its existence does not leak. A gated route is untouched either way. An unreadable value refuses the start rather than being guessed at. & \code{deny} \\
\rowsep
...
\rowsep
\code{APERIO\_CLIENT\_DOWN\_THRESHOLD} & Seconds without a heartbeat before a client is dropped from the routing pool (it rejoins on the next ping). & \code{15}. A connection silent for four times this is closed and removed by the server itself, so a socket dropped without a Close frame does not keep its slot. \\
\rowsep
...
\rowsep
\code{APERIO\_SERVER\_SIDE\_TARGETS} & Comma-separated host/CIDR patterns a client may ask the server to reach itself (\code{server\_side: true} on a service) instead of relaying through the client. Same spellings as \code{outbound.allowlist}. \textbf{Unset permits nothing}, the opposite default, because this list opens a channel a tenant steers rather than a callback the server makes. A target outside it is refused at declaration; the token must also allow it. & off \\
\rowsep
\code{APERIO\_IDENTITY\_HEADERS} & \code{1} = tell the backend which client, organization and token served the request (\code{x-aperio-client-id}, \code{x-aperio-org}, \code{x-aperio-token}). Off by default: new trust surface should be adopted deliberately. Inbound \code{x-aperio-*} headers are stripped from every proxied request regardless, so a visitor cannot forge one and a backend that trusts them need not check whether the announcement is on. & \code{0} \\
\rowsep
...
\rowsep
\code{APERIO\_SECURE\_COOKIES} & \code{1} = set the \code{Secure} flag on session cookies, which also lets the session cookie carry the \code{\_\_Host-} prefix; with it on, only the prefixed cookie is read, so a tenant on a sibling hostname can neither displace an operator's session nor plant one. Defaults to the \code{APERIO\_TRUST\_PROXY} value. &  \\
\rowsep
...
\code{APERIO\_OUTBOUND\_PROXY} & HTTP proxy the server's outbound callbacks go through, on a network with no direct route out (yaml \code{outbound.proxy}). The ambient \code{HTTP\_PROXY} family is no longer read by anything. Through a proxy the destination's name is resolved by the proxy, so \code{block\_private} covers literal addresses only and CIDR allowlist entries cannot admit a named destination; the startup log says which half is in force. & direct \\
\code{APERIO\_OUTBOUND\_NO\_PROXY} & Destinations called directly instead of through the proxy (yaml \code{outbound.no\_proxy}): exact names, or \code{.suffix} / \code{*.suffix} for a domain and everything under it. Loopback is always direct. These keep the whole outbound policy, since the server chooses their addresses itself. & empty \\
\rowsep
...
\rowsep
\code{APERIO\_STREAM\_MIN\_THROUGHPUT} & Bytes per second a streamed response's consumer must take while data is waiting for it, or the stream is ended. Closes the gap the per-chunk stall timeout leaves: a reader that accepts one chunk just inside the timeout, forever. Only time the consumer kept data waiting counts, so a quiet backend never costs the consumer its stream. & \code{0} \\
\rowsep
\code{APERIO\_ALTERNATE\_SERVERS} & Comma-separated Aperio server URLs a client of this one may fall back to, announced on every handshake, so a planned migration does not mean editing every client's config. A client appends them after the servers its own config names, and the rotation wraps, so an alternate is never a one-way door. At most 8. &  \\
\rowsep
\code{APERIO\_MAX\_STREAMS\_PER\_IP} & Streamed responses one visitor address may hold open at once. Saturating a service's concurrency budget otherwise takes one host holding many slow streams; this makes it take a botnet. A concurrency limit, not a rate limit: the slot frees when the stream ends. Check \code{trust\_proxy} before setting it, or every visitor behind a CDN shares one address. & \code{0} \\
\rowsep
\code{APERIO\_OTEL\_BRIDGE} & \code{1} = accept OTLP exports from tunnel clients and forward them to the collector \code{otel.endpoint} names. Off by default: it is an outbound path a client can drive. & \code{0} \\
\rowsep
\code{APERIO\_OTEL\_HEADERS} & Extra headers on every outgoing OTLP request, as \code{k=v,k=v}. Where a collector's credential goes, and it stays on the server. &  \\
\rowsep
\code{APERIO\_SHUTDOWN\_DRAIN} & Seconds to let in-flight proxied requests finish before shutdown ends the connections carrying them. Behind a load balancer this decides whether a deploy is invisible or shows up as a handful of 502s. \code{auto} takes the longest drain budget connected clients announce, capped at 30s. & \code{0} \\
\rowsep
\code{APERIO\_SHUTDOWN\_TIMEOUT} & Seconds after which shutdown stops waiting for anything still holding a connection open and exits. & \code{10} \\
\rowsep
\code{APERIO\_ACCESS\_LOG\_SAMPLE\_RATE} & Fraction of \textbf{successful} requests that produce an access line, \code{0.0} to \code{1.0}. Thins out both the \code{aperio\_access} event and the \code{APERIO\_ACCESS\_LOG} file. Sampling is deterministic (\code{0.1} = exactly one line in ten). A response of \code{500} or worse is always logged whatever the rate, and so is every refused or failed request. The dashboard's counters, the latency histogram and the rate charts are fed before sampling and stay exact. & \code{1.0} \\
\rowsep
...
\code{APERIO\_BACKUP\_KEY} & Encryption key for the snapshots, 64 hex characters or base64 of 32 bytes. Unset writes them in the clear; set, each snapshot is \code{aperio-<epoch>.db.enc} under AES-256-GCM. Prefer the key file. A key that cannot be used disables backups rather than falling back to plaintext. & off \\
\code{APERIO\_BACKUP\_KEY\_FILE} & File holding that key. Refused when it is inside \code{APERIO\_BACKUP\_DIR}: whoever has the backups would have the key. Restore with \code{aperio-server --decrypt-backup}, which is the first step only: the store runs in WAL mode, so the restored file has to replace \code{aperio.db} with the server stopped and with no \code{-wal}/\code{-shm} left beside it. & off \\
\rowsep
...
\rowsep
\code{APERIO\_OTEL\_PROTOCOL} & OTLP transport: \code{http} (protobuf over HTTP, the spec spelling \code{http/protobuf} is also accepted) or \code{grpc}. Falls back to the standard \code{OTEL\_EXPORTER\_OTLP\_TRACES\_PROTOCOL} / \code{OTEL\_EXPORTER\_OTLP\_PROTOCOL}. Unset, the endpoint's port decides: 4317 is gRPC, anything else HTTP. A collector answering the other protocol drops every span in silence, so the startup probe checks the transport actually chosen and warns when the port contradicts it. & port-derived \\
\rowsep
...
\rowsep
\code{APERIO\_DASHBOARD\_HOSTNAME} & A hostname whose root is the dashboard: \code{panel.example.com} opens the panel the way \code{panel.example.com/aperio} does, and \code{/aperio} stays on every hostname. One exact name; it serves the panel and nothing else, so no bind may claim it. An organisation picks its own inside its fence. Yaml: \code{dashboard.hostname}. & unset \\
\rowsep
\code{APERIO\_DASHBOARD\_FENCED\_LOGIN} & \code{1} = fence the dashboard login to the hostname's organisation: on a hostname inside an organisation's allowlist only its people and anyone reaching master sign in, a hostname no fence claims admits master's people and the users of unfenced organisations, and a session is good only on the hostname it was minted on. Off by default. Yaml: \code{dashboard.fenced\_login}. & \code{0} \\
\rowsep
...
\rowsep
\code{APERIO\_OIDC\_DEFAULT\_GRANTS} & What an email with no dashboard user record is granted at its first login, comma-separated \code{<org>:<role>} entries. Empty means the login is refused until an administrator grants something; \code{master:admin} is what every allowed email got before 0.12.0. Yaml: \code{oidc.default\_grants}. & empty \\
\rowsep
\code{APERIO\_OIDC\_GROUPS\_CLAIM} & The claim that carries the provider's groups, read from userinfo and then from the ID token. Yaml: \code{oidc.groups\_claim}. & \code{groups} \\
\rowsep
\code{APERIO\_OIDC\_GROUP\_GRANTS} & What each group means, comma-separated \code{<group>=<org>:<role>} entries, applied at every login: produced grants are written, ones no longer produced are taken back, hand-written ones are left alone. Yaml: \code{oidc.group\_grants}. & empty \\
\rowsep
\bottomrule
\end{longtable}
...
\begin{longtable}{@{}R{0.21}R{0.18}R{0.15}R{0.36}@{}}
\toprule
...
\code{APERIO\_EGRESS\_PROXY} & \code{--egress-proxy} & \code{egress\_proxy} & HTTP proxy to dial the tunnel server through, on a network with no direct outbound connection: \code{host:port} or \code{http://host:port}, optionally with \code{user:password@} in front. The client sends \code{CONNECT} and runs TLS inside the tunnel the proxy opens, so the proxy sees the hostname and nothing else. The tunnel only: your backend is never reached through it. An \code{https://} proxy is refused rather than dialed in the clear, and a refused \code{CONNECT} names the proxy and its status. \\
\code{APERIO\_TLS\_MIN\_VERSION} & \code{--} & \code{tls\_min\_version} & Lowest TLS version offered when dialing the tunnel server over \code{wss://}: \code{1.2} or \code{1.3}. Unset leaves rustls' own set (1.2 and 1.3), which is the right default; pin it when a policy has to name the floor in writing. A value this client cannot offer is refused at startup, never silently ignored. Process-wide. \\
\code{APERIO\_TLS\_CIPHER\_SUITES} & \code{--} & \code{tls\_cipher\_suites} & Exact cipher suites offered when dialing the tunnel server, by their IANA names, comma-separated. Unset leaves rustls' preference order alone, which is almost always better. An unknown name, or a set that cannot meet the configured floor, is refused at startup. Process-wide. \\
\rowsep
...
\rowsep
\code{APERIO\_SERVE\_SPA} &  & \code{serve\_spa} & \code{1} = a navigation (\code{Accept: text/html}) that resolves to no file falls back to the root \code{index.html} with status \code{200}, the single-page-app deployment. A missing hashed asset still \code{404}s. \\
\rowsep
\code{APERIO\_SERVE\_404} &  & \code{serve\_404} & Path to an HTML file served (status \code{404}) for misses the SPA fallback does not cover. \\
\rowsep
...
\rowsep
\code{APERIO\_VISITOR\_AUTH} & \code{--visitor-auth} & \code{auth} & Gate this service behind a client-set visitor login, superseding the server's own \code{APERIO\_SERVER\_AUTH} for it (only the client's credentials work; master and dashboard passwords always do). A successful login is scoped to that hostname. Same token permission as \code{public}; ignored if the server sets \code{APERIO\_IGNORE\_CLIENT\_AUTH}. Per \code{services:} entry via \code{auth:}. In yaml this is the \code{user:password} scalar or a method block (Section~\ref{sec:visitor-auth}); the flag and the environment variable are scalars, so they always mean \code{basic}. \\
\rowsep
...
\rowsep
\code{APERIO\_OTEL\_BRIDGE\_LISTEN} &  & \code{otel\_bridge.listen} & Address for the client's OTLP/HTTP receiver, \code{127.0.0.1:4318} by default; anything on the edge host exports to it with one environment variable. \\
\rowsep
\code{APERIO\_OTEL\_BRIDGE\_LISTEN\_GRPC} &  & \code{otel\_bridge.listen\_grpc} & Address for the OTLP/gRPC receiver. Unset, the default, means no gRPC listener. \\
\rowsep
\code{APERIO\_OTEL\_BRIDGE\_TRANSPORT} &  & \code{otel\_bridge.transport} & \code{tunnel}, the default, sends exports on the WebSocket the client already holds; \code{https} posts them to the server's endpoint. \\
\rowsep
\code{APERIO\_OTEL\_BRIDGE\_QUEUE} &  & \code{otel\_bridge.queue} & Exports held when the far end is not keeping up, \code{256} by default; past this the newest is dropped and counted, never waited on. \\
\rowsep
\code{APERIO\_STARTUP\_DELAY} &  & \code{startup\_delay} & Seconds a service waits before opening its tunnel, \code{0} by default, for a backend that starts alongside the client. Only the first connection of a parallel pool waits. \\
\rowsep
\code{APERIO\_DEPENDS\_ON} &  & \code{depends\_on} & Services this one waits for before opening its tunnel, comma-separated in the variable. At the top level it is the default for every \code{services:} entry that names none of its own, except the entries the list itself names, which are what is being waited for. The wait is bounded: after 60 seconds it proceeds anyway and logs what it gave up on, because a dependency that never arrives must not keep a service that could be serving traffic off the air. On a multiplexed connection a dependency on one of its own members is dropped, since both open together. \\
\rowsep
\code{APERIO\_PID\_FILE} &  & \code{pid\_file} & Path to write the process id to at startup, removed on a clean exit. Unset by default. A pid file that cannot be written is a warning, not a refusal to start. \\
\rowsep
\code{APERIO\_ADAPTIVE\_CONCURRENCY} &  & \code{adaptive\_concurrency} & Off by default. Moves the announced \code{max\_concurrent} with backend pressure: halves it while requests queue waiting for a local permit, climbs back one at a time when they stop. Needs \code{max\_concurrent} set, and never leaves the band \code{1..=max\_concurrent}. \\
\rowsep
\code{APERIO\_CONNECT\_TIMEOUT} &  & \code{connect\_timeout} & Seconds to wait for the TCP connection to a backend, separate from \code{timeout}, which covers the whole request. Per service. Unset = only the whole-request budget applies. \\
\rowsep
\code{APERIO\_MIN\_TLS\_VERSION} &  & \code{min\_tls\_version} & Lowest TLS version accepted from an \code{https://} backend, \code{1.2} or \code{1.3}. Per service. A value that is not a TLS version is refused at startup rather than ignored. \\
\rowsep
\code{APERIO\_METRICS\_LABELS} &  & \code{metrics\_labels} & Static Prometheus labels attached to this client's own \code{aperio\_client\_requests\_total} series. YAML takes a mapping (\code{\{env: prod\}}); the environment spelling is a flat \code{env=prod,region=eu-west} list. At most 8 labels, names must match \code{[a-zA-Z\_][a-zA-Z0-9\_]*}, and the names the server writes itself are refused. \\
\rowsep
\code{APERIO\_CONNECTIONS\_MIN} &  & \code{connections.min} & Floor of an elastic pool: \code{connections: \{min: 1, max: 8\}} opens one connection and grows towards eight while requests queue up. A plain number stays a fixed pool. \\
\rowsep
\code{APERIO\_CONNECTIONS\_MAX} &  & \code{connections.max} & Ceiling of an elastic pool. The server's \code{max\_connections\_per\_service} still wins over it, and the \code{bandwidth} share is divided by this value rather than by the pool's current size. \\
\rowsep
\code{APERIO\_MULTIPLEX} &  & \code{multiplex} & Off by default. Carries every service that sets it on one WebSocket instead of one each: a single socket, TLS session, reader, writer and heartbeat for the group rather than that many of each on both ends. Settable per \code{services:} entry, so a file can turn it on once and keep one service on a connection of its own with \code{multiplex: false}. Needs two services, a \code{name:} on each, and a server speaking tunnel protocol 8 or newer (0.10.0), which it announces on the handshake; against an older one the services are held back rather than half-served. \code{connections:} is not honored for a multiplexed service, and the client reports that in the dashboard's config view. \\
\rowsep
...
\rowsep
\code{APERIO\_NAME} & \code{--name} & \code{name} & What to call this client on screen and in the server's logs, e.g. \code{eu\_server\_1}. A label, not an address: \code{client\_id} stays the identity failover and \code{bind\_tunnels:} recognize this process by, because those need something unique and a name is not. \\
\rowsep
...
\rowsep
\code{APERIO\_SECURITY\_HEADERS} &  & \code{security\_headers} & The security response-header preset: HSTS, \code{X-Frame-Options: DENY}, \code{X-Content-Type-Options: nosniff} and \code{Referrer-Policy: strict-origin-when-cross-origin}. Explicit \code{headers:} rules always win over it. Off by default. \\
\rowsep
\code{APERIO\_SECURITY\_HEADERS\_HSTS} &  & \code{security\_headers.hsts} & Inject \code{Strict-Transport-Security} on its own. Only meaningful behind HTTPS. \\
\rowsep
\code{APERIO\_SECURITY\_HEADERS\_HSTS\_MAX\_AGE} &  & \code{security\_headers.hsts\_max\_age} & HSTS \code{max-age} in seconds. Default \code{63072000}, two years. \\
\rowsep
\code{APERIO\_SECURITY\_HEADERS\_FRAME\_OPTIONS} &  & \code{security\_headers.frame\_options} & \code{X-Frame-Options} value to inject. \\
\rowsep
\code{APERIO\_SECURITY\_HEADERS\_NOSNIFF} &  & \code{security\_headers.nosniff} & Inject \code{X-Content-Type-Options: nosniff}. \\
\rowsep
\code{APERIO\_SECURITY\_HEADERS\_REFERRER\_POLICY} &  & \code{security\_headers.referrer\_policy} & \code{Referrer-Policy} value to inject. \\
\rowsep
\code{APERIO\_SECURITY\_HEADERS\_CSP} &  & \code{security\_headers.csp} & \code{Content-Security-Policy} value to inject. No default: a policy is application-specific, and a wrong one breaks the page. \\
\rowsep
\code{APERIO\_SCALING\_URL} &  & \code{scaling.url} & Endpoint the server POSTs to when a service of this client needs capacity. HTTPS and public addresses only, unless the operator opened those (see \emph{Autoscaling}). Without it there is nothing to call, so the declaration is off. \\
\rowsep
\code{APERIO\_SCALING\_SECRET} &  & \code{scaling.secret} & Sent as \code{Authorization: Bearer} on that call. Write-only: never echoed back, never logged. \\
\rowsep
\code{APERIO\_SCALING\_MIN} &  & \code{scaling.min} & Instances that should always be running. \code{0}, the default, opts into scale-to-zero: a request for an unserved hostname cold-starts instead of answering 504. \\
\rowsep
\code{APERIO\_SCALING\_MAX} &  & \code{scaling.max} & Ceiling the server will never ask to exceed. \code{0}, the default, means cold starts only, no scale-out. \\
\rowsep
\code{APERIO\_SCALING\_COLD\_START} &  & \code{scaling.cold\_start} & How long a visitor request may be held while a cold start completes, e.g. \code{45s}, which is the default. \code{0} answers immediately instead of holding. \\
\rowsep
\code{APERIO\_SCALING\_TARGET\_UTILIZATION} &  & \code{scaling.target\_utilization} & Pool utilisation above which the server asks for one more instance, between 0 and 1. Default \code{0.8}. \\
\rowsep
\code{APERIO\_SCALING\_WINDOW} &  & \code{scaling.window} & How long utilisation must stay above the target before scaling out, e.g. \code{15s}, the default. Guards against reacting to a single spike. \\
\rowsep
\code{APERIO\_SCALING\_COOLDOWN} &  & \code{scaling.cooldown} & Minimum gap between two calls for one bind, default \code{60s}. A new instance needs time to appear; without this the server would ask again while it is still starting. \\
\rowsep
...
\begin{longtable}{@{}R{0.34}R{0.42}R{0.18}@{}}
\toprule
...
\rowsep
\code{GET /aperio/api/activity} & Request volume per bucket over the requested span (\code{range=15m}, \code{2h} or \code{1d}), for the activity chart's long views. & dashboard session \\
...
\rowsep
\code{GET /aperio/api/tunnels} & Lists the tunnels declared by the connected clients of the caller's organization (name, target, client, token, protocol, availability), the dashboard's Tunnels view. & master token (Bearer) or dashboard session \\
\rowsep
...
\rowsep
\code{GET /aperio/health} & Liveness probe. Without a credential: \code{status} and the default \code{ui\_language}. With the master token, a tunnel token, a dashboard session or an admin key: also the server version, protocol version, connected client count, uptime and request total. & none for liveness; a credential for the numbers \\
\rowsep
\code{GET /aperio/healthz} & Liveness probe for a container runtime: \code{200} with an empty body, no locks taken. For a Docker \code{HEALTHCHECK} or a Kubernetes \code{livenessProbe}. & none \\
\rowsep
\code{GET /aperio/readyz} & Readiness probe: \code{200} while the server should receive traffic, \code{503} from the moment a shutdown signal arrives. Pair with \code{APERIO\_SHUTDOWN\_DRAIN}. Never wire it to a \code{livenessProbe}. & none \\
\rowsep
...
\rowsep
\code{GET /aperio/api/export} & Logical JSON dump, a failsafe for upgrades and migrations. \code{?include=} names the sections (\code{tokens}, \code{webhooks}, \code{users}, \code{organizations}, \code{scaling}, \code{settings\_overrides} by default, plus \code{statistics}, \code{uptime}, \code{activity}, \code{inbox}, \code{admin\_keys}). Without \code{organizations}, only master's rows travel. Sessions and the audit log are never included. & master super-admin \\
\rowsep
...
current protocol version is 9, negotiated per connection, and each step
...
sends more bytes for less CPU than it did before v7. v8 adds an
optional \code{services} list to the Ping: each entry carries what the
top-level per-service fields carry one at a time, and when the list is
present it is authoritative and those fields are ignored. A list of
several is served as several, each routed by its own binds, gated by
its own \code{auth:}, ejected on its own backend failures and shown
separately in the dashboard, which is what \code{multiplex: true} on
the client produces; identity across heartbeats is by the name the
client gives a service, so a multiplexed one needs a \code{name:}. v9 lets a service entry carry \code{server\_side\_target},
asking the server to reach the target itself, and a Ping carry the client's
\code{name}. Messages an older peer can ignore are added without a bump:
\code{HostnameAssigned}, \code{Draining} and \code{ServerShutdown},
the compression offer, \code{OtlpExport}, and the \code{AuthAsk} /
\code{AuthVerdict} pair that carries a \code{forward} gate asked over
the tunnel. An
empty list is refused, since a client that serves nothing is a
disconnect written the long way. Whether a list may be sent at all is
negotiated rather than assumed: the server announces its protocol
version in an \code{x-aperio-protocol} header on the upgrade response,
which is the only point early enough, and a client asked to multiplex
against a server below v8 holds those services back and says which
side has to move, rather than letting the server read the singular
fields and quietly serve the first of them.

\begin{longtable}{@{}R{0.24}R{0.12}R{0.58}@{}}
\toprule
...
\code{token\_rotated}, \code{token\_expiring}, \code{token\_new\_ip},
\code{token\_pin\_mismatch}, \code{canary\_tripped},
\code{tunnel\_created}, \code{tunnel\_deleted},
\code{share\_created}, \code{maintenance\_on}, \code{maintenance\_off},
\code{settings\_updated}, \code{import\_applied}, \code{user\_created},
\code{alert\_triggered}, \code{alert\_resolved},
\code{scaling\_requested}, \code{org\_usage},
\code{disk\_usage\_warning}, \code{db\_backup}, \code{disk\_pruned}.
Subscribe to \code{*} for all of them.

\section{Audit events}

Everything above, plus the administrative actions that have no webhook:
\code{token\_updated}, \code{token\_refreshed},
...
\begin{description}[leftmargin=0pt,style=nextline]
  ...
  \item[Effective organisation] The organisation whose resources a caller
        sees: the one selected on the session, out of those the caller's
        grants reach. A user granted one organisation is simply in it.
