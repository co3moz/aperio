...
A dashboard session carries its holder past the gate as well, since the
two share one session store, and that is fenced by organization: a
session fixed to a tenant admits only on hostnames that tenant serves,
by the same rule maintenance flags and share links already use, covered
by the fence \emph{and} not currently served by another organization's
client. A master session is unfenced here as everywhere else.
