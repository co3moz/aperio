...
%      tectonic aperio.tex
%
%  Only standard packages are used, so a plain TeX Live or MacTeX
%  installation is enough; nothing is downloaded at build time beyond
%  what those distributions already ship.
%
...
% --- Colours -------------------------------------------------------
\definecolor{AperioGreen}{HTML}{4C9A2A}
...
\lstdefinelanguage{aperiosh}{
  morecomment=[l]{\#},
  morestring=[b]",
  morekeywords={curl,docker,aperio-client,aperio-server,export,printf,jq},
  sensitive=true,
}
...
\newcommand{\yamlfile}[1]{\lstset{language=aperioyaml}}
...
% --- Admonitions ----------------------------------------------------
% Deliberately built from plain primitives rather than tcolorbox, so the
% book compiles on a minimal TeX installation.
\newenvironment{aperionote}[1]
  {\par\medskip\noindent\begin{minipage}{\linewidth}%
   \color{AperioGreen}\rule{\linewidth}{0.6pt}\par\smallskip
   \color{black}\textbf{#1.}\enspace\ignorespaces}
  {\par\smallskip\color{AperioGreen}\rule{\linewidth}{0.6pt}%
   \color{black}\end{minipage}\par\medskip}

\newcommand{\aperiokey}[1]{\subsubsection*{\code{#1}}}

\title{\vspace{-1.5cm}\Huge\textbf{Aperio}\\[0.4cm]
       \LARGE The Complete Guide\\[0.8cm]
       \large Reverse tunnelling, routing, and operations}
\author{Version 0.5.0}
\date{}
...
there. Dashboard overrides win over both at runtime. The client adds a
fourth surface, CLI arguments, which outrank everything.
\end{aperionote}
...
[ Visitor ] --------------> [ aperio-server ] <========================= [ aperio-client ] --> [ backend ]
                                   |                                                            :3000
                             dashboard, API,
                             cache, routing
\end{lstlisting}
...
# Optional, but worth setting early:
max_concurrent: 8          # what one instance of the backend can take
target_health: /health     # hold the service out of routing until it answers
\end{lstlisting}
...
cache, failover, rate limits, lockout, body and concurrency limits, audit
rotation, \code{require\_hostname\_bind}, tunnel compression, UI
language, preview noindex, visitor auth, plus the structured
\code{headers:}, \code{routes:} and \code{error\_pages:} sections.
...
random-subdomain pattern, the custom error-page file paths, and
\code{expose:} ports. Reloads are audit-logged as \code{config\_reloaded}.
...
\begin{description}[leftmargin=1.6em,style=nextline]
  ...
  \item[A directory] \code{serve: ./dist} starts a built-in static file
        ...
        root index, which is what a client-side router needs.
...
trim_bind: true               # strip /api before forwarding (default when a path bind is set)
\end{lstlisting}

\section{Several services from one process}

Replace \code{target} with \code{services:} and the client opens one
tunnel connection per entry, each with its own binds, health probe and
limits:
...
\begin{itemize}[leftmargin=1.3em]
  ...
  \item Raw TCP and UDP relays for emergency tunnels.
\end{itemize}
...
Two blunt but effective controls:
...
Entries are an exact hostname or a subdomain wildcard (\code{*.acme.com}
matches any depth of subdomain but not \code{acme.com} itself, so list
both if you want both). The fence is enforced at every point a hostname
can be claimed, not once:
...
\begin{description}[leftmargin=1.6em,style=nextline]
  \item[Overview] Live request rate, totals, connected clients.
  ...
  \item[Tokens, Users, Organizations, Webhooks, Audit] The management
        surfaces, each mirroring an API endpoint.
  \item[Server Settings] The live-editable configuration surface. Changes
        apply immediately and persist to \code{settings.json}.
  \item[API Explorer] The generated OpenAPI document, browsable and
...
\begin{lstlisting}[language=aperioyaml]
# aperio-server.yaml
otel: true                                  # env: APERIO_OTEL
otel_endpoint: http://otel-collector:4318   # env: APERIO_OTEL_ENDPOINT
\end{lstlisting}
...
which a peer client may reach through the server:

\begin{lstlisting}[language=aperioyaml]
# ./aperio.yaml (client), on the machine next to the private service
tunnels:
...
\begin{lstlisting}[language=aperiosh]
# from any machine, with the SAME token that client connected with
aperio-client --bind-tunnels <client-id> \
  --server-url https://tunnel.example.com \
  --server-token apr_...
\end{lstlisting}

Each declared tunnel becomes a local \code{127.0.0.1} listener, by default
on the same port as the remote target, so existing tools connect without
...
\code{429} under mild load & Which limit? Server-wide, per-client, per-IP
and per-route are four different settings
...
\begin{lstlisting}[language=aperiosh]
# in a checkout, with a nightly toolchain
...
\addlinespace[2pt]
\code{APERIO\_DASHBOARD\_AUTH} & Separate dashboard-only password (username \code{aperio}), so the master token doesn't have to be shared with dashboard users. &  \\
\addlinespace[2pt]
...
\begin{longtable}{@{}p{0.24\linewidth}p{0.17\linewidth}p{0.15\linewidth}p{0.36\linewidth}@{}}
\toprule
...
\addlinespace[2pt]
\code{APERIO\_SERVE} & \code{--serve} & \code{serve} & Serve a local directory of static files with a built-in file server instead of proxying to a backend, the directory takes the place of \code{target}. \\
\addlinespace[2pt]
...
\addlinespace[2pt]
\code{APERIO\_IDLE\_TIMEOUT} & \code{--} & \code{idle\_timeout} & Retire this client after it has served nothing for this long (e.g. \code{5m}); unset = never. The scale-in half of \code{scaling:}, see \emph{Autoscaling}. The shutdown is graceful, and the clock only starts after the first request. \\
\addlinespace[2pt]
...
The complete surface under \code{/aperio}. The generated OpenAPI
document at \code{GET /aperio/api/openapi.json} is authoritative and
always matches the running build.

\begin{longtable}{@{}p{0.36\linewidth}p{0.44\linewidth}p{0.16\linewidth}@{}}
\toprule
...
\addlinespace[2pt]
\code{GET /aperio/tunnels/:client\_id} & Declared-tunnels discovery for \code{--bind-tunnels} (see \emph{Emergency Tunnels}). & the same token the client connected with (or master) \\
\addlinespace[2pt]
\code{GET /aperio} & Admin dashboard. & dashboard session \\
\addlinespace[2pt]
\code{GET /aperio/api/stats}, \code{/api/logs}, \code{/api/audit} & Live stats, request log, audit events. & dashboard session \\
\addlinespace[2pt]
...
\addlinespace[2pt]
\code{GET/POST /aperio/api/maintenance} & List / toggle per-hostname maintenance mode. & dashboard session \\
\addlinespace[2pt]
...
\addlinespace[2pt]
\code{GET /aperio/api/export} & Logical JSON dump of tokens, webhooks, users, organizations and settings overrides, a failsafe for upgrades and migrations (no statistics/sessions). & master super-admin \\
\addlinespace[2pt]
...
request id. Messages are JSON objects tagged with \code{type}; protocol
v2 additionally moves chunk payloads onto binary frames with a compact
\code{[tag][id\_len][id][payload]} header instead of base64.
...
for emergency tunnels. \\
\addlinespace[2pt]
\code{UdpOpen} / \code{UdpDatagram} / \code{UdpClose} & both & The
best-effort datagram equivalent, with an idle timeout. \\
\addlinespace[2pt]
...
Written for Aperio 0.5.0. The prose chapters are maintained alongside the
topic articles under \code{docs/}; the reference appendices were derived
mechanically from the tables in \code{docs/configuration.md}, which is
the authority for every setting and endpoint. When a setting changes,
update that file first and re-derive the appendix rather than editing
them apart.

Build it with any standard TeX distribution:

\begin{lstlisting}[language=aperiosh]
# in a checkout, from docs/book
pdflatex aperio.tex && pdflatex aperio.tex    # twice, so the TOC resolves
\end{lstlisting}
