...
\begin{description}[leftmargin=1.6em,style=nextline]
  ...
  \item[\code{bearer}] An opaque secret presented as
        ...
        withdrawing the old. Server-side for now: the handshake carries a
        client's gate as one \code{user:password}, so a client declaring
        it refuses to start rather than announcing something weaker.
\end{description}
...
readable. On the client side a policy saying more than the tunnel
handshake can carry refuses the start, rather than announcing a gate
weaker than the one written.
