...
hostname, the method, the path and the credential headers, so no secret
is held in it and a \emph{yes} for one path is never a \emph{yes} for
another. The endpoint is told the method and the path and may answer on
