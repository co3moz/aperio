...
The current protocol version is 6. Version 3 added flow control for
everything that streams; version 5 moved a buffered response body out of
JSON and into the frame that carries its envelope, and version 6 did the
same for a buffered request body, in the direction an upload travels.
...
\code{GET/POST /aperio/api/maintenance} & List / toggle maintenance mode for a hostname, a \code{*.example.com} subdomain wildcard, or \code{*} (master only). \code{reason} and \code{ttl\_seconds} are optional. & dashboard session \\
\rowsep
