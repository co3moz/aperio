...
The current protocol version is 3, and what it adds is flow control for
everything that streams.
...
\code{[tag][id\_len][id][payload]} header instead of base64, and protocol
v3, the current version, adds the per-stream pause/resume pair
(Section~\ref{sec:flow-control}).
