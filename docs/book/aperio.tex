...
\rowsep
\code{GET /aperio/api/stats}, \code{/api/logs}, \code{/api/audit} & Live stats, request log, audit events. \code{/api/audit} takes \code{event}, \code{actor}, \code{q}, \code{from}, \code{to} and \code{limit}; any of them searches the durable log (active file plus rotated generations) instead of the recent ring, newest first. & dashboard session \\
\rowsep
