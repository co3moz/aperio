...
The metrics are \code{connected\_clients}, \code{pending\_requests},
\code{store\_bytes} and \code{rss\_bytes} (Linux only, and reported at
startup as unfirable elsewhere). \code{for} applies to firing \emph{and} to
resolving, so a value resting on its threshold cannot alert every tick. Rules
emit the same \code{alert\_triggered} / \code{alert\_resolved} events with
\code{kind} set to the rule's name.
