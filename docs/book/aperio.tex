...
\rowsep
\code{APERIO\_OTEL\_BRIDGE\_LISTEN} &  & \code{otel\_bridge.listen} & Address for the client's OTLP/HTTP receiver; anything on the edge host exports to it with one environment variable. & \code{127.0.0.1:4318} \\
\rowsep
\code{APERIO\_OTEL\_BRIDGE\_LISTEN\_GRPC} &  & \code{otel\_bridge.listen\_grpc} & Address for the OTLP/gRPC receiver. Unset = no gRPC listener. &  \\
\rowsep
\code{APERIO\_OTEL\_BRIDGE\_TRANSPORT} &  & \code{otel\_bridge.transport} & \code{tunnel} sends exports on the WebSocket the client already holds; \code{https} posts them to the server's endpoint. & \code{tunnel} \\
\rowsep
\code{APERIO\_OTEL\_BRIDGE\_QUEUE} &  & \code{otel\_bridge.queue} & Exports held when the far end is not keeping up; past this the newest is dropped and counted, never waited on. & \code{256} \\
\rowsep
\code{APERIO\_STARTUP\_DELAY} &  & \code{startup\_delay} & Seconds a service waits before opening its tunnel, for a backend that starts alongside the client. Only the first connection of a parallel pool waits. & \code{0} \\
\rowsep
\code{APERIO\_PID\_FILE} &  & \code{pid\_file} & Path to write the process id to at startup, removed on a clean exit. A pid file that cannot be written is a warning, not a refusal to start. &  \\
\rowsep
\code{APERIO\_ADAPTIVE\_CONCURRENCY} &  & \code{adaptive\_concurrency} & Move the announced \code{max\_concurrent} with backend pressure: halve it while requests queue waiting for a local permit, climb back one at a time when they stop. Needs \code{max\_concurrent} set, and never leaves the band \code{1..=max\_concurrent}. & \code{false} \\
\rowsep
