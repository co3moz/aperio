...
\rowsep
\code{APERIO\_NAME} & \code{--name} & \code{name} & What to call this client on screen and in the server's logs, e.g. \code{eu\_server\_1}. A label, not an address: \code{client\_id} stays the identity failover and \code{bind\_tunnels:} recognize this process by, because those need something unique and a name is not. & the client id \\
\rowsep
