...
"may read infrastructure events" flag. \textbf{With one consequence worth
knowing:} a token granted \code{\#}, the "all topics" grant, covers the
reserved namespace as well, so a token written that way carries the
server's events whether or not that was the intention. A token that
should signal on its own subtree should say so (\code{deploy/\#}) rather
than being handed \code{\#}, which is the same advice as for any other
permission, and here it is the difference between a tenant that can talk
to its own fleet and one that is told when every token in its
