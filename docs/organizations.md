...
- **Maintenance mode and share links** are limited to hostnames the fence admits, so one tenant cannot 503 another's site or hand out access to it. For a fenced organization the allowlist is the whole test, no client has to be connected: putting up a maintenance page is most wanted precisely when nothing is up. An organization with **no** fence has no allowlist to read, so those two fall back to the older rule, one of its own clients must currently serve the hostname.
