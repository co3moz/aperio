...
**The first ten minutes.** A server with no client connected shows, above the tiles, what makes a tunnel: mint a token, run the client next to the service with it, watch it appear here, each step one click from the page that does it, with the client's command line filled in with this server's address and a link to the getting-started walkthrough. The card leaves the moment a client connects. Every page header carries a *Docs* link to the article for that page, and the login form says which credential the built-in account takes. A hostname on the Clients or Tokens page is a link that opens the site (a pattern such as `*.example.com` stays text). Creating a token shows, under the secret, the command line that runs a client with it. An overridden server setting says who last changed it and when, read from the audit log. The notification bell counts the alerts recorded since yesterday when it opens, so what fired while the tab was closed is not lost. On a phone the Clients and Live Traffic tables become a stacked list, one card per row with what a phone is pulled out for: which service, is it up, when did it last speak. Every clickable row is a keyboard one (Tab to it, Enter or Space to open it), and every status dot carries its meaning in words for a screen reader and a tooltip, so colour is never the only signal. A page fed by the stream carries an "updated 12 s ago" stamp where a *Refresh* button used to be; the button comes back only while the stream is down or for a filtered audit search, which the stream cannot carry. Clearing the webhook inbox and purging the response cache ask first, like revoking a token does. The token, user and webhook lists each have a search box. Dates are written the way the UI language writes them, in the browser's own time zone. The words the product uses that nobody outside it knows, bind, draining, ejected, canary, expose and a hostname override, carry a dotted underline where they appear as a badge or a label; hovering or focusing one shows a one-sentence definition and a link to the article. A page's explanatory paragraph folds behind the info button beside its title: open on the first visit, and closed for good once you close it (remembered in the browser), so the table it explains is at the top of the page on every visit after the first. A one-sentence description stays where it is.
...
The *Topology* page (Traffic group) draws the reverse-tunnel mesh as a live three-column map, routes → tunnel clients → backends, with a per-client live request rate on the edge, health-colored (green healthy, amber draining or failing backend probes, red unhealthy / disabled / ejected). Unlike the clients table it also shows the routing the server owns with **no live client**: static `routes:` (redirects / fixed responses) and public `expose:` ports as self-contained nodes, and token-granted binds that no client currently serves as dashed **offline** nodes, so "the service that should be up but isn't" is visible at a glance. It also draws **client-to-client dependencies**: another machine dialing a client's tunnel with `--bind-tunnels` appears as an amber node pointing at the client that declares it, so a database three machines bind locally is visibly three services that stop working when the declaring client goes away. A consumer has no client id (it opens a fresh WebSocket per connection), so it is identified by the address it dialed from, which means several processes behind one NAT or one Kubernetes node are a single node, the right grain for "who depends on this tunnel". An edge outlives its connections by fifteen minutes, because a consumer that has just finished a query is idle rather than gone, and an idle one is drawn dashed. Backed by its own `/api/topology` endpoint. See [Response Caching](caching.md) and [Routing & Load Balancing](routing-and-load-balancing.md) for the underlying concepts.
...
The buffer of fifty is shared across organizations, and eviction is by **fair share** rather than by age: when it is full, the capture dropped is the oldest one belonging to whichever organization currently holds the most. Evicting the oldest overall was fair only if every tenant is equally busy, and one org serving a thousand requests a minute used to walk a quiet org's captures out of the buffer within seconds, so a tenant investigating one request an hour could never find it. There is no per-org number to set: an org alone on the server may use the whole buffer, and two busy orgs converge on an even split by themselves.

**A capture says which client served it**, above the waterfall, named the way the clients table names it: the `custom_name` if an operator set one, else the `name` of the `services:` entry. The connection id is on the API as `client_id` and one hover away on screen, since a uuid is what an action addresses and not what a person recognizes. A client that declared neither name shows the id, which is then all there is.
...
Everything on this pane is **live**: a save applies immediately and takes effect on the running server, no restart and no reconnect. Environment variables stay the defaults; an edit becomes a **persisted override** (`APERIO_DATA_DIR/settings.json`) that survives restarts, wins over the environment, and can be reset per field back to the environment default. A key that `aperio-server.yaml` writes is the file's: the pane refuses to override it and says so, and a stored override for such a key is dropped at the next start, because the file is what an operator versions and reviews. Every change is audited as `settings_updated`.
