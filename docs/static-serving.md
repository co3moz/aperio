...
A top-level `serve:` (with no `services:` list) used to be the same thing. **A config file has not accepted it since 0.9.0**, so write it as a `services:` entry; `--serve` and `APERIO_SERVE` are unchanged. See [Configuration](configuration.md#aperioyaml--aperioyaml).
